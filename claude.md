...
## 作用域与层级合并
- 本文件位于仓库 **根目录**，适用于整个仓库。
- 允许在子目录放置更细粒度的 `CLAUDE.md`（如 `patra-ingest/CLAUDE.md`）。**“就近优先”**：离被修改文件最近的 CLAUDE.md 生效；与上层内容**自上而下合并**。
...
## 14. 变更边界（TL;DR）
	...
	• 子目录若存在 CLAUDE.md，就近优先采用更细规则（本文件为全局基线）。

## 15. 文档维护与同步
- 当你修改或新增代码、配置、脚本时，请同步检查相关文档（仓库根 README、`docs/`、模块 README、运行手册等）是否需要更新，确保描述与实现一致。
- 若本说明（`CLAUDE.md`）涉及内容已过期，或新增了需要智能体遵循的约束，请在完成变更后立刻更新对应章节，并在提交信息中标注“docs”或“CLAUDE”方便追踪。
