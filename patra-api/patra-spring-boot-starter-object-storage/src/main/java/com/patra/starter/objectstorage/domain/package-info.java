...
///   <tr><td>storageKey</td><td>String</td><td>完整存储路径,如
// "prod/patra-ingest/2025/01/15/abc.pdf"</td></tr>
...
/// ```java
/// try {
///     UploadResult result = objectStorageOps.upload(bucket, key, inputStream, metadata); catch
// (InvalidUploadRequestException ex) {
///     // 参数验证失败,不重试
///     log.error("上传参数无效: {", ex.getMessage());
///     return ResponseEntity.badRequest().body("文件上传失败: " + ex.getMessage()); catch
// (UploadFailedException ex) {
///     // 上传失败(已自动重试)
