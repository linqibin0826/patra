 ...
 * 注册表配置的分发作用域枚举。
 *
 * <p>标识注册表条目应用的级别——涵盖表达式字段、渲染规则、API 参数映射, 以及与溯源相关的设置,如端点、分页、HTTP、重试或速率限制:
 *
 * <ul>
 *   <li>{@link #SOURCE}: 应用于给定溯源内的每个任务。
 *   <li>{@link #TASK}: 应用于特定任务类型,可能覆盖溯源级别的默认值。
 * </ul>
 *
 * <p>此枚举使配置作用域在代码中显式化,避免与其他作用域值(例如授权或分析作用域)混淆。 持久化列继续存储 {@code SOURCE}/{@code TASK},因此不需要历史数据迁移。
 *
 * @author linqibin
 * @since 0.1.0
 * @see #fromCode(String)
