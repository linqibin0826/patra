...
///
/// 在 JSON 规范化过程中，将各种格式的日期时间值统一转换为标准的 ISO-8601 格式： `yyyy-MM-dd'T'HH:mm:ss.SSS'Z'`（UTC 时区）。
///
/// **支持的输入格式**:
///
/// - Unix 时间戳（秒或毫秒）
///   - ISO-8601 标准格式（如 2023-12-31T23:59:59.123Z）
///   - 本地日期时间（如 2023-12-31 23:59:59）
///   - 仅日期（如 2023-12-31）
///   - 紧凑格式（如 20231231235959）
///
...
final class TemporalCoercion {
  ...
  /// @return 转换后的 ISO-8601 字符串，如果无法转换则返回 `Optional.empty()`
