 ...
 *
 * <p>Identifies the level at which a registry entry applies—covering expression
 * fields, rendering rules, API parameter mappings, and provenance-related
 * settings such as endpoints, pagination, HTTP, retry, or rate limiting:</p>
 * <ul>
 *     <li>{@link #SOURCE}: applies to every task within a given provenance.</li>
 *     <li>{@link #TASK}: applies to a specific task type and may override
 *         provenance-level defaults.</li>
 * </ul>
 * <p>This enumeration makes the configuration scope explicit in code and avoids
 * confusion with other scope values (for example, authorization or analytics
 * scopes). Persisted columns continue to store {@code SOURCE}/{@code TASK}, so
 * no historical data migration is required.</p>
 *
 * @author linqibin
 * @since 0.1.0
 * @see #fromCode(String)
