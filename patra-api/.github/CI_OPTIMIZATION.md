...
| **缓存命中率** | ~20%（基于 commit sha） | ~80%（基于 build.gradle.kts hash） | **4倍提升** |
| **Setup Job 耗时** | 5 分钟（每次 clean build） | 1-2 分钟（增量构建） | **60-80% ↓** |
...
**优化后**：
```yaml
key: build-${{ runner.os }}-${{ hashFiles('**/*.gradle.kts', 'gradle/libs.versions.toml') }}-${{ github.sha }}
restore-keys: |
  build-${{ runner.os }}-${{ hashFiles('**/*.gradle.kts', 'gradle/libs.versions.toml') }}-  # 构建脚本不变时复用
  build-${{ runner.os }}-                                                                    # 兜底缓存
```

**效果**：
- 构建脚本不变时（大部分提交），可以复用之前的构建产物
- Setup job 从 5 分钟 → 1-2 分钟（80% 的情况）
- 缓存命中率从 ~20% → ~80%

#### 1.3 优化构建命令 - 利用 Gradle 增量构建

**实施位置**：`setup` job 的构建步骤

**优化前**：
```bash
./gradlew clean build -x test  # 每次都清除 build，重新编译
```
...
# 缓存未命中时
./gradlew clean build -x test  # 完整构建
```

**效果**：
- Gradle 增量编译比完整编译快 40-60%
- Configuration Cache 进一步加速配置阶段
- 结合缓存优化，大部分情况下都是增量编译
...
```bash
# 只构建变更的模块
./gradlew :patra-ingest:patra-ingest-boot:build :patra-registry:patra-registry-boot:build -x test
```

**效果**：
- 小改动（1-2个模块）：构建时间减少 70-80%
- 中等改动（3-5个模块）：构建时间减少 40-60%
- 大改动（build-logic 变更）：自动回退到完整构建
...
**实施位置**：`build-logic/src/main/kotlin/patra.java-base.gradle.kts` - Test 配置
...
**效果**：
- Configuration Cache 大幅减少配置阶段时间
- 并行构建充分利用多核 CPU
- Build Cache 避免重复编译

#### 3.3 可选：升级到更强的 Runner
...
| **build-logic 改动** | ~13 分钟 | ~9-11 分钟 | 15-30% |
...
可能原因：
- build.gradle.kts 或 libs.versions.toml 频繁变更
...
```bash
MODULE_MAP["新模块目录名"]="新模块Gradle路径"
```

示例：
```bash
MODULE_MAP["patra-analytics"]=":patra-analytics:patra-analytics-boot"
```
...
- [Gradle 增量构建文档](https://docs.gradle.org/current/userguide/incremental_build.html)
...
A: 可能原因：
1. 首次构建，没有缓存
2. build.gradle.kts 或 libs.versions.toml 变更，需要重新下载依赖
3. Maven Central 网络问题

### Q: 变更检测会漏掉一些模块吗？

A: 变更检测只识别直接修改的模块。如果模块 A 依赖模块 B，修改 B 不会触发 A 的重新构建（除非同时修改了 A）。这是设计行为，因为 Gradle 会自动处理依赖关系。
