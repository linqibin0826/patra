...
- 为 Codex 提供3项 MCP 服务（Sequential Thinking、Context7、Serena）的选择与调用规范，控制查询粒度、速率与输出格式，保证可追溯与安全。
...
- 主题与查询：提供 topic/关键词聚焦；tokens 默认 5000，按需下调以避免冗长（示例 topfic：hooks、routing、auth）。
...
- 无 Key 策略：可直接调用；若限流则提示并降级到本地保守答案，并请用户提供官方来源以便后续验证。
...
- 意图判定：规划/分解 → Sequential；文档/API → Context7；最新信息 → 暂不调用外部服务，提示用户提供权威来源。
...
- 选择示例：
    - React Hook 用法 → Context7；跨文件调用关系梳理 → Serena；多文件重构计划 → Sequential Thinking。
...
- 若使用 MCP，在答复末尾追加“工具调用简报”包含：
    ...
    - 重试与退避信息；来源标注（Context7 的库 ID/版本；如无外呼则省略）。
