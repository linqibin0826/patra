...
* 为 Codex 提供 4 项 MCP 服务（Sequential Thinking、DuckDuckGo、Context7、IntelliJ IDEA）的选择与调用规范，控制查询粒度、速率与输出格式，保证可追溯与安全。
...
* 主题与查询：提供 topic/关键词聚焦；tokens 默认 5000，按需下调以避免冗长。
* 筛选：多库匹配时优先信任度高与覆盖度高者；歧义时请求澄清或说明选择理由。
* 输出：精炼答案 + 引用文档段落链接或出处标识；标注库 ID/版本；给出关键片段摘要与定位；避免大段复制。
...
IntelliJ IDEA（IDEA MCP Server）

* 用途：驱动 IDE 本地能力（索引搜索、语义重构、读写文件、运行/终端、VCS 只读），支撑仓库内高可信执行与诊断。
* 触发：需要 IDE 索引/重构、运行测试或应用并收集日志、跨工程快速搜索定位。
* 权限级别：

    * R0（只读）：search_in_files、find_files、list_directory_tree、get_file_text_by_path、get_symbol_info、get_file_problems
    * R1（读写）：replace_text_in_file、reformat_file、rename_refactoring（需先统计影响面）
    * R2（执行）：execute_run_configuration、execute_terminal_command（Brave Mode 默认关闭）
* 安全边界：

    * Run Config 仅限 `Test*`、`All Tests`、`Application:*`、`Gradle:*`；
    * 终端仅允许 gradlew/mvn/git status|diff|log|rev-parse/ls/cat/echo；禁止 rm/sudo/ssh/curl|wget|docker|kubectl 等；
    * 路径沙箱：仅仓库 `/apps/**`、`/modules/**`、`/core/**`、`/config/**`、`/scripts/**`；
    * 文件类型：允许 `.java/.kt/.xml/.gradle/.kts/.yml/.yaml/.json`，二进制不可写。
* 阈值：变更文件 ≤10，单文件 ≤200 行；rename_refactoring 引用 ≤200；搜索结果 ≤200；Run Config ≤5 分钟；终端命令 ≤60 秒，输出 ≤2000 行。
* 失败与回退：IDEA 工具不可用 → 降级 DuckDuckGo 或 Context7；执行失败 → 返回关键日志与诊断建议。
* 可追溯：答复末尾追加“工具调用简报”，记录工具名、触发原因、输入摘要、关键参数、结果概览、时间戳与重试信息。
...
* IntelliJ IDEA：IDE 索引搜索、语义重构、运行/终端调用、VCS 只读。
