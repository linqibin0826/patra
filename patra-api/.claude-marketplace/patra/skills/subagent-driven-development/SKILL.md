...
**TOC 同步（控制者职责）：** 左栏 `<ol class="toc-tasks">` 内每条 `<li data-task-status="…">` 与中栏 `<article id="task-N" data-status="…">` **一一对应**。控制者每次切 article `data-status` 时必须同步切对应 toc-tasks li 的 `data-task-status`（同样是 plan HTML 由控制者单点写入），否则左栏目录状态符号与中栏卡片不一致。

## Implementation Notes 维护机制

plan 模板右栏 `<aside class="notes-panel" id="notes">` 是**实施期的偏离日志**——记录"plan 没规定、但实施时不得不做的决定 / 更改 / 权衡，以及任何用户应知事项"。这是给人类伙伴看的，也是给未来回看这份 plan 的你看的。

**职责分工**（重要——避免并发污染）：
- **控制者**追加 entry 到 `<ol class="notes-log">`、维护 `notes-stats` counter（与 data-status 一样，plan HTML 由控制者单点写入）
- **implementer 子智能体**不读不改 plan HTML，只在 DONE 报告里列 `DEVIATIONS` 字段（见 `implementer-prompt.md`）
- **spec-reviewer 子智能体**在审查时校对：implementer 报告中的偏离是否都已被控制者写进 notes-panel（见 `spec-reviewer-prompt.md`）

### 四类 entry

| `data-type` | 何时用 | 例 |
|---|---|---|
| `decision` | plan 留白处做了选择 | "plan 未指定缓存 key 前缀，采用 `patra:{service}:{entity}`" |
| `change` | 偏离了 plan 的明文要求 | "plan 要求 Flyway V12，但本地已存在 V12，改为 V13" |
| `tradeoff` | 知情让步，未来可能要还的债 | "Repository 暂未抽 interface，因当前只一个实现" |
...
| 触发点 | 动作 |
|---|---|
| implementer 报告 `DEVIATIONS` 非空 | 逐条追加为对应 `data-type` entry，**追加完才能启动 spec-reviewer**（让 reviewer 能直接对账） |
| implementer 报告 `BLOCKED` | 把 article `data-status` 改为 `"blocked"` + 追加一条 `other` entry 描述阻塞原因和建议 |
| 审查者发现实现里有偏离但 implementer 未在 DEVIATIONS 中报告 | 让 implementer 补报，控制者再追加（这表示 implementer 自审有漏，需要纠偏） |

### 怎么追加

打开 plan HTML，找到 `<ol class="notes-log">` 内 `<!-- ENTRY TEMPLATE … -->` 注释（注释保留原位），复制注释里的 `<li class="note">` 块到 `<ol>` 内，填字段：
...
<li class="note" data-type="change" data-time="2026-05-20T14:30">
  ...
  <p>plan 要求 Flyway V12，但本地已存在 V12（来自另一分支），改为 V13。理由：避免编号冲突。</p>
</li>
```

追加后：
1. 同步 `<footer class="notes-stats">` 里对应 `<span data-counter="…">` 数字 +1
2. 若 `<ol class="notes-log">` 首次有内容，删除其下方的 `<p class="empty-state">no notes yet</p>`
...
digraph process {
    ...
    subgraph cluster_per_task {
        label="每个任务";
        "同步标 article + toc li data-status='in-progress'" [shape=box];
        ...
        "同步标 article + toc li data-status='done'" [shape=box];
    }
    ...
    "读取 plan HTML，提取所有任务的完整文本，记录上下文" -> "同步标 article + toc li data-status='in-progress'";
    "同步标 article + toc li data-status='in-progress'" -> "分派实现子智能体 (./implementer-prompt.md)";
    ...
    "代码质量审查子智能体通过?" -> "同步标 article + toc li data-status='done'" [label="是"];
    "同步标 article + toc li data-status='done'" -> "还有剩余任务?";
    "还有剩余任务?" -> "同步标 article + toc li data-status='in-progress'" [label="是"];
...
2. 如果任务太大，拆分为更小的部分
3. 如果计划本身有问题，上报给人类，把 article `data-status` 改为 `"blocked"`（同步改对应 toc li `data-task-status`），并按"Implementation Notes 维护机制"在 notes-panel 追加一条 `data-type="other"` entry 描述阻塞原因和建议
...
[控制者：追加 1 条 data-type="decision" entry 到 plan HTML notes-panel，decision counter +1]
[分派规格合规审查]
规格审查者：❌ 问题：
...
- **让 implementer 子智能体改 plan HTML**——`data-status` / toc-tasks / notes-panel 都由控制者单点写入；implementer 只在 DONE 报告里列 DEVIATIONS
