...
2. 如果任务太大，拆分为更小的部分
3. 如果计划本身有问题，上报给人类，把 article `data-status` 改为 `"blocked"`，并按"Implementation Notes 维护机制"在 notes-panel 追加一条 `data-type="other"` entry 描述阻塞原因和建议
...
- **让 implementer 子智能体改 plan HTML**——`data-status` 与 notes-panel 都由控制者单点写入；implementer 只在 DONE 报告里列 DEVIATIONS
