...
- **Trace Context**: Represents the distributed tracing information that flows through the system, containing trace ID (unique per user request), correlation ID (unique per business operation), and optional parent span ID for nested operations
...
- **Sanitization Rule**: Configuration defining which data fields or patterns should be masked or removed from logs to prevent sensitive information exposure
