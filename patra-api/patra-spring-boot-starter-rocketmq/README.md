...
- **架构边界**：Starter 负责封装底层 RocketMQ 细节；业务层在 domain 定义 ChannelKey 枚举；提供 MessageHandler<T> 接口供消费端实现

## 2. 核心能力

- **强类型 Channel**：强制使用枚举定义 Channel（domain.resource.event），编译期类型检查
- **统一消息模型**：`Message<T>` 封装 eventId/traceId/occurredAt + 业务载荷
- **TraceId 自动注入**：集成 TraceProvider SPI，透传分布式追踪上下文
- **自动白名单**：启动时扫描所有 ChannelKey 枚举并注册，发送/消费时自动校验
- **命名规范校验**：Topic/Tag/ConsumerGroup 强制命名规范，避免运维混乱
- **灵活发布**：支持普通/顺序/延迟消息，可通过 Channel 或 Destination 发送
- **简化消费**：`@MessageListener` 注解驱动，自动注册监听容器
...
patra:
  messaging:
    rocketmq:
      ...
      channel:
        enforce-whitelist: true  # 强制白名单校验（默认 true）
      retry:
        max-attempts: 3
...
- **Channel 未注册**：发送时抛出 `ApplicationException`，包含已注册 channel 列表
