...
```bash
# 1. 确保临时目录存在
mkdir -p /tmp/patra-api

# 2. 写正文到临时文件（用 Write 工具写，不要 echo/printf）
#    路径：/tmp/patra-api/issue-<类型>-<时间戳>.md
#    示例：/tmp/patra-api/issue-bug-20260516-173242.md

# 3. 用 gh issue create 提交
#    必须单行命令，不允许反斜杠换行
#    必须用 -F file，禁止 -b "inline"
gh issue create -t "修复邮箱含+号时登录报500" -F /tmp/patra-api/issue-bug-20260516-173242.md -l "bug" -a @me
```
