...
|------|------|--------|
| 是否有**概念上的身份认同**？ | ✅ 是 | ❌ 否 |
| 两个对象属性相同，业务上是否相等？ | ❌ 否（是不同的个体） | ✅ 是（完全等价） |
| 是否可以被另一个同值对象替换？ | ❌ 否（需要更新原对象） | ✅ 是（替换无影响） |
| 是否需要跨时间追踪变化？ | ✅ 是 | ❌ 否 |

> ⚠️ **注意**：值对象在数据库中可能有技术 ID（主键），但这不影响它在领域层面是值对象。判断标准是**业务概念**，不是**存储实现**。
>
> **示例**：`Address` 在数据库有 `id` 主键，但两个地址如果省市区街道门牌号相同，它们在业务上就是"相等的"，可以互相替换。
