...
public final class ExternalCallConverter {
    ...
    public static ExternalCallCommand toCommand(ExternalCallRequestDTO dto) {
        ...
        HttpRequest httpRequest = new HttpRequest(
                dto.url(),
                parseHttpMethod(dto.method()),
                dto.headers(),
                dto.body()
        );

        // Convert resilience config (optional)
        ResilienceConfig resilienceConfig = dto.config() != null
                ? toResilienceConfig(dto.config())
                : null;
    ...
    private static ResilienceConfig toResilienceConfig(ResilienceConfigDTO dto) {
        return new ResilienceConfig(
                dto.timeoutSeconds() != null ? Duration.ofSeconds(dto.timeoutSeconds()) : Duration.ofSeconds(30),
                dto.maxRetries() != null ? dto.maxRetries() : 3,
                dto.retryBackoffSeconds() != null ? Duration.ofSeconds(dto.retryBackoffSeconds()) : Duration.ofSeconds(1),
                dto.rateLimit() != null ? dto.rateLimit() : 100,
                dto.circuitBreakerThreshold() != null ? dto.circuitBreakerThreshold() : 10,
                dto.circuitBreakerWindowSeconds() != null ? Duration.ofSeconds(dto.circuitBreakerWindowSeconds()) : Duration.ofMinutes(1),
    ...
    private static RateLimitStatusDTO toRateLimitStatusDTO(RateLimitStatus status) {
        ExternalRateLimitInfoDTO externalInfo = status.externalInfo() != null
                ? toExternalRateLimitInfoDTO(status.externalInfo())
                : null;

        return new RateLimitStatusDTO(
                status.limit(),
                status.remaining(),
                status.resetAfter().getSeconds(),
                externalInfo
        );
    }
    ...
    private static ExternalRateLimitInfoDTO toExternalRateLimitInfoDTO(ExternalRateLimitInfo info) {
        return new ExternalRateLimitInfoDTO(
                info.limit(),
                info.remaining(),
                info.resetTimestamp()
        );
    ...
    private static RetryAdviceDTO toRetryAdviceDTO(RetryAdvice advice) {
        return new RetryAdviceDTO(
                advice.retryable(),
                advice.suggestedDelay().getSeconds(),
                advice.reason()
        );
    }
}
