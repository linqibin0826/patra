 ...
 * @since 0.1.0
 */
public record RetryAdvice(boolean retryable, Duration suggestedDelay, String reason) {
  /**
  ...
  public static RetryAdvice fromResponse(HttpResponse response, ResilienceConfig config) {
    ...
    if (statusCode == 429 || statusCode == 503) {
      Duration delay = extractRetryAfter(response.headers()).orElse(config.retryBackoff());
      String reason = statusCode == 429 ? "Rate limited" : "Service unavailable";
  ...
  private static java.util.Optional<Duration> extractRetryAfter(Map<String, List<String>> headers) {
    ...
    try {
      ...
      // If the header is not numeric, it may use HTTP date syntax, which is intentionally not
      // parsed yet.
      return java.util.Optional.empty();
    }
  }
