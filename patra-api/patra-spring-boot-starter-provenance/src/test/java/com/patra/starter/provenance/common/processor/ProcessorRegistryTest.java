 ...
 * <ul>
 *   <li>自动注册功能测试</li>
 *   <li>Processor查找测试（getProcessor、findProcessor）</li>
 *   <li>类型支持检查测试（supports、getSupportedTypes）</li>
 *   <li>异常情况测试（重复注册、Processor不存在、空列表）</li>
 * </ul>
 *
...
class ProcessorRegistryTest {
    ...
    void should_get_processor_by_data_type() {
        ...
        DataProcessor<CanonicalLiterature> processor =
            registry.getProcessor(DataType.LITERATURE);
    ...
    void should_find_processor_with_optional() {
        ...
        // When
        Optional<DataProcessor<?>> processor =
            registry.findProcessor(DataType.LITERATURE);
    ...
    void should_return_empty_optional_when_not_found() {
        ...
        // When
        Optional<DataProcessor<?>> processor =
            registry.findProcessor(DataType.DRUG);
    ...
    void should_return_all_supported_types() {
        ...
        assertThat(types)
            .hasSize(2)
            .contains(DataType.LITERATURE, DataType.JOURNAL);
    }
    ...
    void should_detect_duplicate_registration() {
        ...
        List<DataProcessor<?>> processors = List.of(
            new MockLiteratureProcessor(),
            new AnotherLiteratureProcessor()
        );
    ...
    static class MockJournalProcessor implements DataProcessor<MockJournal> {
        ...
        @Override
        public ProcessResult<MockJournal> process(
                ProviderRequest request,
                ProviderContext context) {
            return ProcessResult.success(List.of(), null);
        }
    ...
    static class MockCitationProcessor implements DataProcessor<MockCitation> {
        ...
        @Override
        public ProcessResult<MockCitation> process(
                ProviderRequest request,
                ProviderContext context) {
            return ProcessResult.success(List.of(), null);
        }
