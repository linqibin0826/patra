 ...
 * JSON节点解析防御性工具类
 *
 * <p>PubMed/EPMC等数据源的响应载荷包含混合的节点形状（数组、带"#text"字段的对象、单例等）。 这些工具方法将它们规范化为可预测的Java类型，同时容忍缺失或格式错误的数据。
 *
 * <p>设计目标：
 *
 * <ul>
 *   <li>统一处理不同数据源的JSON响应差异
 *   <li>提供防御性解析，避免NPE或类型转换异常
 *   <li>标准化单例/数组的处理逻辑
 * </ul>
 *
 * @author linqibin
...
public final class JsonHelpers {
   ...
   * <ul>
   *   <li>普通文本节点：直接返回文本
   *   <li>对象节点：尝试提取 {"#text": "value"} 格式的值
   *   <li>缺失/null节点：返回null
   * </ul>
   *
   * @param node 源节点，可能缺失或形如 {"#text": value}
   * @return 文本表示，不可用时返回 {@code null}
   */
