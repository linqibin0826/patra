 ...
 *
 * <ul>
 *   <li>自动发现和注册DataProcessor（Spring依赖注入）
 *   <li>提供O(1)时间复杂度的Processor查找
 *   <li>类型安全的Processor获取
 *   <li>重复注册检测和警告
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>线程安全：使用ConcurrentHashMap保证并发安全
 *   <li>自动注册：Spring自动注入所有DataProcessor实现
 *   <li>灵活查找：提供getProcessor（抛异常）和findProcessor（返回Optional）两种方式
 *   <li>防御性编程：检测重复注册，处理null输入
 * </ul>
 *
...
public class ProcessorRegistry {
   ...
   *
   * <ul>
   *   <li>注册阶段：在构造函数中完成，无并发问题
   *   <li>查找阶段：多线程并发读取，ConcurrentHashMap保证安全
   * </ul>
   */
   ...
   *
   * <p>Spring会自动收集所有标注@Component的DataProcessor实现， 并注入到这个构造函数中。
   *
   ...
   *
   * <ul>
   *   <li>检测：如果DataType已注册，记录警告日志
   *   <li>保留：保留第一个注册的Processor
   *   <li>忽略：忽略后续注册的Processor
   *   <li>不抛异常：避免启动失败
   * </ul>
   *
   ...
   * 获取指定数据类型的Processor
   *
   * <p><strong>类型安全注意事项</strong>： 虽然使用了@SuppressWarnings("unchecked")，但类型安全由以下机制保证：
   *
   * <ul>
   *   <li>DataType.getDataClass()定义了预期的数据类型
   *   <li>DataProcessor实现时需确保返回类型与DataType一致
   *   <li>运行时通过DataType验证类型一致性
   * </ul>
   *
   * <p><strong>使用场景</strong>：
   *
   * <ul>
   *   <li>确定Processor存在的情况（如配置明确指定支持的类型）
   *   <li>允许抛出异常中断流程
   * </ul>
   *
   * @param <T> 数据类型
   ...
   *
   * <ul>
   *   <li>不确定Processor是否存在的情况
   *   <li>需要优雅处理Processor不存在的情况
   *   <li>避免异常处理的函数式编程风格
   * </ul>
   *
   * <p><strong>使用示例</strong>：
