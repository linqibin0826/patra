...
/// 数据源提供者执行结果包装器
///
/// 由 {@link ProvenanceDataProvider} 实现返回的统一结果封装。该结果捕获成功状态、数据载荷、游标提示和错误分类, 使采集引擎能够实施细致的重试和失败策略。
///
/// **核心特性**：
///
/// - 泛型化：支持任意数据类型（CanonicalPublication、Journal、Drug等）
///   - 数据类型标识：dataType字段标识结果的数据类型
///   - 扩展元数据：metadata字段支持自定义元数据
///   - 类型安全：工厂方法确保类型一致性
///
/// **核心职责:**
///
