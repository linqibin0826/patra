...
--    预估数据量: 1.4亿条
-- 2. cat_author_organization (作者-机构关联表) - 依赖 cat_author, cat_organization
--    预估数据量: 1.75亿条
-- ============================================================
...
-- ============================================================
-- 表 2: cat_author_organization (作者-机构关联表)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `cat_author_organization` (
    -- ========================================
    -- 业务字段
    -- ========================================
    `id` BIGINT NOT NULL COMMENT '主键,雪花算法生成',
    `author_id` BIGINT NOT NULL COMMENT '作者ID(外键:cat_author.id)',
    `org_id` BIGINT NOT NULL COMMENT '机构ID(外键:cat_organization.id)',
    `publication_id` BIGINT NULL DEFAULT NULL COMMENT '文献ID(外键:cat_publication.id,可选)',
    ...
    INDEX `idx_org` (`org_id`) COMMENT '机构索引,支持查询某机构的所有作者(中频)',
