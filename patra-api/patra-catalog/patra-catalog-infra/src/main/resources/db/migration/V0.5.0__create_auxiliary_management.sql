...
CREATE TABLE IF NOT EXISTS `cat_publication_date` (
    ...
    CONSTRAINT `chk_pub_date_precision` CHECK (`date_precision` IN ('year', 'month', 'day')),
...
CREATE TABLE IF NOT EXISTS `cat_oa_location` (
    ...
    -- 约束
    -- ========================================
    CONSTRAINT `chk_oa_location_status` CHECK (`oa_status` IN ('gold', 'green', 'hybrid', 'bronze', 'closed')),
    ...
    CONSTRAINT `chk_oa_version_type` CHECK (`version_type` IN ('publishedVersion', 'acceptedVersion', 'submittedVersion') OR `version_type` IS NULL),
