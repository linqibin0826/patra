...
-- 4. cat_venue_rating (载体评级表) - 依赖 cat_venue（新增）
-- 5. cat_venue_source_data (数据源表) - 依赖 cat_venue（新增）
-- 6. cat_venue_instance (载体实例表) - 依赖 cat_venue
-- 7. cat_publication (出版物主表) - 依赖 cat_venue_instance, cat_venue
-- 8. cat_identifier (标识符表) - 依赖 cat_publication
-- 9. cat_author (作者表) - 无依赖（独立表）
-- 10. cat_abstract (摘要表) - 依赖 cat_publication
-- ============================================================
