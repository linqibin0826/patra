...
/**
 * 物质数据库实体,映射到表 {@code cat_substance}。
 *
 * <p>表结构: 存储化学物质、药物、生物制品等信息,支持 CAS 号等注册号检索。
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code registry_number} 注册号(如 CAS 号,格式:50-78-2),唯一索引 uk_registry
 *   <li>{@code name} 物质名称(英文)
 *   <li>{@code substance_class} 物质分类(枚举:chemical/drug/biological/enzyme/antibody/protein)
 *   <li>{@code molecular_formula} 分子式(如 C9H8O4)
 *   <li>{@code synonyms} 同义词列表(JSON 数组,多语言)
 ...
 *
 * <ul>
 *   <li>{@code uk_registry} 注册号唯一索引,支持 CAS 号精确查询(<5ms)
 *   <li>{@code idx_name} 物质名称索引,支持按名称查询
 *   <li>{@code idx_class} 物质分类索引,支持按分类筛选
 * </ul>
 *
 * @author linqibin
 * @since 0.2.0
