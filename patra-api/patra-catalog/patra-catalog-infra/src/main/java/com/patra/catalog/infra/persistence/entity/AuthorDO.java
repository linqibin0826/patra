...
/**
 * 作者数据库实体,映射到表 {@code cat_author}。
 *
 * <p>表结构: 存储作者信息,支持复合去重策略(ORCID + 姓名 + 机构 + 邮箱)。
 *
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code last_name} 姓(Last Name/Family Name)
 *   <li>{@code fore_name} 名(First Name/Given Name)
 *   <li>{@code orcid} ORCID 标识符,唯一索引 uk_orcid
 *   <li>{@code email} 邮箱地址,索引 idx_email
 *   <li>{@code dedup_key} 复合去重键,应用层计算 MD5 哈希,索引 idx_dedup_key
