 ...
 * MeSH 导入转换器接口。
 *
 * <p>使用 MapStruct 进行 Domain 对象与 DO 对象之间的转换。
 *
 * <p><b>设计原则</b>：
 *
 * <ul>
 *   <li>双向转换：支持 Domain → DO 和 DO → Domain
 *   <li>强类型 ID：处理 MeshImportId 与 Long 的互转
 *   <li>枚举转换：处理枚举与字符串的互转
 *   <li>集合转换：处理 List<TableProgress> 与 List<MeshTableProgressDO> 的互转
 *   <li>Spring 集成：使用 {@code componentModel = "spring"} 注入到 Spring 容器
 * </ul>
 *
 * <p><b>转换方法</b>：
 *
 * <ul>
 *   <li>{@link #toDomain(MeshImportTaskDO, List)} - 转换为聚合根
 *   <li>{@link #toTaskDO(MeshImportAggregate)} - 转换为 TaskDO
 *   <li>{@link #toProgressDOList(MeshImportAggregate)} - 转换为 ProgressDO 列表
 *   <li>{@link #toTableProgress(MeshTableProgressDO)} - 转换为 TableProgress
 *   <li>{@link #toProgressDO(TableProgress)} - 转换为 ProgressDO
 * </ul>
 *
 * @author linqibin
 * @since 0.2.0
...
public interface MeshImportConverter {
   ...
   * 转换为领域聚合根。
   *
   * <p>从 TaskDO 和 ProgressDO 列表重建聚合根。
   *
   * @param taskDO 任务 DO
