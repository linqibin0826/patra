...
/// JCR 期刊评级 JPA Repository。
///
/// **职责**：
///
/// - 提供 JcrRatingEntity 的 CRUD 操作
...
/// @author linqibin
/// @since 0.1.0
public interface JcrRatingDao extends JpaRepository<JcrRatingEntity, Long> {
  ...
  /// @param year 评级年份
  /// @return JCR 评级实体
  Optional<JcrRatingEntity> findByVenueIdAndYear(Long venueId, Short year);

  /// 查找某期刊的所有 JCR 评级（全部年份）。
  ///
  /// @param venueId 期刊 ID
  /// @return JCR 评级列表
  List<JcrRatingEntity> findByVenueId(Long venueId);

  /// 批量查找多个期刊的 JCR 评级（ReadAdapter 分页组装用）。
  ///
  ...
  @Query(
      """
      SELECT r FROM JcrRatingEntity r
      WHERE r.venueId = :venueId
      ORDER BY r.year DESC
      LIMIT 1
      """)
  Optional<JcrRatingEntity> findLatestByVenueId(@Param("venueId") Long venueId);
}
