...
///         ├── reader: JpaPagingItemReader (有 ISSN-L 且未富化的期刊)
///         ├── processor: LetPubVenueItemProcessor (调用 LetPubEnrichmentPort 爬取)
///         └── writer: LetPubVenueItemWriter (JdbcTemplate 更新 letpub_data JSON 列)
/// ```
///
/// **设计要点**：
///
...
/// - Reader 过滤条件 `letpub_data IS NULL`：天然断点续传
...
public class LetPubEnrichmentJobConfig {
  ...
  public LetPubVenueItemProcessor letPubVenueItemProcessor() {
    return new LetPubVenueItemProcessor(enrichmentPort);
  }
  ...
  public LetPubVenueItemWriter letPubVenueItemWriter() {
    return new LetPubVenueItemWriter(jdbcTemplate, objectMapper);
  }
}
