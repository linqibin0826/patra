...
/**
 * 文献出版物数据库实体,映射到表 {@code cat_publication}。
 *
 * <p>表结构: 文献出版物核心实体,包含标识符、标题、语言、出版信息、OA 状态、作者和引用信息。
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code pmid} PubMed ID,唯一约束 uk_pmid
 *   <li>{@code doi} 数字对象标识符,唯一约束 uk_doi
 *   <li>{@code venue_id} 和 {@code venue_instance_id} 关联载体信息(冗余优化)
 *   <li>{@code language_base} 生成列,基于 {@code language_code} 自动计算
 *   <li>{@code is_oa} 和 {@code oa_status} OA 开放获取信息(冗余)
 *   <li>{@code ext_data} JSON 扩展数据字段
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
