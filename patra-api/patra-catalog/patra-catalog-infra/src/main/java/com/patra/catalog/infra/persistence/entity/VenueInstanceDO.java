...
/**
 * 载体实例数据库实体,映射到表 {@code cat_venue_instance}。
 *
 * <p>表结构: 存储载体的具体实例(期刊的卷期、书籍的版次、会议的届次)。
 *
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code venue_id} 载体 ID,外键关联 cat_venue.id
 *   <li>{@code volume} 卷号(期刊专用)
 *   <li>{@code issue} 期号(期刊专用)
 *   <li>{@code edition} 版次(书籍专用)
 *   <li>{@code publication_year} 出版年份(必填,用于冗余到主表)
 *   <li>{@code conference_name} 会议名称(会议专用)
 *   <li>{@code instance_metadata} JSON 扩展数据字段
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
