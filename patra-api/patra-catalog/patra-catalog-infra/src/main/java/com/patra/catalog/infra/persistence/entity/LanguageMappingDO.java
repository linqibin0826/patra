...
/**
 * 语言映射数据库实体,映射到表 {@code cat_language_mapping}。
 *
 * <p>表结构: 原始语言值到标准语言代码的映射表,支持动态学习和人工验证
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code raw_value} 原始语言值,唯一约束 uk_raw_value
 *   <li>{@code standard_code} 标准语言代码(ISO 639-1)
 *   <li>{@code confidence_score} 置信度(0-100),DECIMAL 映射为 BigDecimal
 *   <li>{@code usage_count} 使用次数(每次应用层查询自增)
 *   <li>{@code variant_forms} 变体形式(JSON 数组)
 *   <li>{@code metadata} 映射元数据(灵活扩展)
 * </ul>
 *
 *
 * @author linqibin
