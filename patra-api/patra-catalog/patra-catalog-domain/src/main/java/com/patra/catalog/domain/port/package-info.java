 ...
 *
 * <p>定义 Domain 层需要的外部能力接口（Port），由 Infrastructure 层实现，实现依赖倒置原则。
 *
 * <h2>职责</h2>
 * <ul>
 *   <li>定义仓储接口（Repository Port）：用于聚合根的持久化和查询
 *   <li>定义外部服务接口：用于调用外部系统能力（如文件下载、XML 解析）
 *   <li>抽象技术细节：Domain 层只依赖接口，不依赖具体技术实现（MyBatis、Spring、StAX 等）
 *   <li>支持可测试性：Application 层可以 Mock Port 接口进行单元测试
 * </ul>
 *
 * <h2>核心组件</h2>
 * <ul>
 *   <li>{@link com.patra.catalog.domain.port.MeshImportPort} - MeSH 导入任务仓储接口
 *     <ul>
 *       <li>save(MeshImportAggregate)：保存或更新导入任务聚合根</li>
 *       <li>findById(MeshImportId)：根据任务 ID 查询聚合根</li>
 *       <li>findRunningTask()：查询正在运行的任务</li>
 *       <li>existsRunningTask()：检查是否有正在运行的任务</li>
 *     </ul>
 *   </li>
 *   <li>{@link com.patra.catalog.domain.port.MeshBatchDetailPort} - MeSH 批次详情仓储接口
 *     <ul>
 *       <li>findFailedBatches(MeshImportId)：查询失败批次列表</li>
 *       <li>countByStatus(MeshImportId, MeshBatchStatus)：统计某状态批次数量</li>
 *     </ul>
 *   </li>
 *   <li>{@link com.patra.catalog.domain.port.XmlParserPort} - XML 解析器接口
 *     <ul>
 *       <li>parseDescriptors(InputStream)：流式解析主题词</li>
 *       <li>parseQualifiers(InputStream)：流式解析限定词</li>
 *       <li>parseTreeNumbers(InputStream)：流式解析树形编号</li>
 *       <li>parseEntryTerms(InputStream)：流式解析入口术语</li>
 *       <li>parseConcepts(InputStream)：流式解析概念</li>
 *     </ul>
 *   </li>
 *   <li>{@link com.patra.catalog.domain.port.MeshFileDownloadPort} - MeSH 文件下载接口
 *     <ul>
 *       <li>download(String sourceUrl)：下载 XML 文件</li>
 *       <li>validateChecksum(File xmlFile, String expectedHash)：验证文件校验和</li>
 *     </ul>
 *   </li>
 *   <li>{@link com.patra.catalog.domain.port.MeshDescriptorPort} - MeSH 主题词仓储接口
 *     <ul>
 *       <li>save(MeshDescriptorAggregate)：保存主题词聚合根</li>
 *       <li>findById(DescriptorId)：根据主题词 ID 查询</li>
 *       <li>saveBatch(List<MeshDescriptorAggregate>)：批量保存主题词</li>
 *     </ul>
 *   </li>
 * </ul>
 *
 * <h2>设计原则</h2>
 * <ul>
 *   <li><b>依赖倒置</b>：Domain 层定义接口，Infrastructure 层实现接口，依赖方向从外向内
 *   <li><b>接口隔离</b>：每个 Port 接口只定义必要的方法，不混合多种职责
 *   <li><b>面向领域</b>：Port 接口方法参数和返回值使用领域对象（聚合根、实体、值对象），不使用 DO 对象
 *   <li><b>技术无关</b>：Port 接口不暴露技术细节（如 MyBatis、JPA），保持 Domain 层纯净
 *   <li><b>流式处理</b>：解析接口返回 Stream，支持大数据量流式处理，避免内存溢出
 * </ul>
 *
 * <h2>使用示例</h2>
 * <pre>{@code
