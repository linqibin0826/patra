 ...
 * MeSH 数据类型枚举。
 *
 * <p>定义 MeSH 数据库的 5 种核心数据类型及其导入顺序。
 * 导入顺序由 {@link #importOrder} 定义，必须按照依赖关系执行。
 *
 * <p><strong>导入依赖关系：</strong>
 * <ol>
 *   <li>QUALIFIER（限定词） - 无依赖，必须最先导入</li>
 *   <li>DESCRIPTOR（主题词） - 依赖 QUALIFIER</li>
 *   <li>TREE_NUMBER（树形编号） - 依赖 DESCRIPTOR</li>
 *   <li>ENTRY_TERM（入口术语） - 依赖 DESCRIPTOR</li>
 *   <li>CONCEPT（概念） - 依赖 DESCRIPTOR</li>
 * </ol>
 *
 * @author Patra
 */
...
public enum MeshDataType {
   ...
   * <p>此值必须与以下保持一致：
   * <ul>
   *   <li>配置文件中的 key（如 {@code patra.catalog.mesh.import.batch-size-map.descriptor}）</li>
   *   <li>{@link com.patra.catalog.app.config.MeshImportConfig} 中的 Map key</li>
   *   <li>{@link com.patra.catalog.domain.model.aggregate.MeshImportAggregate#updateTableProgress} 的参数</li>
   * </ul>
   */
