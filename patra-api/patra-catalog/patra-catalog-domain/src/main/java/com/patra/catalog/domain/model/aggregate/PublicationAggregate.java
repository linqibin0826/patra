 ...
 *
 * <ul>
 *   <li>主要来源：PubMed（patra-ingest 采集，通过 RocketMQ 发布事件）
 *   <li>未来支持：EPMC、Crossref、OpenAlex 等多数据源
 *   <li>数据特点：批量导入、周期性同步、幂等处理
 * </ul>
 *
 * <p><b>一致性边界</b>：
 *
 ...
 *   <li>重复数据（相同 PMID）跳过，不覆盖已有数据
 * </ul>
 *
 * <p><b>状态转换</b>：
 *
 * <ul>
 *   <li>创建时默认为非 OA 状态（is_oa = false）
 *   <li>被引次数只能增加，不能减少（通过 incrementCitationCount）
 *   <li>被引次数同步（通过 syncCitationCount）由定期任务更新
 *   <li>OA 状态由外部 OA 位置管理器更新
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
...
public class PublicationAggregate extends AggregateRoot<Long> {
   ...
   * 创建全新的文献聚合根（用于新采集的文献）。
   *
   * <p>使用场景：patra-ingest 通过 RocketMQ 发布文献创建事件，patra-catalog 消费事件时调用此方法。
   *
   ...
   * 同步被引次数（用于定期全量同步）。
   *
   * <p><b>包私有方法</b>：仅供 Repository 层调用，外部不应直接"设置"被引数。
   *
   * <p>使用场景：定期任务从外部API（如PubMed）同步最新的被引次数。
   *
   ...
   * 同步参考文献数量（用于定期全量同步）。
   *
   * <p><b>包私有方法</b>：仅供 Repository 层调用，外部不应直接"设置"参考文献数。
   *
   * <p>使用场景：从外部数据源同步参考文献数量。
   *
   ...
   * 同步最后同步时间（由仓储层调用）。
   *
   * <p><b>包私有方法</b>：仅供 Repository 层调用。
   *
   * <p>使用场景：Repository 在保存聚合时自动更新同步时间。
   *
   * @param syncedAt 同步时间
   */
