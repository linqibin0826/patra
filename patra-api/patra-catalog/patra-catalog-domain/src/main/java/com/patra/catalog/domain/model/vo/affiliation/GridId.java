 ...
 *
 * <ul>
 *   <li>格式：grid.XXXXX.YY(其中 X 为数字,Y 为字母或数字)
 *   <li>示例：grid.38142.3c
 *   <li>GRID 是全球研究机构的标识符数据库
 *   <li>已于 2021 年合并到 ROR,但历史数据仍在使用
 * </ul>
 *
 * <p><b>设计原则</b>：
 *
 * <ul>
 *   <li>不可变性：Record 自动提供
 *   <li>格式验证：严格验证 GRID 格式
 *   <li>兼容性：支持历史数据中的 GRID ID
 * </ul>
 *
 * <p><b>使用示例</b>：
 *
...
public record GridId(String value) implements Serializable {
  ...
  public GridId {
    ...
    // GRID 格式验证
    Assert.isTrue(
        value.matches(GRID_ID_PATTERN),
        "GRID 格式无效,必须符合 'grid.XXXXX.YY' 格式：%s",
        value);
  }
