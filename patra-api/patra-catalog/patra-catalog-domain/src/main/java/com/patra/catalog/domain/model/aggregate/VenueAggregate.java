...
public class VenueAggregate extends AggregateRoot<Long> {
  ...
  private VenueAggregate(
    ...
    if (StrUtil.isNotBlank(isbn)) {
      Assert.isTrue(
          isbn.matches("^(?:97[89]-)?\\d{1,5}-\\d{1,7}-\\d{1,7}-[\\dX]$"), "ISBN格式无效：%s", isbn);
    }
  ...
  public static VenueAggregate createBook(
      String title, String isbn, String country, String publisher) {
    return new VenueAggregate(
        null,
        VenueType.BOOK,
        title,
  ...
  /// @param country 举办国家
  /// @return 会议聚合根
  public static VenueAggregate createConference(String title, String country) {
    return new VenueAggregate(
        null, VenueType.CONFERENCE, title, null, null, null, null, null, country, null);
  }
  ...
  public static VenueAggregate createOther(String title) {
    return new VenueAggregate(
        null, VenueType.OTHER, title, null, null, null, null, null, null, null);
  }
  ...
  public String toString() {
    return String.format(
        ...
        getId(), venueType.getCode(), title, getIssn(), isbn);
  }
}
