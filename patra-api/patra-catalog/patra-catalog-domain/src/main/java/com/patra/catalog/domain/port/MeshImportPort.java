 ...
 * MeSH 导入任务仓储接口（Port）。
 *
 * <p>定义聚合根的持久化操作，由 Infrastructure 层实现。
 *
 * <p><b>设计原则</b>：
 *
 * <ul>
 *   <li>纯接口定义：不包含实现逻辑
 *   <li>面向领域对象：参数和返回值都是领域对象
 *   <li>六边形架构：领域层定义接口，基础设施层实现
 *   <li>命名规范：使用 Port 后缀表示端口
 * </ul>
 *
 * <p><b>实现说明</b>：
 *
 * <ul>
 *   <li>Infrastructure 层实现此接口（MeshImportRepositoryImpl）
...
public interface MeshImportPort {
   ...
   *
   * <ul>
   *   <li>如果 aggregate.getId() 为 null，则新增任务（分配雪花 ID）
   *   <li>如果 aggregate.getId() 不为 null，则更新任务（使用乐观锁）
   *   <li>同时保存或更新关联的表进度记录（cat_mesh_table_progress）
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>查询任务主表（cat_mesh_import_task）
   *   <li>关联查询表进度记录（cat_mesh_table_progress）
   *   <li>使用 MapStruct 转换为领域对象
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>查询条件：status = 'PROCESSING'
   *   <li>关联查询表进度记录
   *   <li>如果有多个正在运行的任务，返回最早开始的任务
   * </ul>
   *
