...
public interface StreamingDownloadPort {
  ...
  /// 流式下载远程资源。
  ///
  /// 建立 HTTP 连接并返回响应体输入流，无磁盘落盘。
  /// 调用方**必须**使用 try-with-resources 确保流正确关闭。
  ///
  /// **注意事项**：
  ///
  /// - 返回的流与 HTTP 连接绑定，关闭流会释放连接
  /// - 大文件场景使用 `longRunningRestClient`（10 分钟超时）
  /// - 网络中断无法恢复，需重新调用
  ///
  /// @param url 远程资源 URL（必须是 HTTP 或 HTTPS 协议）
