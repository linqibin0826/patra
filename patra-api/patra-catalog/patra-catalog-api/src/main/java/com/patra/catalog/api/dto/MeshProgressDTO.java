 ...
 *
 * <p>用于实时监控导入进度，符合 API 契约定义 (mesh-import-api.yaml#ImportProgressResponse)。
 *
 * <p><b>设计原则</b>：
 *
 * <ul>
 *   <li>API 契约优先：严格遵循 OpenAPI 规范
 *   <li>不可变对象：使用 Record 类型保证线程安全
 *   <li>跨模块共享：放在 api 模块，供其他服务调用
 *   <li>扁平化设计：避免过度嵌套，便于前端使用
 * </ul>
 *
 * <p><b>核心字段</b>：
 *
 * <ul>
 *   <li>overallProgress - 整体进度百分比（0.0 ~ 100.0）
 *   <li>processSpeed - 处理速度（记录/秒）
 *   <li>estimatedRemainingSeconds - 预计剩余时间（秒）
 *   <li>tableProgress - 各表进度列表
 *   <li>failedBatches - 失败批次列表（用于错误追踪）
 * </ul>
 *
 * @author linqibin
