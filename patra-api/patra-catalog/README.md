...
7. **PubMed Venue 数据导入**
   ...
   - **Wikidata 中文标题富化**：通过 SPARQL 查询 Wikidata，按 ISSN-L 批量获取期刊中文标题（`ChineseTitleQueryPort` → `WikidataChineseTitleQueryAdapter`）
...
- `WikidataChineseTitleQueryAdapter`：Wikidata 中文标题查询适配器（实现 `ChineseTitleQueryPort`，通过 SPARQL 批量查询期刊中文标题）
...
- **Venue（载体）相关表**
  - `cat_venue`：载体主表（含 title_zh 中文标题字段，4 个 JSON 嵌入式值对象：publication_profile、citation_metrics、open_access、affiliated_societies）
