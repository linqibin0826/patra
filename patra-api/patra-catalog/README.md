...
9. **PubMed Baseline 文献批量导入**（Spring Batch 实现）
   - 使用 Spring Batch 实现大规模 PubMed 文献导入（2025 年约 3,700 万条）
   - **流式下载**：从 NLM FTP 流式下载 gzip 压缩 XML（无磁盘落盘）
   - **单文件模式**：每次 Job 执行只处理一个文件（1,274 个文件可并行调度）
   - **VenueCache 双索引**：NLM ID + ISSN 组合匹配，减少 95%+ 数据库查询
   - **VenueInstance 自动创建**：按卷期自动创建或复用期刊实例
   - 支持断点续传（Job 级别幂等执行）
   - XXL-Job 调度入口（`pubmedBaselineImportJob`，参数：`fileIndex=1~1274`）
