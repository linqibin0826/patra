...
4. **MeSH 限定词导入**
   ...
   - XXL-Job 调度入口（`catalogMeshQualifierImportJob`）
...
### v0.5.0 (2025-11-26)
- 新增：MeSH 限定词（Qualifier）导入功能
- 新增：XXL-Job 调度入口 `catalogMeshQualifierImportJob`
- 新增：`MeshQualifierImportCommand` 和 `MeshQualifierImportResult` 命令/结果对象
- 新增：`XmlParserPort.parseQualifiers(Path, String)` 支持从文件路径解析
- 修复：XXE 安全漏洞（OWASP A03:2021），禁用 XMLInputFactory 外部实体
- 修复：硬编码 MeSH 版本号问题，改用动态传入的版本参数

### v0.4.0 (2025-11-26)
- 增强：MeSH 导入支持从 HTTP/HTTPS URL 下载文件（XXL-Job 参数 `filePath` 改为 `url`）
- 新增：`FileDownloadPort` 接口和 `FileDownloadRestClientAdapter` 实现
- 新增：`MeshImportJobExecutionListener` 批处理完成后自动清理临时文件
- 新增：`MeshImportResult` 增加 `sourceUrl` 字段区分源 URL 和本地文件路径

### v0.3.0 (2025-11-26)
- 新增：使用 Spring Batch 实现 MeSH 数据批量导入
- 新增：支持断点续传（基于 Job Execution 幂等性）
- 新增：支持增量导入和全量重导入两种模式
- 新增：批量 SQL 插入优化，显著提升导入性能
- 新增：XXL-Job 调度集成（MeshImportScheduleJob）
- 新增：MeshImportParams 值对象封装导入参数
