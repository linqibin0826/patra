...
7. **PubMed Venue 数据导入**
   ...
   - **OpenAlex 富化**：通过 REST API 查询 OpenAlex Sources，按 ISSN-L 批量获取期刊引用指标（H 指数、i10 指数、两年平均被引）及年度发文统计（`OpenAlexEnrichmentQueryPort` → `OpenAlexEnrichmentQueryAdapter`）
...
- **查询传输值对象**（非持久化，用于跨端口传递查询结果）
  ...
  - `VenueOpenAlexEnrichment`：OpenAlex 富化数据（OpenAlex Source ID、引用指标快照、年度发文统计）
