...
5. **OpenAlex Venue 数据批量导入**（Spring Batch 实现）
   ...
   - 写入策略为 Upsert（增量更新），支持断点续传
   - **支持对象存储缓存**：优先从 MinIO 读取，减少 OpenAlex S3 远程下载
   - 支持增量导入（INCREMENTAL）和全量重导入（TRUNCATE_REIMPORT）模式
...
OpenAlex Venue 源文件（gzip 压缩的 JSONL 分区文件）支持对象存储缓存，减少从 OpenAlex S3 公开存储桶下载的频率。
...
1. 检查对象存储（MinIO）中是否存在缓存文件
2. **缓存命中**：从对象存储下载到本地临时目录
3. **缓存未命中**：从 OpenAlex S3 下载，并异步上传到对象存储作为缓存
4. **静默降级**：缓存检查/下载失败时自动回退到远程下载（记录 warn 日志）
...
**示例**：
- Manifest 文件 → `openalex/sources/manifest`
- 分区文件 → `openalex/sources/updated_date=2025-11-02/part_000.gz`
...
patra:
  ...
  catalog:
    openalex-cache:
      enabled: true                                    # 是否启用缓存（默认 false）
      bucket: patra-catalog-cache                      # 存储桶名称
      key-prefix: openalex/sources                     # 对象键前缀
      s3-base-url: https://openalex.s3.amazonaws.com   # OpenAlex S3 基地址
      s3-sources-path: data/sources                    # S3 中 Sources 路径
```
...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `patra.catalog.openalex-cache.enabled` | boolean | `false` | 是否启用缓存 |
| `patra.catalog.openalex-cache.bucket` | String | `patra-catalog-cache` | 存储桶名称 |
| `patra.catalog.openalex-cache.key-prefix` | String | `openalex/sources` | 对象键前缀 |
| `patra.catalog.openalex-cache.s3-base-url` | String | `https://openalex.s3.amazonaws.com` | OpenAlex S3 基地址 |
| `patra.catalog.openalex-cache.s3-sources-path` | String | `data/sources` | S3 中 Sources 路径 |

### 依赖关系

- **必需依赖**：`patra-spring-boot-starter-object-storage`（对象存储）
- **必需配置**：`patra.async.pools.cache-upload`（异步上传线程池）
- **推荐配置**：`patra.catalog.openalex-cache.enabled=true`（启用缓存）

## 📝 变更日志
1. v0.5.0 (2025-12-03)：OpenAlex Venue 批量导入
   ...
   - 新增 `OpenAlexCacheProperties`：OpenAlex 文件缓存配置属性
   - 重命名 `MeshDescriptorImportMode` → `DataImportMode`：通用化导入模式枚举
