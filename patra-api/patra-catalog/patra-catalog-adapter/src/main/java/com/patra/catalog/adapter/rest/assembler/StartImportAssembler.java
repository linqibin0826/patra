 ...
 *
 * <p>职责：将外部 HTTP 请求（{@link StartImportRequest}）转换为内部应用层命令（{@link StartImportCommand}）。
 *
 * <p>装配逻辑：
 *
 * <ul>
 ...
 * </ul>
 *
 * <p><b>设计模式</b>：DTO Assembler（Martin Fowler - Patterns of Enterprise Application
 * Architecture）
 *
 * <p><b>架构层次</b>：Adapter 层 → Application 层
 *
 * <p><b>职责边界</b>：
 *
 * <ul>
 *   <li>✅ 数据格式转换（Request → Command）
 *   <li>✅ 应用默认值和配置
 *   <li>✅ 数据规范化（trim、null 处理）
 *   <li>❌ 业务逻辑校验（由 Domain 层负责）
 *   <li>❌ 参数格式校验（由 Request 的 @Valid 注解负责）
 * </ul>
 *
 * @author linqibin
 * @since 0.2.0
