 ...
 *
 * <ul>
 *   <li>声明数据来源为 PUBMED(来自 ProvenanceCode 枚举)
 *   <li>声明操作类型为 HARVEST(原始数据采集)
 *   <li>暴露 XXL-Job 执行入口点(@XxlJob 注解)
 *   <li>委托参数解析和业务编排给基类模板方法
 * </ul>
 *
 * <p>调度策略: 由 XXL-Job 调度中心配置(通常按日/周定期执行),支持手动触发和失败重试。
 *
 * <p>设计模式: 模板方法模式 - 子类只需定义数据来源和操作类型,具体执行流程由基类统一处理。
 */
@Slf4j
@Component
