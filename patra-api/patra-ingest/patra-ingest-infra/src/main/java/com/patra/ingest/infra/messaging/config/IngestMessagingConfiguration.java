...
/**
 * Ingest 模块消息通道注册配置。
 * <p>自动从领域层 {@link IngestPublishingChannels} 枚举收集所有 channel 并注册到 RocketMQ Starter 的白名单中。
...
public class IngestMessagingConfiguration {
     ...
     * <p>自动从 {@link IngestPublishingChannels} 枚举提取所有 channel，确保与领域定义保持一致。
     */
    @Bean
    public ChannelCatalog ingestChannelCatalog() {
        return () -> Arrays.stream(IngestPublishingChannels.values())
