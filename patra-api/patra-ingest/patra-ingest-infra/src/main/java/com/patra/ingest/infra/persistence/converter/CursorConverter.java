...
public interface CursorConverter {
    ...
    @Mapping(target = "namespaceScopeCode", source = "namespaceScope", qualifiedByName = "namespaceScopeToCode")
    @Mapping(target = "cursorValue", source = "value.raw")
    @Mapping(target = "normalizedInstant", expression = "java(com.patra.ingest.infra.persistence.converter.CursorConverter.normalizedInstant(source))")
    @Mapping(target = "normalizedNumeric", expression = "java(com.patra.ingest.infra.persistence.converter.CursorConverter.normalizedNumeric(source))")
    @Mapping(target = "observedMaxValue", expression = "java(com.patra.ingest.infra.persistence.converter.CursorConverter.observedMaxValue(source))")
    ...
    static Cursor toCursor(CursorDO entity) {
        ...
        CursorValue value = new CursorValue(type,
                entity.getCursorValue(),
                entity.getNormalizedInstant(),
                entity.getNormalizedNumeric());
        CursorWatermark watermark = new CursorWatermark(entity.getObservedMaxValue(),
                entity.getNormalizedInstant(),
                entity.getNormalizedNumeric());
        CursorLineage lineage = new CursorLineage(entity.getScheduleInstanceId(),
                entity.getPlanId(),
                entity.getSliceId(),
                entity.getTaskId(),
                entity.getLastRunId(),
                entity.getLastBatchId());
        return Cursor.restore(entity.getId(),
