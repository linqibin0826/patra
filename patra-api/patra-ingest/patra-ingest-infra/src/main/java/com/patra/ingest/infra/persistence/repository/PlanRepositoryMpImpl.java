 ...
 *
 * <ul>
 *   <li>使用 {@link PlanDO} 作为持久化模型
 *   <li>通过 {@link PlanConverter} 进行聚合根与 DO 转换
 *   <li>根据 ID 是否存在决定 insert 或 update
 *   <li>依赖 MyBatis-Plus 的 version 字段实现乐观锁
 * </ul>
 *
 * <p>日志策略:
 *
 * <ul>
 *   <li>DEBUG: 记录 insert/update 操作的关键字段(id, planKey)
 *   <li>INFO: 避免高频 CRUD 产生日志噪音
 * </ul>
 *
 * <p>线程安全: 无状态单例,通过依赖注入实现线程安全。
 *
 * @author linqibin
 * @since 0.1.0
 */
...
public class PlanRepositoryMpImpl implements PlanRepository {
   ...
   * 保存采集计划。
   *
   * <p>根据聚合根 ID 是否存在决定插入或更新。转换为 DO 后再转回聚合根,以确保 version 和自增字段被正确回写。
   *
   * @param plan 计划聚合根(必需)
   * @return 持久化后的聚合根
   */
