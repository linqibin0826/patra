...
/// 索引假设(用于 SQL 优化):
///
/// - `idx_message_id(message_id, started_at DESC)`: 按消息查询日志
///   - `idx_batch_id(relay_batch_id)`: 按批次查询日志
///   - `idx_channel_time(channel, started_at)`: 按通道和时间范围查询日志
...
public interface OutboxRelayLogMapper extends BaseMapper<OutboxRelayLogDO> {
  ...
  ///
  /// 使用索引: `idx_message_id(message_id, started_at DESC)`
  ///
  ...
  ///
  /// 使用索引: `idx_batch_id(relay_batch_id)`
  ///
  ...
  /// 使用场景:
  ///
  /// - 监控仪表盘: "INGEST 通道在最近 1 小时内的成功率"
  ///   - 告警查询: "最近 10 分钟内有多少 FAILED 中继?"
  ///
  /// 使用索引: `idx_channel_time(channel, started_at)` 或 {@code idx_status(relay_status,
  /// started_at)}
  ///
  ...
  /// 查询最近失败的中继日志(用于告警)。
  ///
  /// 使用场景: 值班分流 - "显示最近 10 个失败"
  ///
  /// 使用索引: `idx_status(relay_status='FAILED', started_at DESC)`
  ///
  /// @param channel 通道名称过滤器(NULL = 所有通道)
  ...
  ///
  /// 使用场景: 历史回顾 - "显示 INGEST 通道今天的所有中继尝试"
  ///
  /// 使用索引: `idx_channel_time(channel, started_at)`
  ///
  /// @param channel 通道名称过滤器(NULL = 所有通道)
  /// @param startTime 时间范围起始(含)
  /// @param endTime 时间范围结束(不含)
