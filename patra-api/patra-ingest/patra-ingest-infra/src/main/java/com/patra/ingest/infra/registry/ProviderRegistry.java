 ...
 * <ul>
 *   <li>二维索引：(provenanceCode, dataType) → Provider</li>
 *   <li>一对多支持：一个Provider可以支持多个DataType</li>
 *   <li>自动发现和注册Provider（Spring依赖注入）</li>
 *   <li>O(1)时间复杂度的查找</li>
 *   <li>线程安全设计</li>
 * </ul>
 *
...
public class ProviderRegistry {
    ...
    private void register(DataSourceProvider provider) {
        ...
        if (typesByProvenance.containsKey(provenanceCode)) {
            log.warn("检测到重复的provenanceCode: {}, existing={}, new={}",
                provenanceCode,
        ...
        log.debug("注册Provider: code={}, types={}, impl={}",
            provenanceCode, supportedTypes, provider.getClass().getSimpleName());
    }
    ...
    public DataSourceProvider getProvider(String provenanceCode, DataType dataType) {
        ...
        if (provider == null) {
            throw new ProviderNotFoundException(
                String.format("未找到Provider: provenanceCode=%s, dataType=%s",
    ...
    private String normalizeProvenanceCode(String provenanceCode) {
        return provenanceCode != null
            ? provenanceCode.toLowerCase().trim()
            : "";
    }
    ...
    private void logRegistrationSummary() {
        ...
        log.info("Provider注册完成: 共注册{}个Provider, {}个(provenanceCode, dataType)组合",
            providerCount, combinationCount);

        // 详细日志
        typesByProvenance.forEach((code, types) -> {
    ...
    private record ProviderKey(String provenanceCode, DataType dataType) {
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equals(provenanceCode, that.provenanceCode) &&
