...
public interface IngPlanMapper extends BaseMapper<IngPlanDO> {
    ...
    IngPlanDO findByPlanKey(@Param("planKey") String planKey);
    
    /**
     * 根据调度实例ID查找计划列表。
     */
    @Select("SELECT * FROM ing_plan WHERE schedule_instance_id = #{scheduleInstanceId} AND deleted = 0 ORDER BY created_at")
    List<IngPlanDO> findByScheduleInstanceId(@Param("scheduleInstanceId") Long scheduleInstanceId);
    ...
    @Select("""
        ...
        """)
    List<IngPlanDO> findActiveByProvenanceAndOperation(
