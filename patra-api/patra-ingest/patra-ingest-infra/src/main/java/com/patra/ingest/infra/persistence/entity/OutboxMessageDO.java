...
///   - 排序/分区: `partition_key` 建议格式为 "`provenance:operation`",由索引 `idx_outbox_partition(channel,
// partition_key, status_code)` 利用以控制并行度和保序(如 `PUBMED:HARVEST`)
...
///
/// 建议状态机: `PENDING → PUBLISHING → PUBLISHED`;失败进入 `FAILED`。基于退避策略,计算 `next_retry_at` 以返回
// `PENDING`,或根据策略标记为 `DEAD`。
///
...
public class OutboxMessageDO extends BaseDO {
  ...
  ///
  /// 典型字段:
  // `taskId`/`sliceKey`/`planKey`/`provenance`/`operation`/`endpoint`/`priority`/`notBefore`。
  ///
