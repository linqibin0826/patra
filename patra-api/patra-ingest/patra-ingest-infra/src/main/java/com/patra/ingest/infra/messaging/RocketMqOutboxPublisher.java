...
@ConditionalOnProperty(
    name = "papertrace.ingest.outbox.publisher",
...
public class RocketMqOutboxPublisher implements OutboxPublisherPort {
  ...
  public RocketMqOutboxPublisher(
      StreamBridge streamBridge, ObjectMapper objectMapper, OutboxMqProperties properties) {
  ...
  public PublishResult publish(OutboxMessage message, RelayPlan plan) {
    ...
    try {
      ...
      if (!sent) {
        throw new OutboxPublishException(
            Reason.SEND_FAILED, "StreamBridge returned false when sending to channel " + channel);
      }
      if (log.isDebugEnabled()) {
        log.debug(
  ...
  private Message<String> buildMessage(OutboxMessage message) {
    MessageBuilder<String> builder =
        MessageBuilder.withPayload(defaultPayload(message))
  ...
  private Map<String, Object> parseHeaders(String headersJson) {
    ...
    try {
      ...
      throw new OutboxPublishException(
          Reason.HEADERS_INVALID, "Failed to parse Outbox headers JSON", ex);
    }
  }
}
