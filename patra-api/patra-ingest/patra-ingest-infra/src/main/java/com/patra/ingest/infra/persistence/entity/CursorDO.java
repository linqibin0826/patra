...
public class CursorDO extends BaseDO {
    ...
    /** 原始游标值（JSON 或原始字符串表示） */
    @TableField("cursor_value") private String cursorValue;
    /** 观测到的最大值（用于推进策略判断） */
    @TableField("observed_max_value") private String observedMaxValue;
    /** 标准化时间值（若类型=TIME） */
    @TableField("normalized_instant") private java.time.Instant normalizedInstant;
    /** 标准化数值（若类型=NUMERIC） */
    @TableField("normalized_numeric") private java.math.BigDecimal normalizedNumeric;
    /** 最近关联的调度实例 ID（追踪更新来源） */
    ...
    /** 最近批次 ID */
    @TableField("last_batch_id") private Long lastBatchId;
