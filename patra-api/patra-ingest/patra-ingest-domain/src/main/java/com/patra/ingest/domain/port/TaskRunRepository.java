...
public interface TaskRunRepository {
  ...
  ///
  /// @param taskId 任务标识符
  /// @return 最大尝试编号,或 `0`(无执行记录时)
  int getLatestAttemptNo(Long taskId);

  /// 根据标识符查询执行尝试。
  ///
  /// **业务含义**: 通过技术主键(ID)检索执行尝试。
  ///
  /// @param runId run 标识符
  ...
  ///
  /// **业务含义**: 判断任务是否已成功执行过,用于幂等性检查。
  ///
  /// @param taskId 任务标识符
  /// @return `true` 表示存在 `SUCCEEDED` 执行记录
