...
///
/// **职责**: 持久化和查询采集/清洗流程的 Cursor 水位线(watermark),确保:
///
/// - 去重保证 - 避免重复采集相同数据窗口
///   - 窗口连续性 - 防止窗口漂移或遗漏
///   - 增量采集 - 支持从上次中断位置继续
...
public interface CursorRepository {
  ...
  /// 根据业务主键查询 Cursor。
  ///
  /// **业务含义**: 通过五元组(provenance + operation + cursorKey + namespace)定位唯一 Cursor。
  ///
  ...
  /// 持久化或更新 Cursor 状态。
  ///
  /// **业务含义**: 保存 Cursor 的当前水位线、命名空间、版本信息等。
  ///
  ...
  ///
  /// **业务含义**: 查询全局时间 Cursor 的最新水位线,用于跨数据源的时间对齐。
  ///
