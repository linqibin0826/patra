 ...
 * Ingest 领域模型 - 聚合根包。
 *
 * <p>本包包含数据采集领域的所有聚合根(Aggregate Roots),每个聚合根代表一个事务一致性边界和状态机流转单元。 聚合根是 DDD
 ...
 *       <ul>
 *         <li>封装数据采集计划的窗口规范、切片策略和配置快照
 *         <li>状态机: DRAFT → SLICING → READY → COMPLETED/PARTIAL/FAILED
 *         <li>通过 planKey 哈希保证幂等性
 *       </ul>
 *   <li>{@link com.patra.ingest.domain.model.aggregate.TaskAggregate} - 任务执行聚合根
 *       <ul>
 *         <li>原子工作单元,包含租约管理、执行追踪和断点续传
 *         <li>状态机: QUEUED → RUNNING → SUCCEEDED/FAILED/PARTIAL
 *         <li>支持技术重试和补偿机制
 *       </ul>
 *   <li>{@link com.patra.ingest.domain.model.aggregate.PlanSliceAggregate} - 计划切片聚合根
 *       <ul>
 *         <li>计划的分片单元,将大窗口拆分为可并行执行的切片
 *         <li>状态机: PENDING → ASSIGNED → FINISHED
 *         <li>追踪切片级别的执行进度
 *       </ul>
 *   <li>{@link com.patra.ingest.domain.model.aggregate.ScheduleInstanceAggregate} - 调度实例聚合根
 *       <ul>
 *         <li>追踪外部调度器(XXL-Job)触发的单次执行
 *         <li>关联 trigger 参数、执行结果和耗时
 *         <li>支持调度器级别的幂等性
 *       </ul>
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>一致性边界</b>: 聚合内部保证事务一致性,聚合之间最终一致
 *   <li><b>封装状态</b>: 所有状态变更通过行为方法,禁止 public setter
 *   <li><b>ID 引用</b>: 聚合之间仅通过 ID 引用,不持有对象引用
 *   <li><b>领域事件</b>: 状态转换时发布领域事件,支持异步解耦
 *   <li><b>乐观锁</b>: 继承 {@code AggregateRoot} 的版本控制机制
 * </ul>
 *
 * <h2>状态机模式</h2>
 *
 ...
 *
 * <ul>
 *   <li>{@code create(...)}: 创建全新聚合根,分配初始状态和 ID
 *   <li>{@code restore(...)}: 从持久化存储恢复聚合根,携带版本号
 * </ul>
 *
 * <pre>{@code
 * // 创建新聚合
 ...
 * <h2>线程安全</h2>
 *
 * <p>聚合根不是线程安全的,应在单线程中操作。并发控制通过乐观锁版本号实现, 由基础设施层的仓储在保存时检查版本冲突。
 *
