 ...
 *
 * <p>Combines multiple registry sub-domain tables into a single immutable view so a scheduler
 * execution observes a consistent, replayable configuration.
 *
 * <p>Selection rule: for each dimension choose the latest row where {@code NOW()} falls within
 * {@code [effective_from, effective_to)} and {@code lifecycle=ACTIVE} and {@code deleted=0}
 * (ordered by {@code effective_from DESC}, {@code id DESC}). Credential dimensions may return
 * multiple rows.
 *
 * <p>Included dimensions (table → nested record): reg_provenance → ProvenanceInfo;
 * reg_prov_window_offset_cfg → WindowOffsetConfig; reg_prov_pagination_cfg → PaginationConfig;
 * reg_prov_http_cfg → HttpConfig; reg_prov_batching_cfg → BatchingConfig; reg_prov_retry_cfg →
 * RetryConfig; reg_prov_rate_limit_cfg → RateLimitConfig.
 *
 * @author linqibin
 * @since 0.1.0
 */
public record ProvenanceConfigSnapshot(
  ...
  /**
   * Provenance metadata (reg_provenance). Dictionary: lifecycle_status =
  ...
  public record ProvenanceInfo(
      ...
      /* Lifecycle status (lifecycle_status: DRAFT|ACTIVE|DEPRECATED|RETIRED) */ String
          lifecycleStatusCode) {}

  /**
   * Window & Offset configuration (reg_prov_window_offset_cfg). Dictionary: window_mode =
   * SLIDING|CALENDAR; time_unit = SECOND|MINUTE|HOUR|DAY; offset_type = DATE|ID|COMPOSITE;
   * lifecycle_status as above.
   */
  ...
  /**
   * Pagination / Cursor / Token / Scroll configuration (reg_prov_pagination_cfg). Dictionary:
   * pagination_mode = PAGE_NUMBER|CURSOR|TOKEN|SCROLL; lifecycle_status as above.
   */
  public record PaginationConfig(
      ...
      /* Effective from (inclusive) */ Instant effectiveFrom,
      /* Effective to (exclusive; NULL=long-term) */ Instant effectiveTo,
      /* Pagination mode (pagination_mode: PAGE_NUMBER|CURSOR|TOKEN|SCROLL) */ String
          paginationModeCode,
  ...
  /**
   * HTTP policy configuration (reg_prov_http_cfg). Dictionary: retry_after_policy =
   * IGNORE|RESPECT|CLAMP; lifecycle_status as above.
   */
  public record HttpConfig(
      ...
      /* Retry-After handling policy (retry_after_policy: IGNORE|RESPECT|CLAMP) */ String
      ...
      /* Idempotency key TTL seconds (if client/server supports) */ Integer
          idempotencyTtlSeconds) {}

  /**
   * Batching & request shaping configuration (reg_prov_batching_cfg). Dictionary: lifecycle_status
   * as above.
   */
  ...
  /**
   * Retry and backoff configuration (reg_prov_retry_cfg). Dictionary: scope = SOURCE|TASK;
   * backoff_policy_type = FIXED|EXP|EXP_JITTER|DECOR_JITTER; lifecycle_status as above.
   */
