...
///
/// 用于将同一作业执行的所有中继日志分组,支持批次级别的统计和故障排查。
///
/// **格式:** `yyyyMMddHHmmss-xxxxxxxx`
///
/// - yyyyMMddHHmmss: UTC 时间戳(中继批次触发时间)
///   - xxxxxxxx: 8 位随机十六进制 UUID(保证唯一性)
///
...
public final class RelayBatchId {
  ...
  ///
  /// 生成的 ID 通过随机 UUID 组件保证并发调用下的唯一性。
  ///
  ...
  /// 提取批次 ID 的时间戳部分。
  ///
  /// 示例: `20251031150000-a1b2c3d4` → `20251031150000`
  ///
  ...
  /// 提取批次 ID 的 UUID 部分。
  ///
  /// 示例: `20251031150000-a1b2c3d4` → `a1b2c3d4`
  ///
