...
public class TaskRunBatch {
   ...
   * @param fetchedCount number of records retrieved
   * @param nextCursorToken next cursor token for pagination
   * @param errorMessage error message when the batch failed
   * @param storageKey storage reference (e.g., object storage path)
   * @return {@link TaskRunBatch} instance
   */
  public static TaskRunBatch create(
      Long runId,
      int batchNo,
      boolean success,
      int fetchedCount,
    ...
    // Generate idempotent key: SHA256(runId|batchNo)
    // This ensures deduplication on retries (per DB schema: SHA256(run_id + before_token |
    // page_no))
    String idempotentMaterial = runId + "|" + batchNo;
    IdempotentKey idempotentKey = new IdempotentKey(HashUtils.sha256Hex(idempotentMaterial));

    return new TaskRunBatch(
        null, // id - generated by the database
        runId,
        null, // taskId - optional
        null, // sliceId - optional
        null, // planId - optional
        null, // provenanceCode - optional
        null, // operationCode - optional
        batchNo,
        null, // pageNo - optional
        null, // pageSize - optional
        null, // beforeToken
        nextCursorToken, // afterToken
        null, // exprHash - optional
        idempotentKey,
        status,
        stats,
        now, // committedAt
