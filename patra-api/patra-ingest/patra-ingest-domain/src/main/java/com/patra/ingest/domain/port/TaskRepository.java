...
public interface TaskRepository {
  ...
  ///
  /// **前置条件**: `WHERE lease_owner=#{owner`}。更新 `status_code='RUNNING'`, `started_at`,
  // `last_heartbeat_at`, `leased_until`。
  ///
  /// @param taskId 任务标识符
  /// @param owner 租约拥有者
  /// @param now 当前时间戳
  ...
  /// 通过心跳续期租约。
  ///
  /// **前置条件**: `WHERE lease_owner=#{owner`}。更新 `leased_until`, `last_heartbeat_at`,
  // `lease_count`+1。
  ///
  /// @param taskId 任务标识符
  /// @param owner 租约拥有者
