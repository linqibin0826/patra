 ...
 * Ingest 领域模型 - 枚举包。
 *
 * <p>本包包含数据采集领域的所有枚举类型,用于定义状态机、业务代码和分类常量。枚举类型是类型安全的常量集合,
 * 封装了领域特定的业务逻辑和状态转换规则。
 *
 * <h2>状态机枚举</h2>
 *
 ...
 *       <ul>
 *         <li>DRAFT(草稿) → SLICING(切片中) → READY(就绪) → COMPLETED/PARTIAL/FAILED</li>
 *         <li>用于追踪计划从创建到执行完成的完整生命周期</li>
 *       </ul>
 *   </li>
 *   <li>{@link com.patra.ingest.domain.model.enums.TaskStatus} - 任务状态机
 *       <ul>
 *         <li>QUEUED(已排队) → RUNNING(运行中) → SUCCEEDED/FAILED/PARTIAL</li>
 *         <li>支持 isTerminal() 判断是否为终态</li>
 *       </ul>
 *   </li>
 *   <li>{@link com.patra.ingest.domain.model.enums.TaskRunStatus} - 任务执行状态机
 *       <ul>
 *         <li>RUNNING(运行中) → SUCCEEDED(成功)/FAILED(失败)/PARTIAL(部分成功)</li>
 *         <li>用于 TaskRun 实体的执行追踪</li>
 *       </ul>
 *   </li>
 *   <li>{@link com.patra.ingest.domain.model.enums.SliceStatus} - 切片状态机
 *       <ul>
 *         <li>PENDING(待处理) → ASSIGNED(已分配) → FINISHED(已完成)</li>
 *         <li>用于追踪计划切片的分配和执行</li>
 *       </ul>
 *   </li>
 *   <li>{@link com.patra.ingest.domain.model.enums.OutboxStatus} - Outbox 消息状态机
 *       <ul>
 *         <li>PENDING(待发布) → PUBLISHED(已发布)/FAILED(失败)/DEFERRED(延迟)</li>
 *         <li>用于事务性消息发布模式</li>
 *       </ul>
 *   </li>
 *   <li>{@link com.patra.ingest.domain.model.enums.RelayStatus} - 中继状态机
 *       <ul>
 *         <li>PENDING(待中继) → SUCCESS(成功)/FAIL(失败)</li>
 *         <li>用于 Outbox Relay 模式的补偿机制</li>
 *       </ul>
 *   </li>
 *   <li>{@link com.patra.ingest.domain.model.enums.BatchStatus} - 批次状态机
 *       <ul>
 *         <li>PROCESSING(处理中) → SUCCESS(成功)/PARTIAL(部分成功)/FAIL(失败)</li>
 *         <li>用于批量处理的状态追踪</li>
 *       </ul>
 *   </li>
 * </ul>
 ...
 *
 * <ul>
 *   <li>枚举类名使用单数名词,如 {@code TaskStatus} 而不是 {@code TaskStatuses}</li>
 *   <li>状态枚举以 {@code Status} 结尾</li>
 *   <li>代码枚举以 {@code Code} 结尾</li>
 *   <li>策略枚举以 {@code Strategy} 结尾</li>
 *   <li>枚举常量使用全大写下划线命名,如 {@code TASK_READY}</li>
 * </ul>
 *
 * <h2>最佳实践</h2>
 *
 * <ul>
 *   <li>枚举优于魔法字符串:使用枚举而不是 {@code String} 常量</li>
 *   <li>封装业务逻辑:将状态判断逻辑放在枚举内部</li>
 *   <li>不可变性:枚举天然不可变,线程安全</li>
 *   <li>单例保证:JVM 保证枚举实例唯一性,可用 {@code ==} 比较</li>
 *   <li>序列化安全:枚举序列化自动处理,避免反序列化攻击</li>
 * </ul>
 *
