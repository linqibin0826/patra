 ...
 *
 * <p>Wraps failures that occur while transitioning outbox messages between {@code PUBLISHED},
 * retry, and dead-letter states. Typical root causes include concurrent updates (optimistic locking
 * updating zero rows), temporary database unavailability, or sequence generation failures.
 *
 * <p>Recovery guidance:
 *
 * <ul>
 *   <li>Concurrency conflicts: retry with exponential backoff keyed by the retry count.
 *   <li>Connection/timeouts: classify as retryable so the scheduler can reattempt.
 *   <li>Sustained failures (beyond thresholds): trigger alerts and investigate database health or
 *       table locks.
 * </ul>
 */
