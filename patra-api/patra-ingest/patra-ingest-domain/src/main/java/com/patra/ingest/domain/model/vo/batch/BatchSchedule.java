 ...
 * 批次执行调度表。
 *
 * <p>封装批次调度构建器生成的批次列表、执行上下文和抓取元数据。
 *
 ...
 *   <li>{@code fetchMetadata}: 抓取元数据（包含总记录数、会话令牌等）
 * </ul>
 *
 * <p>{@code fetchMetadata} 用于后续的参数映射，例如：
 *
 * <ul>
 ...
 *   <li>{@code fetchMetadata} 不能为 {@code null}
 * </ul>
 *
 * @param batches 批次列表
 * @param context 执行上下文
...
public record BatchSchedule(
    List<Batch> batches, ExecutionContext context, FetchMetadata fetchMetadata) {
  ...
  public BatchSchedule {
    ...
    if (fetchMetadata == null) {
      throw new IllegalArgumentException("fetchMetadata must not be null");
    }
  }
   ...
   *
   * @param ctx 执行上下文
   * @param metadata 抓取元数据（totalRecords=0）
   * @return 空调度表
   */
  public static BatchSchedule empty(ExecutionContext ctx, FetchMetadata metadata) {
    return new BatchSchedule(List.of(), ctx, metadata);
  }
   ...
   * @return 调度表
   */
  public static BatchSchedule single(Batch batch, ExecutionContext ctx, FetchMetadata metadata) {
    return new BatchSchedule(List.of(batch), ctx, metadata);
  }
