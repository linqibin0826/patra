...
///
/// 触发条件:在基于所有子 Task 的状态重新计算 Slice 的状态后触发。
///
/// 用途:
///
/// - 聚合:触发基于所有子 Slice 的父 Plan 状态重新计算
///   - 指标:按数据源测量切片完成率、成功率和失败率
///   - 审计:跟踪切片从创建到完成的生命周期
///   - 监控:对高切片失败率或卡住的切片发出告警
///
/// 幂等性:`sliceId` + `newStatus` 作为复合键。处理程序必须检查状态是否已被处理。
///
