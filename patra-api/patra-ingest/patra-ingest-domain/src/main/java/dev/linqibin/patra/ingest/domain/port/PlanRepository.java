...
///
/// **职责**: 持久化、去重和查询采集计划聚合根,保证:
///
/// - 计划创建的一致性 - 通过 planKey 唯一约束防止重复计划
///   - 计划回放能力 - 支持历史计划的精确检索
///   - 聚合根完整性 - 确保 Plan 及其元数据的事务一致性
///
...
public interface PlanRepository {
  ...
  /// 持久化或更新单个计划聚合根。
  ///
  /// **业务含义**: 保存完整的计划聚合根,包括时间窗口、触发器、切片策略等。
  ///
  ...
  /// 根据 `planKey` 查询计划聚合根。
  ///
  /// **业务含义**: 通过业务主键(从数据源、操作、窗口等派生)定位唯一计划。
  ///
