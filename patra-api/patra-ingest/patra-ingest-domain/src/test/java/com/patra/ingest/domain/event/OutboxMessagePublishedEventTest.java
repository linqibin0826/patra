...
class OutboxMessagePublishedEventTest {
    ...
    class RecordConstructionAndAccessorTests {
        ...
        void shouldConstructEventAndExposeAllFields() {
            ...
            // When: 创建事件
            OutboxMessagePublishedEvent event = new OutboxMessagePublishedEvent(
                messageId, channel, partitionKey, occurredAt
            );
    ...
    class RecordEqualityAndHashCodeTests {
        ...
        void twoEventsWithSameValuesShouldBeEqual() {
            ...
            OutboxMessagePublishedEvent event1 = new OutboxMessagePublishedEvent(
                messageId, channel, partitionKey, occurredAt
            );
            OutboxMessagePublishedEvent event2 = new OutboxMessagePublishedEvent(
                messageId, channel, partitionKey, occurredAt
            );
        ...
        void eventShouldBeEqualToItself() {
            // Given: 创建一个事件
            OutboxMessagePublishedEvent event = new OutboxMessagePublishedEvent(
                100L, "test-channel", "test-key", Instant.now()
            );
    ...
    class DomainEventInterfaceTests {
        ...
        void occurredAtMethodShouldReturnCorrectTimestamp() {
            ...
            // When: 创建事件
            OutboxMessagePublishedEvent event = new OutboxMessagePublishedEvent(
                1L, "channel", "key", expectedTimestamp
            );
    ...
    class BusinessScenarioTests {
        ...
        void shouldSupportIdempotencyCheckForBatchMessages() {
            ...
            OutboxMessagePublishedEvent event1 = new OutboxMessagePublishedEvent(
                1L, "channel", "key1", baseTime
            );
            OutboxMessagePublishedEvent event2 = new OutboxMessagePublishedEvent(
                2L, "channel", "key2", baseTime.plusSeconds(1)
            );
            OutboxMessagePublishedEvent event3 = new OutboxMessagePublishedEvent(
                3L, "channel", "key3", baseTime.plusSeconds(2)
            );
        ...
        void shouldSupportMetricsGroupingByChannel() {
            ...
            Instant now = Instant.now();
            OutboxMessagePublishedEvent pubmedEvent = new OutboxMessagePublishedEvent(
                1L, "literature.pubmed", "key1", now
            );
            OutboxMessagePublishedEvent epmcEvent = new OutboxMessagePublishedEvent(
                2L, "literature.epmc", "key2", now
            );
            OutboxMessagePublishedEvent crossrefEvent = new OutboxMessagePublishedEvent(
                3L, "literature.crossref", "key3", now
            );
        ...
        void shouldSupportPartitionDistributionAnalysisByPartitionKey() {
            ...
            String channel = "literature.pubmed";
            OutboxMessagePublishedEvent event1 = new OutboxMessagePublishedEvent(
                1L, channel, "partition-0", now
            );
            OutboxMessagePublishedEvent event2 = new OutboxMessagePublishedEvent(
                2L, channel, "partition-1", now
            );
            OutboxMessagePublishedEvent event3 = new OutboxMessagePublishedEvent(
                3L, channel, "partition-2", now
            );
    ...
    class SerializationTests {
        ...
        void eventShouldBeSerializableAndDeserializable() throws Exception {
            // Given: 创建一个事件
            OutboxMessagePublishedEvent originalEvent = new OutboxMessagePublishedEvent(
                12345L,
                "literature.pubmed",
                "PMID_38572234",
    ...
    class BoundaryConditionTests {
        ...
        void shouldHandleMaxLongValueForMessageId() {
            ...
            // When: 创建事件
            OutboxMessagePublishedEvent event = new OutboxMessagePublishedEvent(
                maxMessageId, "channel", "key", Instant.now()
            );
        ...
        void shouldHandleMinLongValueForMessageId() {
            ...
            // When: 创建事件
            OutboxMessagePublishedEvent event = new OutboxMessagePublishedEvent(
                minMessageId, "channel", "key", Instant.now()
            );
        ...
        void shouldHandleVeryLongChannelString() {
            ...
            // When: 创建事件
            OutboxMessagePublishedEvent event = new OutboxMessagePublishedEvent(
                1L, veryLongChannel, "key", Instant.now()
            );
