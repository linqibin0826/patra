 ...
 *
 * <p>Tests Micrometer instrumentation and cardinality control.
 *
 * @author linqibin
 * @since 0.1.0
 */
...
class OutboxMetricsTest {
  ...
  void shouldRecordPublishSuccess() {
    ...
    Counter counter =
        meterRegistry
            .find("papertrace.outbox.publish.total")
    ...
    Timer timer =
        meterRegistry
            .find("papertrace.outbox.publish.duration")
            .tag("aggregateType", "Task")
            .tag("opType", "batch")
            .timer();
  ...
  void shouldRecordBatchSize() {
    ...
    DistributionSummary summary =
        meterRegistry
            .find("papertrace.outbox.publish.batch.size")
            .tag("aggregateType", "Task")
            .summary();
  ...
  void shouldNotThrowExceptionWhenAggregateTypeNotAllowed() {
    ...
    assertThatCode(
            () -> metrics.recordPublish("UnknownType", "batch", true, Duration.ofMillis(100)))
  ...
  void shouldNotRecordMetricsWhenDisabled() {
    ...
    DistributionSummary summary =
        meterRegistry.find("papertrace.outbox.publish.batch.size").summary();
  ...
  void shouldAllowAnyAggregateTypeWhenListIsEmpty() {
    ...
    assertThatCode(
            () ->
                permissiveMetrics.recordPublish(
                    "RandomType", "batch", true, Duration.ofMillis(100)))
        .doesNotThrowAnyException();
