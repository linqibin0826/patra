 ...
 *
 * <p>Tests factory methods, merge logic, and failure detail records.
 *
 * @author linqibin
 * @since 0.1.0
 */
...
class OutboxPublishResultTest {
  ...
  void shouldCreateFailureResult() {
    ...
    OutboxPublishResult result =
        OutboxPublishResult.failure("Database error", Duration.ofMillis(50));
    ...
    List<OutboxPublishResult.FailureDetail> failures =
        List.of(
            ...
            new OutboxPublishResult.FailureDetail(
                "agg-2", "dedup-2", "Validation error", "VALIDATION_ERROR"));
  ...
  void shouldMergeSuccessAndPartialResults() {
    ...
    List<OutboxPublishResult.FailureDetail> failures =
        List.of(new OutboxPublishResult.FailureDetail("agg-1", "dedup-1", "Error", "BUILD_ERROR"));
  ...
  void shouldMergeMultiplePartialResults() {
    // given
    List<OutboxPublishResult.FailureDetail> failures1 =
        List.of(new OutboxPublishResult.FailureDetail("agg-1", "dedup-1", "Error 1", "JSON_ERROR"));
    OutboxPublishResult result1 = OutboxPublishResult.partial(2, failures1, Duration.ofMillis(100));

    List<OutboxPublishResult.FailureDetail> failures2 =
        List.of(new OutboxPublishResult.FailureDetail("agg-2", "dedup-2", "Error 2", "DB_ERROR"));
    ...
    assertThatThrownBy(
            () ->
                result
                    ...
                    .add(
                        new OutboxPublishResult.FailureDetail(
                            "agg-2", "dedup-2", "Error 2", "JSON_ERROR")))
  ...
  void shouldConstructFailureDetailRecord() {
    // when
    OutboxPublishResult.FailureDetail detail =
        new OutboxPublishResult.FailureDetail(
            "aggregate-123", "dedup-xyz", "JSON parsing failed", "JSON_ERROR");
