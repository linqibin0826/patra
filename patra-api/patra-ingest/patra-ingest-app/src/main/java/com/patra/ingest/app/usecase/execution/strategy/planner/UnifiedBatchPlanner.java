...
public class UnifiedBatchPlanner implements BatchPlanner {
  ...
  public BatchPlan plan(ExecutionContext ctx) {
    try {
      ...
      com.patra.ingest.domain.model.vo.plan.BatchPlan batchPlan =
          dataSourcePort.preparePlan(ctx, ctx.dataType());
      ...
      log.info(
          "批次计划已准备: provenance={}, dataType={}, totalRecords={}, hasStateToken={}",
          ctx.provenanceCode(),
          ctx.dataType(),
      ...
      log.info(
          "批次计划已生成: provenance={}, batchCount={}, strategy={}",
          ctx.provenanceCode(),
          batches.size(),
          strategy.getClass().getSimpleName());
