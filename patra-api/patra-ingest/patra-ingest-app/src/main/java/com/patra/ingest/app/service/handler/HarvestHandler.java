...
//    private final QueryPort queryPort;              // patra-query：标准化/校验/渲染
...
//        var normalized = queryPort.normalize(new QueryPort.NormalizeRequest(
//                prov,
//                cmd.expr().dateType(),
//                cmd.expr() /* 直接塞对象，适配层会序列化 */
//        ));
//
//        // 3) 能力校验（占位）
//        var cap = queryPort.check(new QueryPort.CapabilityCheckRequest(
//                prov, normalized.normalizedExpr(), "HARVEST"
//        ));
//        if (!cap.ok()) throw new IllegalStateException("Query capability not supported: " + cap.message());
