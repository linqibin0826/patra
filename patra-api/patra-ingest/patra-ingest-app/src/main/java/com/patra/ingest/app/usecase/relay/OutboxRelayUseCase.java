 ...
 * <p>Responsibilities: accept {@link OutboxRelayCommand} triggered by schedulers or operators, execute one batch
 * publication run for Outbox messages, and return aggregated metrics.</p>
 * <p>Semantics: the call does not perform unbounded retries; failed or pending messages are recovered through domain
 * state and subsequent scheduler cycles.</p>
 */
public interface OutboxRelayUseCase {
     ...
     * <ul>
     *   <li>Idempotency: identical messages are not published twice (enforced via leases and state transitions).</li>
     *   <li>Observability: key counters are emitted to logs.</li>
     * </ul>
     * </p>
