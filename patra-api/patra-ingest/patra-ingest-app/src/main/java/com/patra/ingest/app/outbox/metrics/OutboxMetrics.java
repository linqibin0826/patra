 ...
 * <p>Provides Micrometer instrumentation for:</p>
 * <ul>
 *   <li>papertrace.outbox.publish.total (Counter): Total publish attempts with status tags</li>
 *   <li>papertrace.outbox.publish.duration (Timer): Publish operation duration</li>
 *   <li>papertrace.outbox.publish.batch.size (DistributionSummary): Batch size distribution</li>
 * </ul>
 *
 * <p>All metrics include tags: aggregateType (controlled by allowedAggregateTypes), opType.</p>
 *
 * @author linqibin
 * @since 0.1.0
 */
...
public class OutboxMetrics {
     ...
     * <ul>
     *   <li>papertrace.outbox.publish.total (Counter): Incremented on each publish attempt</li>
     *   <li>papertrace.outbox.publish.duration (Timer): Records operation duration</li>
     * </ul>
     *
     ...
     */
    public void recordPublish(String aggregateType, String opType, boolean isSuccess, Duration duration) {
        ...
        try {
            ...
            log.warn("Failed to record Outbox metrics for aggregateType={}, opType={}: {}",
                    aggregateType, opType, e.getMessage());
        }
    }

    /**
     * Records batch size distribution.
     * <p>Useful for analyzing batch processing patterns and identifying optimization opportunities.</p>
     *
    ...
    public void recordBatchSize(String aggregateType, int batchSize) {
        ...
        try {
            ...
            log.warn("Failed to record batch size metric for aggregateType={}: {}",
                    aggregateType, e.getMessage());
        }
    }
    ...
    private void validateAggregateType(String aggregateType) {
        ...
        if (!allowed.isEmpty() && !allowed.contains(aggregateType)) {
            throw new IllegalArgumentException(
                    String.format("Unknown aggregateType '%s'. Allowed types: %s. " +
