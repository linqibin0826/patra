...
class IngestErrorMappingContributorTest {
  ...
  void shouldMapConfigurationNotFound() {
    RemoteCallException remote =
        new RemoteCallException(
            "REG-0404", 404, "not found", "ProvenanceClient#getConfiguration", "trace-1", Map.of());
    IngestConfigurationException exception =
        new IngestConfigurationException("PUBMED", "HARVEST", "config not found", remote);

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1201);
  }
  ...
  void shouldMapConfigurationServerError() {
    RemoteCallException remote =
        new RemoteCallException(
            ...
            502,
            "bad gateway",
            "ProvenanceClient#getConfiguration",
            "trace-2",
            Map.of());
    IngestConfigurationException exception =
        new IngestConfigurationException("PUBMED", "HARVEST", "registry unavailable", remote);

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1203);
  }
  ...
  void shouldMapSchedulerArgumentErrors() {
    IngestScheduleParameterException exception =
        new IngestScheduleParameterException("Failed to parse relay param: boom");

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1401);
  }
  ...
  void shouldMapOutboxStateErrors() {
    OutboxPersistenceException exception =
        new OutboxPersistenceException(
            OutboxPersistenceException.Stage.MARK_PUBLISHED,
            "Failed to update outbox status, id=1");

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1302);
  }
  ...
  void shouldMapCheckpointParseError() {
    TaskCheckpointException exception =
        new TaskCheckpointException(
            TaskCheckpointException.Type.PARSE,
            "Checkpoint JSON parsing failed",
            new RuntimeException("boom"));

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1501);
  }
  ...
  void shouldMapRelayExecutionError() {
    OutboxRelayExecutionException exception =
        new OutboxRelayExecutionException("Relay failed", new RuntimeException("boom"));

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1402);
  }
  ...
  void shouldMapPlanValidationError() {
    PlanValidationException exception =
        new PlanValidationException(
            "window invalid", PlanValidationException.Reason.WINDOW_INVALID);

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1403);
  }
  ...
  void shouldMapPlanAssemblyError() {
    PlanAssemblyException exception =
        new PlanAssemblyException("assembly failed", PlanAssemblyException.Reason.EMPTY_RESULT);

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1601);
  }
  ...
  void shouldMapPlanPersistenceError() {
    PlanPersistenceException exception =
        new PlanPersistenceException(PlanPersistenceException.Stage.PLAN, "persist plan failed");

    assertThat(contributor.mapException(exception)).contains(IngestErrorCode.ING_1503);
  }
}
