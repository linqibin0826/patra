...
public final class LayerDependencyRules {
   ...
   * </pre>
   *
   * <p><b>注意：</b>Boot 层（启动配置）不在此检查范围内，因为它位于根包， 可以访问所有层，且通常只有启动类和配置，不需要严格的架构约束。
   */
  public static final ArchRule layer_dependencies_are_respected =
      Architectures.layeredArchitecture()
          ...
          .layer("Domain")
          .definedBy("..ingest.domain..")
          .layer("Application")
          .definedBy("..ingest.app..")
          .layer("Infrastructure")
          .definedBy("..ingest.infra..")
          .layer("Adapter")
          .definedBy("..ingest.adapter..")
   ...
   *
   * <p>App 层应该通过 Domain 层的 Port 接口调用 Infra 层的实现， 运行时由 Spring 注入实现类（依赖倒置原则）。
   *
   * <p><b>错误示例：</b>
   *
   * <pre>
  ...
  public static final ArchRule adapter_should_not_depend_on_boot =
      noClasses()
          .that()
          .resideInAPackage("..ingest.adapter..")
          .should()
          .dependOnClassesThat()
          .resideInAPackage("..ingest.boot..")
   ...
   *   <li>com.patra.ingest.domain.model → com.patra.ingest.domain.port →
