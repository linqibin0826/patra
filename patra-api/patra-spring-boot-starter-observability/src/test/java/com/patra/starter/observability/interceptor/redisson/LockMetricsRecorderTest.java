...
class LockMetricsRecorderTest {
  ...
  class OnLockAcquiredTest {
    ...
    void shouldRecordAcquiredCounter() {
      ...
      Counter counter =
          meterRegistry
              .find("patra.redisson.lock.acquired")
    ...
    void shouldRecordWaitTime() {
      ...
      Timer timer =
          meterRegistry
              .find("patra.redisson.lock.wait_time")
  ...
  class OnLockFailedTest {
    ...
    void shouldRecordFailedCounter() {
      ...
      Counter counter =
          meterRegistry
              .find("patra.redisson.lock.failed")
              .tag("key_pattern", "user")
              .tag("lock_type", "REENTRANT")
              .tag("reason", "timeout")
              .counter();
    ...
    void shouldRecordDifferentReasons(String reason) {
      ...
      Counter counter =
          meterRegistry.find("patra.redisson.lock.failed").tag("reason", reason).counter();
  ...
  class OnLockReleasedTest {
    ...
    void shouldRecordHoldTime() {
      ...
      Timer timer =
          meterRegistry
              .find("patra.redisson.lock.hold_time")
              .tag("key_pattern", "order")
              .tag("lock_type", "WRITE")
              .timer();
  ...
  class ExtractKeyPatternTest {
    ...
    void shouldExtractPattern(String lockKey, String expectedPattern) {
      ...
      assertThat(counter).as("锁键 '%s' 应该提取模式 '%s'", lockKey, expectedPattern).isNotNull();
    }
  ...
  class IsStaticPartTest {
    ...
    void shouldFilterDynamicParts(String lockKey, String expectedPattern) {
      ...
      assertThat(counter).as("锁键 '%s' 应该提取模式 '%s'（过滤动态部分）", lockKey, expectedPattern).isNotNull();
    }
  ...
  class LockTypeTagTest {
    ...
    void shouldRecordDifferentLockTypes(String lockType) {
      ...
      Counter counter =
          meterRegistry.find("patra.redisson.lock.acquired").tag("lock_type", lockType).counter();
