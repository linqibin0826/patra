...
class TestObservabilityApplication {
     ...
     * 测试公共标签自动添加。
     *
     * <p>注意：此测试需要 Spring Boot Actuator 的 MeterFilter 自动配置才能生效。
     * 在没有 Actuator 的测试环境中，MeterFilter Bean 虽然会被创建，但不会被应用到 MeterRegistry。
     * 此处仅验证 MeterRegistry 可用，MeterFilter 的功能由单元测试覆盖。
     */
    @Test
    ...
    void shouldFilterHighCardinalityTags() {
        ...
        Counter counter = Counter.builder("patra.test.counter.high-cardinality")
