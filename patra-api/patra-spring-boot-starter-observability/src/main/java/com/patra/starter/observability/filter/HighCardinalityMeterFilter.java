...
public class HighCardinalityMeterFilter implements MeterFilter {
  ...
  /// 默认高基数标签黑名单。
  private static final Set<String> DEFAULT_HIGH_CARDINALITY_KEYS =
      Set.of(
          "userId",
          "user_id",
          "requestId",
  ...
  public HighCardinalityMeterFilter(Set<String> customHighCardinalityKeys) {
    ...
    log.info(
        "初始化高基数标签过滤器，黑名单标签数量: {}, 黑名单: {}",
        this.highCardinalityKeys.size(),
        this.highCardinalityKeys);
  }
  ...
  public Meter.Id map(Meter.Id id) {
    ...
    if (!removedKeys.isEmpty()) {
      ...
      return new Meter.Id(
          id.getName(), Tags.of(filteredTags), id.getBaseUnit(), id.getDescription(), id.getType());
    }
