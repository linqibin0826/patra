---
name: "source-command-post-code"
description: "代码变更后的完整质量检查（架构审查、代码审查、文档检查、测试编译）"
---

# source-command-post-code

Use this skill when the user asks to run the migrated source command `post-code`.

## Command Template

## 参数说明

**用户传入的参数**：从用户请求中读取，例如 `committed`、`uncommitted`、`已提交未推送`、`未提交` 或模块名。

**参数解析规则**（按优先级判断）：

1. **无参数** → 检查所有本地变更（未提交 + 已提交未推送）
...
执行前先运行以下命令获取审查上下文，不要把下面的命令当作自动插值语法：
...
根据用户请求中的参数确定审查范围，然后对该范围内的代码变更执行全面的质量检查。

### 范围确定逻辑

**请先解析用户请求中的参数，确定要审查的范围：**
...
**首先解析用户请求中的参数，确定本次审查的范围**

```
用户参数: 从用户请求中提取；若未提供则视为空。
```
...
**编译和架构测试都通过后，在单个响应中并行启动以下三个 Codex subagents（不要重写它们的规则和输出格式，只说明审查范围）：**
...
- **参数优先**：必须先解析用户请求中的参数确定审查范围（第零阶段），再执行后续操作
...
- **文档处理**：`doc-checker` 只检查并报告文档缺失，不直接修改文档。
...
# 只检查已提交但未推送的变更
$source-command-post-code committed
$source-command-post-code 已提交未推送

# 只检查工作区未提交的变更
$source-command-post-code uncommitted
$source-command-post-code 未提交

# 只检查特定模块的变更
$source-command-post-code patra-spring-boot-starter-web
$source-command-post-code patra-ingest
```

## Codex 调用方式

Claude slash command `post-code` 已迁移为 Codex skill。需要运行这套检查时，按 `$source-command-post-code` 调用，并在请求中直接写清参数，例如 `committed`、`uncommitted` 或模块名。原 slash command 的参数占位和 shell 输出插值已改写为自然语言步骤。
