...
class GlobalRestExceptionHandlerTest {
  ...
  void handleException_shouldReturnAdaptedProblemDetail() throws Exception {
    ...
    GlobalRestExceptionHandler handler =
        new GlobalRestExceptionHandler(new StubAdapter(response), errors -> List.of());

    ResponseEntityAssertions result =
        ResponseEntityAssertions.from(
  ...
  void handleMethodArgumentNotValid_shouldAttachErrorsAndLimitSize() throws Exception {
    BeanPropertyBindingResult bindingResult =
        new BeanPropertyBindingResult(new DemoPayload(), "payload");
    for (int i = 0; i < 3; i++) {
      bindingResult.addError(
          new FieldError("payload", "field" + i, "bad" + i, false, null, null, "msg" + i));
    }
    ...
    GlobalRestExceptionHandler handler =
        new GlobalRestExceptionHandler(new StubAdapter(response), new FixedFormatter(formatted));
    ...
    List<ValidationError> finalErrors =
        (List<ValidationError>) result.problemDetail().getProperties().get(ErrorKeys.ERRORS);
    assertThat(finalErrors).hasSize(100);
  }

  private MethodParameter methodParameter() throws NoSuchMethodException {
    Method method =
        GlobalRestExceptionHandlerTest.class.getDeclaredMethod("handle", DemoPayload.class);
  ...
  private static final class FixedFormatter implements ValidationErrorsFormatter {
    ...
    @Override
    public List<ValidationError> formatWithMasking(
  ...
  private record ResponseEntityAssertions(
      HttpStatus status, MediaType contentType, ProblemDetail problemDetail) {
    static ResponseEntityAssertions from(org.springframework.http.ResponseEntity<?> entity) {
      ...
      HttpStatus status =
          entity.getStatusCode() instanceof HttpStatus httpStatus
              ? httpStatus
