...
| 模块 | 表数量 | 核心功能 | 数据规模 |
|------|--------|---------|---------|
| 核心实体 | 6张 | 文献、载体、作者、摘要、标识符管理 | 200万+ 文献 |
| 分类索引 | 12张 | MeSH标引、关键词、类型、物质分类 | 2000万+ 标引记录 |
| 人员机构 | 6张 | 作者、机构、研究者关联管理 | 1200万+ 关联记录 |
| 关联信息 | 7张 | 资助、引用、相关项、补充材料、历史 | 4500万+ 关联记录 |
| 辅助管理 | 5张 | 日期、元数据、翻译、语言映射、OA | 600万+ 辅助记录 |
...
| 表名 | 中文名 | 核心字段 | 预估规模 | 说明 |
|------|--------|---------|---------|------|
| `cat_publication` | 出版物主表 | pmid, doi, venue_id, title, publication_year | 200万+ | 冗余PMID/DOI/venue_id提升查询性能 |
| `cat_venue` | 出版载体表 | venue_type, title, issn, isbn, publisher | 3万+ | 统一管理期刊/书籍/会议 |
| `cat_venue_instance` | 载体实例表 | venue_id, volume, issue, publication_year | 50万+ | 具体卷期信息 |
| `cat_identifier` | 标识符表 | publication_id, type, value | 800万+ | 存储所有类型标识符 |
| `cat_author` | 作者表 | last_name, fore_name, orcid, dedup_key | 500万+ | 复合去重策略 |
| `cat_abstract` | 摘要表 | publication_id, plain_text, structured_sections | 200万+ | 独立存储优化性能 |
...
| 表名 | 中文名 | 核心字段 | 预估规模 | 说明 |
|------|--------|---------|---------|------|
| `cat_mesh_descriptor` | MeSH主题词表 | ui, name, descriptor_class | 3万+ | NLM 标准词表 |
| `cat_mesh_qualifier` | MeSH限定词表 | ui, name, abbreviation | 100+ | 主题词的限定修饰 |
| `cat_mesh_tree_number` | MeSH树形编号表 | descriptor_id, tree_number, tree_level | 7万+ | 支持层次查询 |
| `cat_mesh_entry_term` | MeSH入口术语表 | descriptor_id, term, lexical_tag | 30万+ | 同义词和入口术语 |
| `cat_mesh_concept` | MeSH概念表 | descriptor_id, concept_ui, concept_name | 9万+ | 概念层级结构 |
| `cat_publication_mesh` | 文献-MeSH关联表 | publication_id, descriptor_id, is_major_topic | 2000万+ | 多对多关联 |

#### 其他分类表（6张）
| 表名 | 中文名 | 核心字段 | 预估规模 | 说明 |
|------|--------|---------|---------|------|
| `cat_keyword` | 关键词表 | term, source, normalized_term | 100万+ | 多来源关键词 |
| `cat_publication_keyword` | 文献-关键词关联表 | publication_id, keyword_id, is_major | 500万+ | 多对多关联 |
| `cat_publication_type` | 出版类型表 | type_code, type_name, parent_type | 100+ | 支持类型层次 |
| `cat_publication_type_mapping` | 文献-类型关联表 | publication_id, type_id | 300万+ | 多类型标注 |
| `cat_substance` | 物质表 | registry_number, name, substance_class | 5万+ | 化学物质/药物 |
| `cat_publication_substance` | 文献-物质关联表 | publication_id, substance_id, is_major | 300万+ | 多对多关联 |

### 3.3 人员与机构表（6张）

| 表名 | 中文名 | 核心字段 | 预估规模 | 说明 |
|------|--------|---------|---------|------|
| `cat_publication_author` | 文献-作者关联表 | publication_id, author_id, author_order | 1000万+ | 保留作者顺序 |
| `cat_affiliation` | 机构表 | name, country, ror_id, dedup_key | 10万+ | 支持多种标识符 |
| `cat_author_affiliation` | 作者-机构关联表 | author_id, affiliation_id, is_primary | 1200万+ | 支持时间维度 |
| `cat_investigator` | 研究者表 | last_name, fore_name, investigator_type | 20万+ | 非作者研究人员 |
| `cat_publication_investigator` | 文献-研究者关联表 | publication_id, investigator_id, role | 30万+ | 多对多关联 |
| `cat_personal_name_subject` | 人物主题表 | publication_id, last_name, subject_type | 1万+ | 传记等主题人物 |

### 3.4 关联信息表（7张）

| 表名 | 中文名 | 核心字段 | 预估规模 | 说明 |
|------|--------|---------|---------|------|
| `cat_funding` | 资助信息表 | agency_name, grant_id, funder_id | 100万+ | 支持 Crossref Funder ID |
| `cat_publication_funding` | 文献-资助关联表 | publication_id, funding_id, is_primary | 150万+ | 多对多关联 |
| `cat_reference` | 参考文献表 | publication_id, cited_pmid, cited_doi | 4000万+ | 支持库内外引用 |
| `cat_external_reference` | 外部引用表 | publication_id, database_name, accession_number | 50万+ | 基因库/临床试验等 |
| `cat_related_item` | 相关项目表 | publication_id, relationship_type, status | 10万+ | 撤稿/勘误/评论 |
| `cat_supplemental_object` | 补充对象表 | publication_id, object_type, url | 100万+ | 图表/数据集等 |
| `cat_publication_history` | 发布历史表 | publication_id, event_type, event_date | 600万+ | 时间线事件记录 |

### 3.5 辅助管理表（5张）

| 表名 | 中文名 | 核心字段 | 预估规模 | 说明 |
|------|--------|---------|---------|------|
| `cat_publication_date` | 日期信息表 | publication_id, date_type, year, month, day | 400万+ | 分离字段精确表达 |
| `cat_publication_metadata` | 元数据表 | publication_id, indexing_status, data_source | 200万+ | 索引方法/状态等 |
| `cat_alternative_abstract` | 其他语言摘要表 | publication_id, language_code, is_official | 20万+ | 多语言版本 |
| `cat_language_mapping` | 语言映射表 | raw_value, standard_code, confidence_score | 1000+ | 动态学习映射 |
| `cat_oa_location` | 开放获取位置表 | publication_id, oa_status, url, is_best | 300万+ | OA多位置管理 |
...
|------|--------|---------|---------|--------|
| 核心实体 | 6张 | 350万+ | 5GB | 10%/年 |
| 分类索引 | 12张 | 2600万+ | 15GB | 5%/年 |
| 人员机构 | 6张 | 1230万+ | 8GB | 8%/年 |
| 关联信息 | 7张 | 4860万+ | 25GB | 12%/年 |
| 辅助管理 | 5张 | 620万+ | 3GB | 7%/年 |
| **总计** | **36张** | **9660万+** | **56GB** | **9%/年** |

#### TOP 5 大表

| 排名 | 表名 | 预估记录数 | 说明 |
|------|------|-----------|------|
| 1 | cat_reference | 4000万+ | 参考文献（每篇平均20条） |
| 2 | cat_publication_mesh | 2000万+ | MeSH标引（每篇平均10条） |
| 3 | cat_author_affiliation | 1200万+ | 作者机构关联 |
| 4 | cat_publication_author | 1000万+ | 文献作者关联 |
| 5 | cat_identifier | 800万+ | 标识符（每篇平均4个） |
