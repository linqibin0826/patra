...
lines_added: 23887
lines_deleted: 6122
modules: [patra-catalog, patra-ingest, patra-registry, patra-spring-boot-starter-batch, patra-spring-boot-starter-provenance, patra-spring-boot-starter-rest-client, patra-spring-boot-starter-test, patra-spring-cloud-starter-feign]
tags: [feat, refactor, fix, docs, test, mesh, parser, xml, batch, integration-test]
linked_bugs: [BUG-001-restclient-loadbalancer-interceptor]
linked_tils: []
linked_adrs: [ADR-001-batch-native-observability, ADR-002-infra-integration-test-strategy, ADR-003-xml-parser-strategy-pattern, ADR-004-mesh-ui-as-relation-key]
---
...
1. **完成 MeshDescriptorRepository 剩余方法实现**
   - `findByUi`、`findByName`、`findByTreeNumber`、`exists` 4 个 TODO 待实现
   - 位置：`MeshDescriptorRepositoryAdapter.java:43-64`

2. **完成 PublicationAggregate OA 状态变更事件**
   - 发布 OA 状态变更事件的 TODO
