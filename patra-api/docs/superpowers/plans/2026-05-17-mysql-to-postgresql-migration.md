...
### Task M1.1：libs.versions.toml 切换 Testcontainers Module
...
### Task M1.2：starter-jpa build.gradle.kts 切换驱动与 Flyway PG 模块
...
### Task M1.3：starter-test build.gradle.kts 切换
...
### Task M1.4：starter-batch build.gradle.kts 切换 compileOnly 驱动
...
### Task M1.5：重写 docker-compose.core.yaml mysql 服务为 postgres
...
### Task M1.6：重写 .env.dev 为 POSTGRES_*
...
### Task M1.7：重写 init 脚本 → postgres/init-scripts/02-create-databases.sql
...
### Task M1.8：M1 smoke test — 起 postgres 容器验证 5 库就位
...
### Task M2.1：ContainerType 枚举 MYSQL → POSTGRESQL
...
### Task M2.2：重写 MySQLContainerInitializer → PostgreSQLContainerInitializer
...
### Task M2.3：catalog/ingest 子类重命名
...
### Task M3.1：JpaErrorMappingContributor 删除 MySQL 错误码 switch
...
### Task M3.2：从 Spring Batch 5 jar 抽取 schema-postgresql.sql
...
### Task M3.3：BatchSchemaInitializer 切换常量 + 删 schema-mysql.sql
...
### Task M3.4：batch starter test application-test.yml 切换 JDBC URL
...
### Task M3.5：DualDataSourceIT.java 内嵌 URL 切换
...
### Task M3.6：BatchDataSourceConfigurationTest MySQLContainer → PostgreSQLContainer
...
### Task M3.7：BatchPropertiesTest hardcoded URL 切换
...
### Task M3.8：HibernatePropertiesCustomizer 追加 metadata 探测优化
...
### Task M3.9：M3 smoke test — starter-jpa + starter-batch 测试全绿
...
### Task M4.1：创建 registry V1__init_registry_schema.sql（合并 3 DDL）
...
### Task M4.2：创建 registry V2__seed_provenance_expr.sql（重写自 V1.1.0）
...
### Task M4.3：创建 registry V3__seed_dict_country.sql（合并 V1.2.0 + V1.4.0 country + V1.4.1）
...
### Task M4.4：创建 registry V4__seed_dict_language.sql（合并 V1.3.0 + V1.4.0 language + V1.4.2）
...
### Task M4.5：删除 registry 原 9 个 V1.x.y 文件
...
### Task M4.7：创建 ingest V1__init_ingest_scheduling.sql（拆自 V0.1.0 第 1-3 段）
...
### Task M4.8：创建 ingest V2__init_ingest_execution.sql（含 partial index）
...
### Task M4.9：创建 ingest V3__init_ingest_cursor.sql
...
### Task M4.10：创建 ingest V4__init_ingest_outbox.sql（含 2 个 partial index）
...
### Task M4.13：创建 catalog V1__create_venue.sql（venue 7 + rating 4 = 11 表）
...
### Task M4.14：创建 catalog V2__create_publication.sql（13 表 — 含 value→identifier_value 重命名 + FULLTEXT 删除 + 生成列）
...
### Task M4.15：创建 catalog V3__create_author.sql（5 表 + 2 个函数索引）
...
### Task M4.16：创建 catalog V4__create_organization.sql（8 表，含 value → name_value 重命名）
...
### Task M4.17：创建 catalog V5__create_mesh.sql（15 表，删 3 FULLTEXT）
...
### Task M4.18：创建 catalog V6__create_keyword.sql（2 表，删 FULLTEXT + 1 冗余索引）
...
### Task M4.19：创建 catalog V7__create_funding.sql（1 表）
...
### Task M4.20：删除 catalog 旧 15 个 V*.sql
...
### Task M4.22：创建 object-storage V1__init_storage_schema.sql
...
### Task M4.23：删除 object-storage 旧 V0.1.0 + M4 验证
...
### Task M5.1：PublicationIdentifierEntity.java value → identifier_value
...
### Task M5.2：OrganizationNameEntity.java value → name_value（含 @UniqueConstraint + @Index）
...
### Task M5.3：VenueDao 改 JSON 函数为 PG 操作符
...
### Task M5.4：VenueDao + PublicationDao LIKE → ILIKE
...
### Task M5.5：PublicationDao ORDER BY 修正
...
### Task M5.9：MeshScr/MeshDescriptor IT 测试断言更新
...
### Task M6.1：4 个 application.yml driver-class-name 切换
...
### Task M6.2：4 个 application-dev.yml JDBC URL 切换（含 catalog 独立 batch DS）
...
### Task M6.3：删除 Flyway baseline-on-migrate 与 baseline-version
...
### Task M6.4：M6 验证 — 4 boot bootRun
...
### Task M7.3：Starter README（jpa/test/batch）
...
### Task M7.4：patra-infra/docker/README.md（14 处）
...
### Task M7.5：catalog docs（pubmed-parsing-gap-analysis.md）
...
### Task FV.3：JSONB native query 抽样验证（spec §10.6）
...
### Task FV.5：HikariCP pool size 现场观察（spec §10.7）
...
*Plan covers 全部 spec §6 M1-M7 + spec §7 验证计划 + spec §10 现场验证项。共 7 milestones / 60+ tasks，每个 task 自包含且有明确 commit boundary。*
