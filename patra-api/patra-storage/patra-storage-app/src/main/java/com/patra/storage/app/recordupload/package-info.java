 ...
 *
 * <p>本包包含"记录文件上传元数据"用例的编排逻辑,作为六边形架构应用层的一部分。
 * 应用层负责协调领域对象完成业务用例,管理事务边界,并作为外部适配器和领域层之间的桥梁。
 *
 * <h2>核心组件</h2>
 *
 * <ul>
 *   <li>{@link com.patra.storage.app.recordupload.RecordUploadOrchestrator} - 用例编排器,协调领域对象执行业务逻辑
 *   <li>{@link com.patra.storage.app.recordupload.RecordUploadCommand} - 用例输入命令,封装编排器所需的所有参数
 *   <li>{@link com.patra.storage.app.recordupload.RecordUploadResult} - 用例输出结果,包含元数据 ID 和记录时间戳
 * </ul>
 *
 * <h2>应用层职责</h2>
 *
 ...
 *
 * <ol>
 *   <li>接收并验证 {@link com.patra.storage.app.recordupload.RecordUploadCommand} 命令
 *   <li>将命令数据转换为领域对象(StorageKey、FileSize、FileChecksum、BusinessContext)
 *   <li>调用聚合根工厂方法创建 {@link com.patra.storage.domain.model.aggregate.FileMetadata}
 *   <li>配置可选属性(contentType、expiresAt、recordRemarks、ipAddress)
 *   <li>通过仓储端口持久化聚合根
 *   <li>记录审计日志
 *   <li>返回 {@link com.patra.storage.app.recordupload.RecordUploadResult} 结果
 * </ol>
 *
 * <h2>使用示例</h2>
 *
 ...
 *
 * <ul>
 *   <li>领域层: {@link com.patra.storage.domain.model.aggregate.FileMetadata}
 *   <li>仓储端口: {@link com.patra.storage.domain.port.FileMetadataRepository}
 *   <li>适配器层: {@code patra-storage-adapter/adapter/rest/internal/StorageEndpointImpl}
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
