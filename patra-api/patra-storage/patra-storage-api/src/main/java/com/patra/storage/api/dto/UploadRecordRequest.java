 ...
 * 上传记录请求。
 *
 * <p>调用方用于持久化上传元数据的请求体。包含文件的存储位置、大小、校验和、 业务上下文等所有必需信息,用于在存储服务中创建元数据记录。
 *
 * <p>验证约束:
 *
 * <ul>
 *   <li>bucketName、objectKey、md5Hash、serviceName、businessType、businessId、providerType 不能为空
 *   <li>fileSize 必须 >= 0
 *   <li>contentType 最大128字符
 *   <li>recordRemarks 最大512字符
 * </ul>
 *
