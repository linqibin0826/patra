...
**patra-storage-adapter** 是 patra-storage 服务的适配器层,负责处理外部请求并将其转换为应用层命令。作为六边形架构的**入站适配器**(Inbound Adapter),本模块实现 REST 端点,接收来自其他微服务的 Feign 调用,并将 HTTP 请求适配为应用层用例。
...
- **patra-storage-boot**: 启动模块依赖本模块提供 REST 端点
...
| patra-storage-app | 0.1.0-SNAPSHOT | 应用层依赖 |
| patra-storage-api | 0.1.0-SNAPSHOT | API 契约层依赖 |
...
- **API 契约**: 参见 `patra-storage-api/README.md` 了解端点定义和 DTO
- **应用层**: 参见 `patra-storage-app/README.md` 了解用例编排器
- **启动模块**: 参见 `patra-storage-boot/README.md` 了解服务配置
