package dev.linqibin.patra.objectstorage.domain.model.vo;

import java.util.Objects;

/// 不可变的存储定位符,包含存储桶和对象键。
///
/// 存储键是文件在对象存储中的唯一标识,由存储桶名称和对象键路径组成。这个值对象封装了存储定位逻辑, 提供了验证、组合和比较功能,确保存储键的完整性和一致性。
///
...
public record StorageKey(String bucket, String objectKey) {
  ...
  ///
  /// 将存储桶和对象键组合成统一的字符串表示形式,格式为 "bucket/objectKey", 用作数据库唯一约束字段,确保同一文件不会重复记录。
  ///
