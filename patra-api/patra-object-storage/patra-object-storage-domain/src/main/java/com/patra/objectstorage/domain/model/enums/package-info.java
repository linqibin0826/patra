 ...
 * Storage 领域模型枚举类型包。
 *
 * <p>本包包含 patra-object-storage 服务的枚举类型,作为六边形架构领域层的一部分。 枚举类型表示有限的固定值集合,提供类型安全、自文档化和编译期检查,是领域模型的重要组成部分。
 *
 * <h2>核心枚举</h2>
 *
 * <ul>
 *   <li>{@link com.patra.objectstorage.domain.model.enums.FileStatus} - 文件状态枚举,表示文件的生命周期状态
 *   <li>{@link com.patra.objectstorage.domain.model.enums.StorageProvider} - 存储提供商枚举,表示对象存储类型
 * </ul>
 *
 * <h2>枚举特性</h2>
 *
