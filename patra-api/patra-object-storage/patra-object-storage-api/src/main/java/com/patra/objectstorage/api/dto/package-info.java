 ...
 * Storage 服务数据传输对象包。
 *
 * <p>本包定义了对象存储元数据服务(patra-object-storage)的内部 API 契约。所有 DTO 类都是六边形架构 API 层的一部分,
 * 作为微服务间通信的数据传输对象,确保内部服务集成的类型安全和契约稳定。
 *
 * <h2>核心 DTO</h2>
 *
 * <ul>
 *   <li>{@link com.patra.objectstorage.api.dto.UploadRecordRequest} - 上传记录请求 DTO,包含文件元数据、业务上下文、存储位置等信息
 *   <li>{@link com.patra.objectstorage.api.dto.RecordUploadResponse} - 上传记录响应 DTO,返回元数据 ID 和记录时间戳
 * </ul>
 *
 * <h2>设计原则</h2>
 *
 ...
 *
 * <ul>
 *   <li><strong>内部 Feign 调用</strong>: patra-ingest、patra-registry 等服务通过 Feign 客户端调用 patra-object-storage
 ...
 *
 * <ul>
 *   <li>API 契约: {@link com.patra.objectstorage.api.endpoint.StorageEndpoint}
 *   <li>Feign 客户端: {@link com.patra.objectstorage.api.client.StorageClient}
 *   <li>模块文档: {@code patra-object-storage/README.md}
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
