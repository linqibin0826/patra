# patra-object-storage-app — 应用层

## 概述

**patra-object-storage-app** 是 patra-object-storage 服务的应用层,负责用例编排和事务管理。本模块作为领域层和外部世界的桥梁,将来自适配器层的请求转换为领域操作,并协调多个领域对象完成业务用例。
...
- **patra-object-storage-domain**: 领域层(聚合根、值对象、仓储端口)
...
- **patra-object-storage-adapter**: 适配器层调用应用层编排器
...
| patra-object-storage-domain | 0.1.0-SNAPSHOT | 领域层依赖 |
| Lombok | 1.18.38+ | 代码生成 |
...
- **领域层**: 参见 `patra-object-storage-domain/README.md` 了解聚合根和仓储端口
- **适配器层**: 参见 `patra-object-storage-adapter/README.md` 了解如何调用编排器
- **基础设施层**: 参见 `patra-object-storage-infra/README.md` 了解仓储实现
