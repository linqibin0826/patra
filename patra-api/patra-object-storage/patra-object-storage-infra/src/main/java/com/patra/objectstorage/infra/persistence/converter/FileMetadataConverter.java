...
/// 文件元数据转换器。
/// 
/// MapStruct映射器,负责在领域聚合根({@link FileMetadata})和数据对象({@link FileMetadataDO})之间进行转换。
/// 这个转换器是基础设施层的反腐败层,隔离领域模型与持久化模型的差异。
/// 
/// 转换规则:
/// 
/// - 领域值对象(如StorageKey、FileSize)解包为DO的基本类型字段
///   - 领域枚举转换为DO的字符串字段
///   - Map类型的关联数据序列化为JsonNode以适配MyBatis的JSON类型处理器
/// 
...
public interface FileMetadataConverter {
  ...
  /// 将领域聚合根映射到其持久化表示。
/// 
/// 将领域模型转换为数据对象,用于插入或更新数据库记录。 MapStruct自动生成实现,通过表达式解包值对象和枚举。
/// 
...
/// @return 持久化数据对象
  ...
  /// 将数据对象映射回领域聚合根。
/// 
/// 从数据库加载的数据对象重建领域模型,恢复值对象和枚举类型。 手动实现以精确控制值对象的构造和验证逻辑。
/// 
...
/// 
/// 用于将领域模型中的Map类型关联数据转换为JsonNode, 适配MyBatis-Plus的JacksonTypeHandler进行JSON字段的持久化。
/// 
/// @param source 源Map对象
/// @return JSON节点表示
  ...
  /// 将JSON节点转换回Map形式。
/// 
/// 从数据库加载的JsonNode反序列化为Map,用于重建领域对象的关联数据。
/// 
/// @param node JSON节点
/// @return Map表示
