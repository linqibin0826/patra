 ...
 * Storage 基础设施层数据对象包。
 *
 * <p>本包包含 patra-object-storage 服务的数据对象(Data Object, DO),作为六边形架构基础设施层的一部分。 数据对象是数据库表的 Java 映射,与
 * MyBatis-Plus 框架配合实现 ORM 功能。
 *
 * <h2>核心组件</h2>
 *
 * <ul>
 *   <li>{@link com.patra.objectstorage.infra.persistence.entity.FileMetadataDO} - 文件元数据数据对象,映射
 ...
 *
 * <ul>
 *   <li><strong>数据库映射</strong>: 通过注解映射到数据库表和字段
 *   <li><strong>ORM 支持</strong>: 与 MyBatis-Plus 配合实现 CRUD 操作
 *   <li><strong>持久化载体</strong>: 作为仓储层与数据库交互的数据载体
 *   <li><strong>审计字段</strong>: 包含创建人、更新人、时间戳、版本号等审计信息
 *   <li><strong>软删除支持</strong>: 通过 deleted 字段实现逻辑删除
 * </ul>
 *
 * <h2>设计原则</h2>
 *
 * <ul>
 *   <li><strong>技术实现</strong>: 数据对象是技术实现细节,不包含业务逻辑
 *   <li><strong>可变对象</strong>: 使用可变字段和 setter 方法,便于 ORM 框架操作
 *   <li><strong>框架注解</strong>: 使用 MyBatis-Plus 注解(@TableName、@TableId、@TableField)
 *   <li><strong>字段完整</strong>: 包含数据库表的所有字段,确保数据完整性
 *   <li><strong>命名规范</strong>: 类名以 DO 结尾,字段名使用驼峰命名,通过注解映射数据库下划线命名
 * </ul>
 *
 * <h2>FileMetadataDO - 文件元数据数据对象</h2>
 *
 * 映射数据库表 {@code storage_file_metadata},包含以下字段分组:
 *
 * <ul>
 *   <li><strong>主键标识</strong>:
 *       <ul>
 *         <li>{@code id} - 主键,BIGINT UNSIGNED AUTO_INCREMENT
 *       </ul>
 *   <li><strong>存储定位</strong>:
 *       <ul>
 *         <li>{@code storageKey} - 完整存储键(bucket/objectKey),VARCHAR(768),唯一索引
 *         <li>{@code bucketName} - 存储桶名称,VARCHAR(128)
 *         <li>{@code objectKey} - 对象键,VARCHAR(512)
 *       </ul>
 *   <li><strong>文件属性</strong>:
 *       <ul>
 *         <li>{@code fileSize} - 文件大小(字节),BIGINT
 *         <li>{@code contentType} - MIME 类型,VARCHAR(128)
 *         <li>{@code md5Hash} - MD5 校验和,VARCHAR(64)
 *         <li>{@code sha256Hash} - SHA-256 校验和,VARCHAR(128)
 *       </ul>
 *   <li><strong>业务上下文</strong>:
 *       <ul>
 *         <li>{@code serviceName} - 调用服务名称,VARCHAR(64)
 *         <li>{@code businessType} - 业务分类,VARCHAR(64)
 *         <li>{@code businessId} - 业务标识,VARCHAR(128)
 *         <li>{@code correlationData} - 关联元数据,JSON
 *       </ul>
 *   <li><strong>生命周期</strong>:
 *       <ul>
 *         <li>{@code providerType} - 存储提供商类型,VARCHAR(32),枚举值
 *         <li>{@code status} - 文件状态,VARCHAR(32),枚举值
 *         <li>{@code uploadedAt} - 上传完成时间,TIMESTAMP(6)
 *         <li>{@code expiresAt} - 过期时间,TIMESTAMP(6)
 *         <li>{@code deletedAt} - 软删除时间,TIMESTAMP(6)
 *       </ul>
 *   <li><strong>审计信息</strong>:
 *       <ul>
 *         <li>{@code recordRemarks} - 审计备注,JSON
 *         <li>{@code version} - 乐观锁版本号,BIGINT UNSIGNED
 *         <li>{@code ipAddress} - 请求者 IP,VARBINARY(16)
 *         <li>{@code createdAt} - 创建时间,TIMESTAMP(6)
 *         <li>{@code createdBy} - 创建人 ID,BIGINT UNSIGNED
 *         <li>{@code createdByName} - 创建人姓名,VARCHAR(100)
 *         <li>{@code updatedAt} - 更新时间,TIMESTAMP(6)
 *         <li>{@code updatedBy} - 更新人 ID,BIGINT UNSIGNED
 *         <li>{@code updatedByName} - 更新人姓名,VARCHAR(100)
 *         <li>{@code deleted} - 软删除标志,TINYINT(1)
 *       </ul>
 * </ul>
 *
