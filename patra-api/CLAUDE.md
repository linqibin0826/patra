...
- 本文件位于仓库 **根目录**，适用于整个仓库。
- 允许在子目录放置更细粒度的 `CLAUDE.md`（如 `patra-ingest/CLAUDE.md`）。**“就近优先”**：离被修改文件最近的 CLAUDE.md
  生效；与上层内容**自上而下合并**。
...
## 13. 测试
    ...
    • 使用 subagent：qa-unit-tests → qa-integration-tests → qa-quality-gates（必要时回流 code-reviewer 指出测试缺口）。

## 14. 变更边界（TL;DR）

	• 使用 subagent：architecture-reviewer → code-reviewer；跨任务用 agent-organizer 编排 Gates/DoD。

## 15. 文档维护与同步

- 使用 subagent：docs-engineer → mermaid-expert → docs-engineer；需要权威引用时由 search-specialist 提供来源。
