...
public class LockExecutor {
  ...
  public LockExecutor(RedissonClient redissonClient, LockObserver lockObserver) {
  ...
  private boolean tryLock(RLock lock, LockContext context) throws InterruptedException {
    if (context.isWatchdogEnabled()) {
      ...
      return lock.tryLock(context.getWaitTime(), context.getLeaseTime(), TimeUnit.MILLISECONDS);
    }
  }
