...
-- =====================================================================
CREATE TABLE IF NOT EXISTS `reg_literature_provenance`
(
...
CREATE TABLE IF NOT EXISTS `reg_literature_provenance_config`
(
    `id`                        BIGINT UNSIGNED NOT NULL COMMENT '主键(雪花/发号器)',
    `literature_provenance_id`            BIGINT UNSIGNED NOT NULL COMMENT '逻辑外键→reg_literature_provenance.id(不建物理FK)',
    ...
    `base_url`                   VARCHAR(255)    NOT NULL COMMENT 'API 基址',
    ...
    PRIMARY KEY (`id`),
    KEY `idx_src` (`literature_provenance_id`),
    KEY `idx_updated` (`updated_at`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_0900_ai_ci
  ROW_FORMAT = DYNAMIC
    COMMENT ='(Registry 域)数据源动态配置(调度/限流/参数映射)— 与 reg_literature_provenance 1:1 对应';
...
(
    ...
    PRIMARY KEY (`id`),
    CONSTRAINT `uk_field_dict` UNIQUE (`field_key`)
)
    ENGINE = InnoDB
    ROW_FORMAT = DYNAMIC
...
(
    ...
    PRIMARY KEY (`id`),
    CONSTRAINT `uk_parammap` UNIQUE (`literature_provenance_id`, `operation`),
    KEY `idx_parammap_src` (`literature_provenance_id`),
    KEY `idx_parammap_lookup` (`literature_provenance_id`, `operation`, `std_key`),
...
(
    ...
    PRIMARY KEY (`id`),
    CONSTRAINT `uk_capability_field` UNIQUE (`literature_provenance_id`),
...
(
    ...
    `deleted`         tinyint(1)              default 0                    not null comment '软删除：0=未删，1=已删',
    PRIMARY KEY (`id`),
    KEY `idx_rr_src` (`literature_provenance_id`),
    KEY `idx_rr_lookup` (`literature_provenance_id`, `field_key`, `op`, `match_type`, `negated`, `value_type`,
                         `priority`),
