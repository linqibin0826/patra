...
/// 
/// 本包是六边形架构基础设施层的根包,包含仓储实现、数据库实体、MyBatis-Plus
/// Mapper、实体转换器等基础设施组件。基础设施层实现领域层定义的端口接口,将领域对象持久化到数据库或与外部系统集成。
/// 
/// ## 架构定位
/// 
/// 在六边形架构中,本层位于被驱动适配器侧,实现领域端口:
/// 
/// - **上游依赖**: 被应用层通过仓储接口调用
///   - **下游依赖**: 依赖 `patra-registry-domain` 领域模型和端口接口
///   - **实现职责**: 实现领域层定义的 `*Repository` 端口接口
///   - **技术选型**: 使用 MyBatis-Plus 作为持久化框架
/// 
/// ## 包结构
/// 
/// - `persistence` - 持久化根包
///       
/// - `repository` - 仓储接口实现(领域端口实现)
///         - `entity` - 数据库实体对象(DO)
///         - `mapper` - MyBatis-Plus Mapper 接口
///         - `converter` - 实体与领域对象转换器
/// 
/// ## 核心职责
/// 
/// - 实现领域仓储接口,提供数据持久化能力
///   - 将领域对象转换为数据库实体(DO)并持久化
///   - 从数据库查询实体并转换为领域对象
///   - 处理数据库事务和并发控制
///   - 优化数据库查询性能(索引、批量操作等)
/// 
/// ## 设计原则
/// 
/// - **端口适配器模式**: 实现领域端口接口,隔离领域层和持久化技术
///   - **DO/VO 分离**: 数据库实体(DO)与领域值对象(VO)分离,通过转换器映射
///   - **MyBatis-Plus 规范**: 使用 MyBatis-Plus BaseMapper 和 IService 简化 CRUD
///   - **只读查询优化**: 对只读查询使用投影和 DTO,避免加载完整聚合根
/// 
/// ## 数据转换流程
/// 
/// ```java
...
/// ## 技术栈
/// 
/// - **MyBatis-Plus**: 持久化框架,提供增强的 CRUD 能力
///   - **MapStruct**: 实体转换器自动生成
///   - **MySQL**: 关系型数据库
///   - **Flyway**: 数据库版本管理和迁移
/// 
