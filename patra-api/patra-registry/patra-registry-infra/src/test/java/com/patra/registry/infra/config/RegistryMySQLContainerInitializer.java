package com.patra.registry.infra.config;

import com.patra.starter.test.container.initializer.MySQLContainerInitializer;

/// Registry 服务专用 MySQL 容器初始化器。
///
/// 继承 starter-test 的 {@link MySQLContainerInitializer}，指定 registry 服务的数据库名。
///
/// ### 使用方式
///
/// ```java
/// @MybatisPlusTest
/// @ContextConfiguration(initializers = RegistryMySQLContainerInitializer.class)
/// class SomeRepositoryIT {
///     // ...
/// }
/// ```
///
/// @author linqibin
/// @since 0.1.0
/// @see MySQLContainerInitializer
public class RegistryMySQLContainerInitializer extends MySQLContainerInitializer {

  /// 返回 registry 服务的数据库名。
  ///
  /// @return 数据库名 "patra_registry"
  @Override
  protected String getDatabaseName() {
    return "patra_registry";
  }
}
