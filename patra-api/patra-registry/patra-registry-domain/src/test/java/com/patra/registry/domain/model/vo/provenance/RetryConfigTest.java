 ...
 *   <li>✅ 非空白字符串验证（backoffPolicyTypeCode）
 *   <li>✅ 必需字段非 null 验证（effectiveFrom）
 *   <li>✅ 字符串 trim 处理测试（operationType, backoffPolicyTypeCode）
 *   <li>✅ 可选字段处理（effectiveTo, maxRetryTimes 等允许为 null）
 *   <li>✅ record 的 equals/hashCode/toString 测试
 *   <li>✅ 不变性保证
 *   <li>✅ 业务场景测试（不同操作类型、退避策略、熔断器配置等）
 *   <li>✅ 边界条件处理
 * </ul>
 *
 * @author Patra Team
