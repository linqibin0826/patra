 ...
 *
 * <p>This port abstracts persistence concerns for provenance metadata and associated operational
 * configurations (HTTP, retry, rate limit, pagination, batching, window offset).
 *
 * @author linqibin
 * @since 0.1.0
 */
public interface ProvenanceConfigRepository {
   ...
   *
   * <p>This method assembles the aggregate by querying all configuration dimensions (provenance,
   * window offset, pagination, HTTP, batching, retry, rate limit) that are effective at the given
   * instant for the specified operation type.
   *
   * @param provenanceId the provenance identifier
   * @param operationType the operation type; nullable for operation-agnostic queries
   * @param at the effective instant for temporal filtering
