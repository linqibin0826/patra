 ...
 *   <li>分页模式(paginationModeCode)不可为空白,支持PAGE_NUMBER/CURSOR/TOKEN/SCROLL
 *   <li>操作类型(operationType)为null时表示适用于所有操作(HARVEST/UPDATE/BACKFILL)
 *   <li>每个端点定义最多有一个当前有效配置,端点级配置优先于数据源级配置
 * </ul>
 *
 * <p><strong>业务语义</strong>:
 *
 * <ul>
 *   <li>PAGE_NUMBER模式:基于页码和页大小的传统分页
 *   <li>CURSOR模式:基于游标的流式分页,适用于大数据集
 *   <li>TOKEN模式:基于令牌的分页,常见于OAuth保护的API
 *   <li>SCROLL模式:基于滚动ID的深度分页,适用于Elasticsearch等
 *   <li>支持通过JSONPath/XPath提取响应中的分页控制信息
 * </ul>
 *
 ...
 * @param paginationModeCode 分页模式代码(字典值),取值为{@code PAGE_NUMBER/CURSOR/TOKEN/SCROLL},不可为空白
