 ...
 *
 * <ul>
 *   <li>定义表达式快照值对象,聚合字段定义、渲染规则、参数映射和能力
 *   <li>定义表达式字段值对象,描述字段的类型、路径和提取规则
 *   <li>定义渲染规则值对象,指定字段的输出格式和转换逻辑
 *   <li>定义参数映射值对象,描述 API 参数到表达式变量的映射关系
 *   <li>定义能力值对象,声明表达式支持的功能特性
 * </ul>
 *
 * <h2>核心值对象</h2>
 *
 ...
 *   <li><strong>聚合结构</strong>: {@link com.patra.registry.domain.model.vo.expr.ExprSnapshot}
 *       作为聚合根,组合其他值对象提供完整视图
 *   <li><strong>时态支持</strong>: 表达式快照支持时态查询,可获取特定时刻的表达式配置
 *   <li><strong>框架无关</strong>: 纯 Java 对象,不依赖 Spring、JPA 等框架
 * </ul>
 *
 * <h2>ExprSnapshot 聚合</h2>
 *
 * <p>表达式快照是表达式系统的核心聚合,包含以下组成部分:
 *
 * <ul>
 ...
 *   <li><strong>API 请求生成</strong>: 根据参数映射和渲染规则生成 HTTP 请求
 ...
 *
 * <ul>
 *   <li><strong>patra-expr-kernel</strong>: 表达式编译和执行引擎
 *   <li><strong>patra-spring-boot-starter-expr</strong>: Spring Boot 集成层
 *   <li><strong>patra-ingest</strong>: 数据采集服务,使用表达式生成 API 请求
 * </ul>
 *
 * @since 0.1.0
 * @author linqibin
