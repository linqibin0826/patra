...
`Provenance` + `EndpointDefinition` + `WindowOffsetConfig` + `PaginationConfig` + `HttpConfig` + `BatchingConfig` + `RetryConfig` + `RateLimitConfig` + `Credential`。
...
迁移进度：Dictionary / Provenance / Expr 三子域 VO + ReadModel + 聚合 已全部完成；历史 IllegalArgumentException 仅在文档说明中保留背景。
...
| 目标 | 方案 | 优先级 |
|------|------|--------|
| 减少热查询 DB 压力 | Caffeine 本地缓存 + 定时刷新 | HIGH |
...
* DTO 字段演进：新增 → 可选；删除 → 标记 `@Deprecated` 后至少一个小版本迁移。
* 行为兼容：Scope 语义或默认值升级需通过特性开关（Nacos 配置）渐进发布。
...
| 2 | Provenance/Expr 冲突检测（时间窗口重叠） | 稳定性 | 待办 |
| 3 | Caffeine 缓存 + 版本失效 | 性能 | 待办 |
| 4 | Snapshot 预编译（表达式/模板） | 性能 | 待办 |
| 5 | Outbox 事件广播缓存刷新 | 可扩展 | 待办 |
| 6 | JSON Schema 校验 paramsJson | 数据质量 | 待办 |
| 7 | 健康巡检任务（周期扫描） | 可观测性 | 待办 |
| 8 | 分层错误码细化 422 vs 400 | 可维护 | 待办 |
...
## 16. 未来可能的演进方向（脑暴）
* 规则：把 Expr 与外部规则引擎（Drools/SpEL）桥接，统一编排 DSL
* 自助化：Registry UI（配置回放 + 对比 + 预估影响）
* 蓝绿配置：双时间窗口预热 + 切换阈值观测
