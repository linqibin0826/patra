 ...
 *
 * <ul>
 *   <li>{@link CircuitBreaker} - 错误解析专用熔断器,保护错误处理管道
 *   <li>{@link CircuitBreakerInterceptor} - 熔断器拦截器,在管道中执行熔断保护
 * </ul>
 *
 * <p>启用条件:
 *
 * <ul>
 *   <li>classpath 中存在 Resilience4j CircuitBreaker
 *   <li>{@code patra.error.circuit-breaker.enabled=true}(默认启用)
 * </ul>
 *
...
public class CircuitBreakerErrorAutoConfiguration {
   ...
   *
   * <ul>
   *   <li>失败率阈值: 达到此阈值时触发断路器打开
   *   <li>滑动窗口大小: 计算失败率的样本窗口
   *   <li>最小调用次数: 触发断路器所需的最小调用数
   *   <li>半开状态允许调用数: 断路器半开时允许的探测调用数
   *   <li>断路器打开等待时长: 断路器打开后等待多久进入半开状态
   * </ul>
   *
   ...
   *
   * <p>启用条件: 存在名为 "errorResolutionCircuitBreaker" 的 Bean
   *
