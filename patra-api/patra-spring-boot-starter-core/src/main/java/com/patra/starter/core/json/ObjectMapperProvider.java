...
/// 职责和使用指南：
///
/// - **在 Spring 上下文中**首选方法仍然是构造注入。本提供者通过在应用上下文就绪时将 Spring 管理的实例注册到 {@link JsonMapperHolder}，
///       将其暴露给静态或非 Spring 代码。
...
/// ### 生命周期和线程安全
///
/// - {@link #setApplicationContext(ApplicationContext)} 在 bean 创建期间被调用一次。
...
public class ObjectMapperProvider implements ApplicationContextAware {
  ...
  ///   - 否则调用回退到 {@link JsonMapperHolder}，可能延迟创建默认映射器。
  ///
