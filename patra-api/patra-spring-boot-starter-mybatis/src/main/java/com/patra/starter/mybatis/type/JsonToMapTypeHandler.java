...
/**
 * 用于在 {@code Map<String, Object>} 与数据库列中存储的 JSON 字符串之间进行转换的类型处理器。
 *
 * <p><b>功能说明:</b>
 *
 * <ul>
 *   <li><b>写入:</b> 将 {@code Map<String, Object>} 序列化为 JSON 字符串,使用 {@code setString} 存储到数据库。
 *   <li><b>读取:</b> 使用 {@code getString} 从数据库反序列化 JSON 字符串为 {@code Map<String, Object>}。
 *   <li><b>验证:</b> 执行轻量级检查,确保 JSON 字符串表示一个对象(即以 '{' 开头)。
 * </ul>
 */
