 ...
 * <p>命名规范：
 * <ul>
 *   <li>格式：patra.{module}.{metric}</li>
 *   <li>正则：^patra\.[a-z0-9_]+(\.[a-z0-9_]+)*$</li>
 *   <li>只能包含小写字母、数字、下划线、点号</li>
 * </ul>
 *
 * <p>使用场景：
 * <ul>
 *   <li>统一命名：确保所有指标遵循一致的命名规范</li>
 *   <li>自动修正：自动修正不符合规范的指标名称</li>
 *   <li>防止混乱：避免因命名不一致导致的查询困难</li>
 * </ul>
 *
 * @author Jobs
 * @since 1.0.0
...
public class MetricNamingMeterFilter implements MeterFilter {
    ...
    public MetricNamingMeterFilter(String customPrefix) {
        ...
        log.info("初始化指标命名规范过滤器，自定义前缀: {}",
            this.customPrefix != null ? this.customPrefix : "无");
    }
