 ...
 * Design goals:
 * <ul>
 *   <li>Register the provider only when {@link ObjectMapper} is on the classpath.</li>
 *   <li>Bridge the Spring-managed mapper into {@code JsonMapperHolder} so non-Spring code paths share
 *   the same configuration.</li>
 * </ul>
 *
 * <h3>Relationship with dependency injection</h3>
 * Application code should still <b>prefer dependency injection</b> to obtain an {@code ObjectMapper}.
 * This auto-configuration merely supplies a bridge for contexts where DI is impossible, and it is
 * not intended to act as a service locator.
 */
@Slf4j
@AutoConfiguration
