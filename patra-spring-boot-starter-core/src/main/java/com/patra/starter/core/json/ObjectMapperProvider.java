 ...
 *   provider simply exposes the Spring-managed instance to static or non-Spring code by registering
 *   it with {@link JsonMapperHolder} once the application context is ready.</li>
 *   <li><b>Outside Spring</b> the {@link JsonMapperHolder} lazily creates a default mapper. When the
 *   application later starts inside Spring, this provider replaces the default instance so both
 *   environments share the same configuration.</li>
 * </ul>
 *
 ...
 * <ul>
 *   <li>{@link #setApplicationContext(ApplicationContext)} is invoked once during bean creation. The
 *   method caches the container-managed mapper and registers it with the global holder.</li>
 *   <li>Registration is idempotent; a later registration simply replaces the previous mapper.</li>
 *   <li>{@link #getObjectMapper()} falls back to {@link JsonMapperHolder} when the cached mapper is
 *   unavailable, ensuring consistent behavior regardless of initialization order.</li>
 * </ul>
 *
 * <h3>Usage tips</h3>
 * <ul>
 *   <li>Favor constructor injection inside Spring components.</li>
 *   <li>Use {@link JsonMapperHolder#getObjectMapper()} only when DI is not an option.</li>
 *   <li>Avoid repeatedly registering different mappers at runtime to prevent configuration drift.</li>
 * </ul>
 */
public class ObjectMapperProvider implements ApplicationContextAware {
     ...
     * <ul>
     *   <li>If the Spring context already exposed the mapper, the cached instance is returned.</li>
