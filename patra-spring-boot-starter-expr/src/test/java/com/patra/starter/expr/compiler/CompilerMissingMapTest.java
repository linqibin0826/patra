...
/** P4.2.3 — Missing param map should yield W-PARAM-MAP-MISSING warning. */
@DisplayName("Compiler Missing Param Map Tests")
class CompilerMissingMapTest {

  @Test
  @DisplayName("Missing mapping for std_key should produce W-PARAM-MAP-MISSING and skip param")
  void missingMappingYieldsWarning() {
        ...
        new ProvenanceSnapshot(
            ...
            Map.of(
                ...
                new ProvenanceSnapshot.Capability(
                    ...
                    Set.of(),
                    null)),
            Map.of(), // apiParameterMap intentionally empty
            List.of());
    ...
    ExprRenderer renderer =
        (expression, s, traceEnabled) ->
            // Emit std_key with no matching mapping; keep query blank to focus on param map warning
            new ExprRenderer.RenderOutcome("", Map.of("foo", "bar"), List.of(), null);
    ...
    assertThat(result.report().warnings())
        .anySatisfy(w -> assertThat(w.code()).isEqualTo("W-PARAM-MAP-MISSING"));
