...
### 用户故事 6 - 实现错误分类和自动重试机制 (优先级: P3)

**用户**: 运维团队

**当前问题**: 所有错误都被视为不可恢复错误，即使是临时的网络超时或API限流也不会自动重试，导致大量需要人工重试的任务。无法区分哪些错误应该重试、哪些错误应该立即失败。

**目标**: 增强AdapterResult支持ErrorType枚举（RETRIABLE、NON_RETRIABLE），适配器根据异常类型和HTTP状态码标识错误类型。修改GenericBatchExecutor实现自动重试逻辑（指数退避，最多3次）。

**为什么是这个优先级**: 这是提升系统自动化水平的能力，能减少人工干预。虽然不影响核心功能，但能显著提升运维效率，支持"减少50%人工干预"的业务指标。优先级低于P1和P2是因为可以在架构改造完成后逐步优化。

**独立测试**: 可以通过Mock不同类型的异常（SocketTimeoutException、HttpClientErrorException(429)、IllegalArgumentException），验证适配器能够正确标识错误类型，GenericBatchExecutor根据错误类型决定是否重试。

**验收场景**:

1. **假设** PubMedClient抛出SocketTimeoutException, **当** 捕获异常, **那么** 返回AdapterResult.retriableFailure("网络超时")，errorType=RETRIABLE
2. **假设** AdapterResult的errorType=RETRIABLE, **当** GenericBatchExecutor处理结果, **那么** 自动重试，重试间隔为1秒、2秒、4秒（指数退避），最多重试3次
3. **假设** HTTP 429 Too Many Requests且响应头包含Retry-After: 60, **当** 解析响应, **那么** 返回retriableFailure并在metadata中记录retryAfterSeconds=60，GenericBatchExecutor等待60秒后重试
4. **假设** HTTP 401 Unauthorized, **当** 解析响应, **那么** 返回AdapterResult.nonRetriableFailure("API认证失败")，errorType=NON_RETRIABLE，GenericBatchExecutor立即停止，不重试
...
- **当数据源API限流导致无法获取数据时如何处理?**
  系统识别限流错误(HTTP 429),自动等待并重试,如果超过最大重试次数仍失败,标记任务为"限流失败,请稍后重试"。
...
- **错误类型(ErrorType)**: 枚举值,表示错误的分类(NONE、RETRIABLE、NON_RETRIABLE、PARTIAL_SUCCESS),用于指导重试策略
...
- **适配器调用失败(AdapterInvocationFailed)**: 适配器调用外部API失败时发布
  - **携带数据**: 数据源代码、错误类型(可重试/不可重试)、错误消息、重试次数、时间戳
...
- **与配置中心集成**: 从patra-registry获取数据源配置(API地址、密钥、超时配置、重试配置)
...
| 错误分类不准确导致错误重试 | 中 | 中 | 1. 基于HTTP状态码和异常类型建立错误分类规则 2. 提供手动配置覆盖机制 3. 监控重试成功率并调整策略 |
...
- GenericBatchExecutor实现自动重试逻辑（指数退避，最多3次）
- 完整的开发文档（README.md）和示例代码

**验收标准**:
- 批量处理单元测试覆盖正常场景和部分成功场景
- 自动重试单元测试覆盖可重试和不可重试错误
