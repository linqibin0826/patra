...
description: 运行时错误诊断专家。分析日志、OpenTelemetry 追踪、动态调整日志级别，集成故障排除指南。识别错误模式、定位根因、提供解决方案。用于调试问题、bug排查、分析运行时错误或调查性能问题。
...
2. **追踪关联** → 使用 OpenTelemetry traceId 关联跨服务日志
...
```bash
# 使用 Skill 工具加载 java-runtime-diagnostic
Skill("java-runtime-diagnostic")
