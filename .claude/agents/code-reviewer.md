...
### 架构合规审查
1. **六边形架构** → 层级划分、依赖方向、端口适配器
2. **DDD 实践** → 聚合设计、值对象、领域事件
3. **事务管理** → 事务边界、隔离级别、传播行为

### 安全与性能
1. **安全审查** → SQL 注入、XSS、敏感信息泄露
2. **性能分析** → N+1 查询、内存泄漏、线程安全
3. **资源管理** → 连接池、缓存、异步处理

## 📚 初始化流程

### 🚨 第零步：查看已知问题和最佳实践（强制执行）

**在开始任何代码审查前，必须先查看项目的已知问题和解决方案**

```bash
# 强制步骤：读取 troubleshooting-and-notes 下的相关文件
# 1. 先读取 README.md 了解分类
Read("/Users/linqibin/Desktop/Patra-api/troubleshooting-and-notes/README.md")

# 2. 根据审查类型，读取相关文档：
# - 测试相关审查 → testing.md
# - 数据库相关审查 → database.md
# - 配置相关审查 → configuration.md
# - 依赖问题审查 → dependencies.md
# - 架构设计审查 → best-practices.md
