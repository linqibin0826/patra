...
- **patra-backend:patra-troubleshooter** - 领域专家。**当且仅当排查对象是 Patra 后端（Java / Spring / JPA）运行时报错**时，用它执行第一阶段「根因调查」的具体证据收集：异常堆栈分析、traceId 链路追踪、`logs/*.log` 日志定位、源码定位、context7 文档查询与联网搜索。**troubleshooter 仅承担第一阶段取证**——定位根因后回到本技能第二～四阶段（模式对比 → 假设验证 → 实施）完成修复，不替代后续流程。前端（patra-portal）、基建（patra-infra）调试不走此项，仍按本技能四阶段通用流程执行。
