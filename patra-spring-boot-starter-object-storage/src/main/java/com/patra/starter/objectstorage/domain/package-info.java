 ...
 * 对象存储领域模型包。
 *
 * <p>本包定义对象存储操作的核心领域对象和异常类型,提供强类型的 API 契约,确保参数验证和结果传递的类型安全性。
 *
 * <h2>职责</h2>
 *
 * <ul>
 *   <li>定义上传操作的输入参数 {@link ObjectMetadata}
 *   <li>定义上传操作的返回结果 {@link UploadResult}
 *   <li>定义领域异常类型,区分验证错误和操作失败
 *   <li>提供不可变的值对象,避免并发修改问题
 * </ul>
 *
 * <h2>核心组件</h2>
 *
 * <ul>
 *   <li>{@link com.patra.starter.objectstorage.domain.ObjectMetadata} - 对象元数据值对象
 *   <li>{@link com.patra.starter.objectstorage.domain.UploadResult} - 上传结果值对象
 *   <li>{@link com.patra.starter.objectstorage.domain.InvalidUploadRequestException} - 参数验证失败异常
 *   <li>{@link com.patra.starter.objectstorage.domain.UploadFailedException} - 上传操作失败异常
 * </ul>
 *
 * <h2>设计决策</h2>
 *
 * <ul>
 *   <li><b>值对象模式:</b> 所有领域对象使用 {@code @Builder} 和 final 字段,确保不可变性
 *   <li><b>异常分层:</b> 区分验证错误(不可重试)和操作失败(可重试),支持精确的异常处理策略
 *   <li><b>类型安全:</b> 使用强类型而非 Map,避免运行时类型转换错误
 *   <li><b>扩展性:</b> 使用 Builder 模式支持未来添加新字段而不破坏现有代码
 * </ul>
 *
 * <h2>领域对象说明</h2>
 *
 ...
 *   <li>inputStream 为 null
 *   <li>contentLength 为负数或超过限制
 *   <li>contentType 格式不正确
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>不可变性:</b> 所有字段使用 final 修饰,避免并发修改问题
 *   <li><b>Builder 模式:</b> 使用 Lombok {@code @Builder} 注解简化对象构造
 *   <li><b>异常消息:</b> 异常消息应清晰描述失败原因,便于调试和日志分析
 *   <li><b>存储键格式:</b> storageKey 包含完整路径,objectKey 仅包含对象部分,注意区分
 * </ul>
 *
 * <h2>相关模块</h2>
 *
 * <ul>
 *   <li>{@link com.patra.starter.objectstorage.ObjectStorageOperations} - 使用领域对象的操作接口
 *   <li>{@link com.patra.starter.objectstorage.ObjectStorageTemplate} - 处理领域异常的模板类
 *   <li>{@link com.patra.starter.objectstorage.metrics.ObjectStorageMetrics} - 基于异常类型记录指标
 * </ul>
 *
 * @since 0.1.0
 * @author linqibin
