 ...
 *
 * <p>Allows callers to override system-level resilience settings (timeout, retries, rate limit,
 * circuit breaker) for specific requests. All overrides are capped at system maximum values.
 *
...
public record ResilienceConfigDTO(
    @Positive(message = "Timeout must be positive") Long timeoutSeconds,
    @PositiveOrZero(message = "MaxRetries cannot be negative") Integer maxRetries,
    @PositiveOrZero(message = "RetryBackoff cannot be negative") Long retryBackoffSeconds,
    @Positive(message = "RateLimit must be positive") Integer rateLimit,
    @Positive(message = "CircuitBreakerThreshold must be positive") Integer circuitBreakerThreshold,
    @Positive(message = "CircuitBreakerWindow must be positive") Long circuitBreakerWindowSeconds,
  ...
  public ResilienceConfigDTO {
    ...
    responseHeaderWhitelist =
        responseHeaderWhitelist != null ? List.copyOf(responseHeaderWhitelist) : null;
  }
}
