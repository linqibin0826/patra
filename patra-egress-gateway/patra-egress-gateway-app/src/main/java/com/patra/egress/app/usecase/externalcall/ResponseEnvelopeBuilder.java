...
public class ResponseEnvelopeBuilder {
  ...
  public ResponseEnvelope build(
    ...
    Map<String, String> filteredHeaders =
        HeaderWhitelistFilter.filter(rawResponse.headers(), headerWhitelist);
    ...
    ExternalRateLimitInfo externalRateLimitInfo =
        ExternalRateLimitInfo.fromHeaders(rawResponse.headers());

    // Create combined rate limit status
    RateLimitStatus combinedRateLimitStatus =
        new RateLimitStatus(
    ...
    RetryAdvice generatedRetryAdvice =
        retryAdvice != null ? retryAdvice : RetryAdvice.notRetryable();
