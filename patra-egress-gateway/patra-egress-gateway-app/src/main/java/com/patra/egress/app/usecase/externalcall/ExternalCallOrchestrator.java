...
/**
 * External call orchestrator implementation Simplified version without resilience capabilities
 * (rate limiting, retry, circuit breaker) These will be added in Task 6
 *
 * @author linqibin
 * @since 0.1.0
 */
...
public class ExternalCallOrchestrator implements ExternalCallUseCase {
  ...
  public ExternalCallResult execute(ExternalCallCommand command) {
    ...
    log.info(
        "[EGRESS][APP] External call started: url={} method={} traceId={}",
        command.request().url(),
    ...
    try {
      ...
      log.info(
          "[EGRESS][APP] External call completed: statusCode={} duration={}ms traceId={}",
          response.statusCode(),
          duration.toMillis(),
          traceId);
      ...
      Duration duration = Duration.between(startTime, Instant.now());
      log.error(
          "[EGRESS][APP] External call failed: url={} duration={}ms error={} traceId={}",
          command.request().url(),
          duration.toMillis(),
          e.getMessage(),
          traceId,
  ...
  private ResilienceConfig loadAndMergeConfig(ExternalCallCommand command, String traceId) {
    ...
    if (command.callerConfig() != null) {
      ...
      if (!command.callerConfig().equals(mergedConfig)) {
        log.warn(
            "[EGRESS][APP] Caller config exceeded system max values, using capped values: traceId={}",
            traceId);
      }
      ...
      log.debug(
          "[EGRESS][APP] No caller config provided, using system default: traceId={}", traceId);
  ...
  private ResponseEnvelope buildResponseEnvelope(HttpResponse response, ResilienceConfig config) {
    ...
    return responseEnvelopeBuilder.build(
        response, rateLimitStatus, retryAdvice, config.responseHeaderWhitelist());
  }
}
