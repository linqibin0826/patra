...
class RetryAdviceTest {
  ...
  void fromResponse_shouldSuggestRetry_when429TooManyRequests() {
    // Given
    HttpResponse response = new HttpResponse(429, Map.of(), "Rate limit exceeded");
  ...
  void fromResponse_shouldSuggestRetry_when503ServiceUnavailable() {
    // Given
    HttpResponse response = new HttpResponse(503, Map.of(), "Service unavailable");
  ...
  void fromResponse_shouldSuggestRetry_when408RequestTimeout() {
    // Given
    HttpResponse response = new HttpResponse(408, Map.of(), "Request timeout");
  ...
  @Test
  @DisplayName(
      "fromResponse() should fall back to the default delay when Retry-After is not numeric")
  void fromResponse_shouldUseDefaultBackoff_whenRetryAfterIsNotNumeric() {
    // Given
    HttpResponse response =
        new HttpResponse(
            429, Map.of("Retry-After", List.of("not-a-number")), "Rate limit exceeded");
    ...
    // Then
    assertThat(advice.retryable()).isTrue();
    assertThat(advice.suggestedDelay())
        .isEqualTo(Duration.ofSeconds(2)); // Falls back to configured backoff
  }
  ...
  void fromResponse_shouldUseRetryAfterHeader_when503WithRetryAfter() {
    // Given
    HttpResponse response =
        new HttpResponse(503, Map.of("Retry-After", List.of("30")), "Service unavailable");
