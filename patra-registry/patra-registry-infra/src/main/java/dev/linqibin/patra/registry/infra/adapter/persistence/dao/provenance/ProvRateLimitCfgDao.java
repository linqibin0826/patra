...
/// 速率限制配置 JPA Repository。
///
/// **职责**：
///
/// - 提供 ProvRateLimitCfgEntity 的 CRUD 操作
/// - 支持时态切片和操作优先级查询
///
/// @author linqibin
/// @since 0.1.0
public interface ProvRateLimitCfgDao extends JpaRepository<ProvRateLimitCfgEntity, Long> {

  /// 查询指定数据源和操作作用域的有效速率限制配置。
  ///
  ...
  @Query(
      value =
          """
          SELECT * FROM reg_prov_rate_limit_cfg
          ...
          """,
      nativeQuery = true)
  Optional<ProvRateLimitCfgEntity> findActiveMerged(
