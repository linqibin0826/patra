...
class ExprRepositoryMpImplTest {
    ...
    @BeforeEach
    void setUp() {
        repo = new ExprRepositoryMpImpl(fieldDictMapper, apiParamMapMapper, capabilityMapper, renderRuleMapper, provenanceMapper, converter);
    }
    ...
    void loadSnapshot_endpointNull_passesNullToApiParamMapper() {
        ...
        when(capabilityMapper.selectActiveByTask(anyLong(), anyString(), any(Instant.class))).thenReturn(Collections.emptyList());
        when(renderRuleMapper.selectActiveByTask(anyLong(), anyString(), any(Instant.class))).thenReturn(Collections.emptyList());
        when(apiParamMapMapper.selectActiveByTask(anyLong(), anyString(), any(), any(Instant.class))).thenReturn(Collections.emptyList());
        ...
        verify(apiParamMapMapper).selectActiveByTask(eq(1L), eq("harvest"), endpointCaptor.capture(), any(Instant.class));
        assertNull(endpointCaptor.getValue(), "endpoint should be null when input endpointName is null");
    }

    @Test
    void resolveProvenance_notFound_throwDomainException() {
        when(provenanceMapper.selectByCode("PUBMED")).thenReturn(Optional.empty());
        assertThrows(ProvenanceNotFoundException.class,
