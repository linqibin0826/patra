 ...
 *
 * <p><strong>不可变性</strong>:此对象一旦创建不可修改,通过值语义比较相等性。作为所有数据源配置的 根实体,为下游配置(分页、HTTP、重试等)提供唯一标识和基础信息。
 *
 * <p><strong>业务约束</strong>:
 *
 * <ul>
 *   <li>数据源ID必须为正整数,作为全局唯一标识符
 *   <li>数据源代码(code)必须全局唯一且稳定不变,如{@code pubmed}、{@code crossref}
 *   <li>数据源名称、默认时区不可为空白字符串
 *   <li>默认基础URL、文档URL为可选项,允许为空
 *   <li>所有字符串字段自动执行trim操作
 *   <li>仅激活状态的数据源才能被用于数据采集流程
 * </ul>
 *
 * @param id 数据源主键,所有下游配置引用的唯一标识符,必须为正整数
 * @param code 数据源代码,全局唯一且稳定(如{@code pubmed}、{@code crossref}),用于编程引用和约束
 * @param name 数据源显示名称(如"PubMed"、"Crossref"),便于人工阅读和界面展示
 * @param baseUrlDefault 默认基础URL,与端点路径组合形成完整API地址,可被HTTP策略覆盖,可为null
 * @param timezoneDefault 默认时区(IANA格式,如{@code UTC}、{@code Asia/Shanghai}),用于时间窗口计算
...
public record Provenance(
   ...
   * <ul>
   *   <li>数据源ID必须为正整数
   *   <li>数据源代码、名称、时区、生命周期状态不可为空白
   *   <li>所有字符串字段自动trim去除首尾空白
   *   <li>可选字段(基础URL、文档URL)允许为null
   * </ul>
   *
   ...
   * 判断数据源是否处于激活状态。
   *
   * <p>仅激活状态的数据源才能被用于数据采集流程。
   *
   * @return 如果数据源激活则返回{@code true},否则返回{@code false}
   */
