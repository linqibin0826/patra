 ...
 *   <li>退避策略类型(backoffPolicyTypeCode)不可为空白,支持FIXED/LINEAR/EXPONENTIAL
 *   <li>操作类型(operationType)为null时表示适用于所有操作(HARVEST/UPDATE/BACKFILL)
 *   <li>与HTTP配置的Retry-After策略配合工作,共同控制429/5xx/网络/客户端错误
 * </ul>
 *
 * <p><strong>业务语义</strong>:
 *
 * <ul>
 *   <li>重试次数控制:maxRetryTimes为0表示不重试,null表示使用默认值
 *   <li>退避策略:FIXED(固定延迟)、LINEAR(线性递增)、EXPONENTIAL(指数退避)
 *   <li>抖动机制:jitterFactorRatio添加随机性,防止雷鸣般的重试风暴
 *   <li>HTTP状态码分类:retryHttpStatusJson定义可重试状态码,giveupHttpStatusJson定义立即放弃的状态码
 *   <li>网络错误处理:retryOnNetworkError控制连接超时/重置等网络级错误是否重试
 *   <li>熔断器保护:连续失败N次后打开熔断器,冷却期后半开尝试
 * </ul>
 *
 ...
 * @param backoffPolicyTypeCode 退避策略类型代码(字典值),定义退避策略({@code FIXED/LINEAR/EXPONENTIAL}),不可为空白
 ...
 * @param retryHttpStatusJson 触发重试的HTTP状态码(JSON数组,如{@code [429, 503]}),null表示使用默认值
 * @param giveupHttpStatusJson 立即放弃的HTTP状态码(JSON数组,如{@code [400, 401, 403]}),null表示无
