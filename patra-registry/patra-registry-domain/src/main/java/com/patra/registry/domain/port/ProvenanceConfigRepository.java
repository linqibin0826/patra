 ...
 *
 * <ul>
 *   <li>保存和查询数据源元数据及其关联的运营配置
 *   <li>根据业务标识(数据源代码)查询数据源
 *   <li>根据时态查询条件检索特定时刻有效的配置
 *   <li>组装完整的数据源配置聚合根
 * </ul>
 *
 * <p><strong>时态查询支持</strong>:所有{@code findActive*}方法接受{@code at}参数, 查询指定时刻有效的配置,支持配置的安全更新、审计和回溯查询。
 *
 * <p><strong>注意</strong>:所有方法使用领域语言表达业务意图,隐藏底层持久化技术细节(SQL/NoSQL)。
 *
...
public interface ProvenanceConfigRepository {
   ...
   * 根据数据源代码查询数据源元数据。
   *
   * <p>数据源代码是全局唯一且稳定的业务标识符,如{@code pubmed}、{@code crossref}。
   *
   ...
   * 查询指定时刻有效的时间窗口偏移配置。
   *
   * <p>支持作用域优先级:TASK级 > OPERATION级 > SOURCE级。
   *
   * @param provenanceId 数据源ID,不可为null
   * @param operationType 操作类型({@code HARVEST/UPDATE/BACKFILL}),null表示查询全局配置
   ...
   *
   * @param provenanceId 数据源ID,不可为null
   * @param operationType 操作类型({@code ALL/HARVEST/UPDATE/BACKFILL}),null表示查询全局配置
   * @param at 查询时刻,用于时态过滤,不可为null
   ...
   * 加载完整的数据源配置聚合根。
   *
   * <p>此方法通过查询所有配置维度(数据源元数据、时间窗口偏移、分页、HTTP、批处理、 重试、速率限制)并组装成一个完整的聚合根对象。仅返回在指定时刻对指定操作类型有效的配置。
   *
   * <p>业务规则:
   *
   * <ul>
   *   <li>数据源元数据是聚合的必需核心,不存在则返回空
   *   <li>各维度配置为可选,缺失时聚合根中对应字段为null
   *   <li>应用配置作用域优先级规则,高优先级配置覆盖低优先级配置
   * </ul>
   *
   * @param provenanceId 数据源ID,不可为null
