 ...
 *
 * <ul>
 *   <li>定义数据源元数据值对象({@link com.patra.registry.domain.model.vo.provenance.Provenance})
 *   <li>定义运营配置策略值对象(HTTP、重试、分页、批处理、限流、时间窗口)
 *   <li>封装业务约束和验证逻辑
 *   <li>提供时态配置支持(effectiveFrom/effectiveTo)
 *   <li>支持配置作用域优先级(TASK > OPERATION > SOURCE)
 * </ul>
 *
 * <h2>核心值对象</h2>
 *
 ...
 * <h2>时态配置</h2>
 *
 * <p>所有运营配置值对象都包含时间有效性范围:
 *
 * <ul>
 *   <li>{@code effectiveFrom} - 配置生效时间(包含),标记此配置开始生效的时刻
 *   <li>{@code effectiveTo} - 配置失效时间(不包含),null 表示永久有效
 * </ul>
 *
 * <p>通过时态查询,可以在任意时间点获取当时有效的配置,支持配置的安全更新和审计。
 *
 * <h2>配置作用域</h2>
 *
 * <p>配置按作用域分为三级,优先级从高到低:
 *
 * <ul>
