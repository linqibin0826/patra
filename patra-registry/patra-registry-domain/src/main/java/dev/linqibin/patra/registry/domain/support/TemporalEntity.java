package dev.linqibin.patra.registry.domain.support;
...
/// 表示具有时态有效性约束的实体。
///
/// 实现此接口的实体具有由 `effectiveFrom` 和 `effectiveTo` 时间戳定义的有效期, 支持时态查询和配置切片。
///
/// @author linqibin
/// @since 0.1.0
public interface TemporalEntity {
  ...
  /// 检查此实体在给定时刻是否有效。
  ///
  /// 当时刻落在有效期内时,实体有效: `effectiveFrom <= instant < effectiveTo`。
  ///
