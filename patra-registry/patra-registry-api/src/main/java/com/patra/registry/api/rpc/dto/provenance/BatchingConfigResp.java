package com.patra.registry.api.rpc.dto.provenance;

import java.time.Instant;

/**
 * Response DTO describing batching configuration for provenance requests.
 *
 * <p>Field descriptions:
 * <ol>
 *   <li>id - primary identifier of the batching configuration row</li>
 *   <li>provenanceId - provenance owning the configuration</li>
 *   <li>operationType - operation discriminator the configuration applies to</li>
 *   <li>effectiveFrom - timestamp from which the configuration becomes effective</li>
 *   <li>effectiveTo - timestamp until which the configuration remains effective</li>
 *   <li>detailFetchBatchSize - default batch size used when fetching detail IDs</li>
 *   <li>idsParamName - request parameter name carrying batched IDs</li>
 *   <li>idsJoinDelimiter - delimiter used when joining IDs into a single value</li>
 *   <li>maxIdsPerRequest - maximum IDs permitted per outbound request</li>
 * </ol>
 *
 * @author linqibin
 * @since 0.1.0
 */
public record BatchingConfigResp(
        Long id,
        Long provenanceId,
        String operationType,
        Instant effectiveFrom,
        Instant effectiveTo,
        Integer detailFetchBatchSize,
        String idsParamName,
        String idsJoinDelimiter,
        Integer maxIdsPerRequest
) {
}
