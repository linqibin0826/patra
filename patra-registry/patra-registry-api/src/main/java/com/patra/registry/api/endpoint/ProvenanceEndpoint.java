 ...
 *
 * <p>通过 Feign 客户端集成向内部微服务暴露数据源元数据和有效配置查询能力。
 *
 * <p>端点:
 *
 * <ul>
 *   <li>GET /_internal/provenances - 列出所有数据源
 *   <li>GET /_internal/provenances/{code} - 获取单个数据源
 *   <li>GET /_internal/provenances/{code}/config - 加载配置聚合
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
...
public interface ProvenanceEndpoint {
   ...
   * 加载数据源的聚合配置。
   *
   * <p>通过解析时态切片并组装所有配置维度到统一视图来检索有效配置。
   *
   * @param code 数据源代码
   * @param operationType 操作类型(如 HARVEST/UPDATE),{@code null} 表示所有类型
   * @param at 查询有效配置的时间点,{@code null} 默认为当前时间
   * @return 聚合数据源配置
   */
