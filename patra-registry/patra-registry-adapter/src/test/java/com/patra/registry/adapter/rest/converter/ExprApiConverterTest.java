 ...
 *
 * <p>测试策略: 使用 MapStruct 生成的实现类进行纯 Java 单元测试, 无需 Spring 容器。
 *
 * <p>测试覆盖:
 *
 * <ul>
 *   <li>字段映射验证 - Query → Resp 所有字段正确映射
 *   <li>List 转换 - 集合转换保持迭代顺序
 *   <li>Null 值处理 - 输入为 null 或可选字段为 null 的场景
 *   <li>布尔字段转换 - boolean 类型正确映射
 *   <li>时间类型转换 - Instant/LocalDate 正确转换
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
