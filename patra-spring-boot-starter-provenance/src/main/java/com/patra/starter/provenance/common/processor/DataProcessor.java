 ...
 * 数据处理器策略接口
 *
 * <p>DataProcessor是多数据类型架构的核心策略接口，每个数据类型（PUBLICATION、JOURNAL、DRUG等） 都有对应的Processor实现类。
 *
 ...
 *
 * <ul>
 *   <li>处理特定数据类型的数据获取流程（ESearch/EFetch等）
 *   <li>转换原始数据为领域模型（如PubmedPublication → CanonicalPublication）
 *   <li>验证数据完整性和正确性
 * </ul>
 *
...
public interface DataProcessor<T> {
   ...
   *
   * <ol>
   *   <li>从外部API获取原始数据（如PubMed ESearch/EFetch）
   *   <li>转换原始数据为领域模型（调用transform方法）
   *   <li>验证数据完整性（调用validate方法）
   *   <li>返回处理结果
   * </ol>
   *
   ...
   *
   * <ul>
   *   <li>网络错误、超时：返回FAILED状态，由上层重试
   *   <li>部分数据转换失败：返回PARTIAL_SUCCESS状态
   *   <li>验证失败：返回VALIDATION_ERROR状态
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>出版物数据：PMID、标题、作者不能为空
   *   <li>期刊数据：ISSN、期刊名不能为空
   *   <li>药品数据：DrugBank ID、药品名不能为空
   * </ul>
   *
   * @param data 待验证的数据
   * @return 验证结果
   ...
   * 转换原始数据为目标类型
   *
   * <p>将外部API返回的原始数据（如PubmedPublication、DoajJournal） 转换为领域模型（如CanonicalPublication、Journal）。
   *
   * @param rawData 原始数据
   * @return 转换后的领域模型
