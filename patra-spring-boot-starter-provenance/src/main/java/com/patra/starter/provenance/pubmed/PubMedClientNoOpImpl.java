...
/**
 * No-op implementation of PubMedClient when EgressGatewayClient is not available. Returns empty
 * responses to prevent service failure.
 *
 * @author linqibin
 * @since 0.1.0
 */
...
public class PubMedClientNoOpImpl implements PubMedClient {
  ...
  public ESearchResponse esearch(ESearchRequest request, ProvenanceConfig config) {
    log.warn(
        "[PROVENANCE][CORE] EgressGatewayClient not available, returning empty esearch response");
  ...
  public EFetchResponse efetch(EFetchRequest request, ProvenanceConfig config) {
    log.warn(
        "[PROVENANCE][CORE] EgressGatewayClient not available, returning empty efetch response");
