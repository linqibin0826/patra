/// PubMed 医学出版物转换器包。
/// 
/// 负责将 PubMed XML/JSON 响应对象转换为 {@link com.patra.common.model.CanonicalPublication} 规范化医学出版物模型。
/// 
/// ## 职责
/// 
/// - 解析 PubMed XML/JSON 响应（通过 Jackson）
///   - 提取完整的医学出版物元数据（P0 核心字段 + P1 医学领域字段）
///   - 转换为 {@link com.patra.common.model.CanonicalPublication} 标准模型
///   - 处理不完整或缺失的字段（可选字段安全处理）
///   - 支持医学领域特有字段（MeSH、研究者、参考文献等）
/// 
/// ## 核心组件
/// 
/// - {@link com.patra.starter.provenance.pubmed.converter.PubmedPublicationConverter} - PubMed 医学出版物转换器（主类）
/// 
/// ## 转换方法清单
/// 
/// ### P0 核心字段转换（必须支持）
/// 
/// - `buildIdentifiers()` - 构建标识符列表（PMID, DOI, PMC, PII 等）
///   - `extractAbstract()` - 提取摘要信息（支持结构化和非结构化摘要）
///   - `convertAuthors()` - 转换作者信息（包含 valid 字段，v0.1.0 新增）
///   - `convertJournal()` - 转换期刊信息（标题、ISSN、卷期等）
///   - `extractPublicationDates()` - 提取出版日期（published, electronic, revised, completed 等）
///   - `convertPagination()` - 转换页码信息（v0.1.0 重构为结构化对象：startPage, endPage, medlinePgn）
///   - `convertFunding()` - 转换资助信息（v0.1.0 重命名 funderIdentifier → funderAcronym）
///   - `convertReferences()` - 转换参考文献列表（v0.1.0 新增）
///   - `extractKeywords()` - 提取关键词集合（支持多来源）
/// 
/// ### P1 医学领域字段转换（增强功能）
/// 
/// - `convertMeshHeadings()` - 转换 MeSH 主题标引（医学索引核心，v0.1.0 从 convertSubjects() 重构）
///   - `convertSupplMeshNames()` - 转换补充 MeSH 概念（疾病、药物试验等，v0.1.0 新增）
///   - `convertInvestigators()` - 转换研究者信息（临床试验、多中心研究，v0.1.0 新增）
///   - `convertPersonalNameSubjects()` - 转换人物主题（传记、医学史、案例报告，v0.1.0 新增）
///   - `convertExternalReferences()` - 转换外部数据库引用（GenBank、ClinicalTrials.gov 等，v0.1.0 新增）
///   - `convertSupplementalObjects()` - 转换补充对象（图表、数据集、多媒体，v0.1.0 新增）
///   - `convertRelatedItems()` - 转换相关项目（更正、撤稿、评论、转载，v0.1.0 新增）
///   - `convertSubstances()` - 转换化学物质列表（CAS 号、MeSH 词表）
///   - `convertGenes()` - 转换基因符号列表（GenBank 引用）
///   - `convertAlternativeAbstracts()` - 转换其他语言或版本的摘要
/// 
/// ### 元数据和出版历史
/// 
/// - `extractMetadata()` - 提取出版物元数据（索引方法、所有者、状态等）
///   - `extractPublicationHistory()` - 提取发布历史时间线（received, accepted, epublish, pubmed 等）
///   - `extractPublicationTypes()` - 提取发表类型列表（Journal Article, Review 等）
///   - `extractAuthorsComplete()` - 提取作者列表完整性标识
/// 
/// ## 转换逻辑示意图
/// 
/// ```
...
/// ## v0.1.0 重大变更
/// 
/// ### 模型重构：从通用学术出版物到医学领域专用
/// 
/// - **移除**: `convertSubjects()` → 替换为 `convertMeshHeadings()`
///   - **重构**: `convertPagination()` → 返回结构化对象而非 String
///   - **增强**: `convertAuthors()` → 新增 valid 字段映射
///   - **增强**: `convertFunding()` → funderIdentifier → funderAcronym
///   - **增强**: `extractPublicationDates()` → 新增 completed 日期
/// 
/// ### 新增转换方法（7 个）
/// 
/// ### 迁移指南
...
/// ## 医学领域标准对齐
/// 
/// - **MeSH** - 美国国家医学图书馆医学主题词表（Medical Subject Headings）
///   - **MEDLINE** - 医学出版物元数据标准（主要数据源）
///   - **PubMed** - PubMed/MEDLINE 数据模型和 XML DTD
///   - **CAS** - 化学文摘社注册号（Chemical Abstracts Service Registry Number）
/// 
/// ## 技术依赖
/// 
/// - `patra-common-model` - CanonicalPublication 规范化模型
///   - `com.patra.starter.provenance.pubmed.model.response` - PubMed 响应模型（XML/JSON 映射）
///   - `Jackson` - JSON/XML 序列化/反序列化
///   - `Spring Utils` - 字符串和集合工具类
/// 
/// @since 0.1.0
/// @author linqibin
