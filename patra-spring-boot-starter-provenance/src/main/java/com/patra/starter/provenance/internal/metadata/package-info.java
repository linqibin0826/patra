 ...
 * <h2>设计定位：</h2>
 *
 * <p>本包属于 <strong>Shared Kernel</strong>（共享内核）模式，提供跨服务共享的通用领域模型， 确保 Ingest、Export、Synchronization
 * 等多个模块对"计划元数据"概念有一致理解。
 *
 * <h2>核心理念：</h2>
 *
 * <ul>
 *   <li><strong>类型安全</strong>：使用继承体系替代 {@code Map<String, Object>}，提供编译时类型检查
 *   <li><strong>多数据源支持</strong>：每个数据源对应一个 PlanMetadata 子类，封装特定元数据
 *   <li><strong>业务约束验证</strong>：在构造函数中强制验证业务规则（如 totalCount ≥ 0）
 *   <li><strong>扩展性</strong>：新增数据源只需创建新子类，无需修改现有代码（符合开闭原则）
 * </ul>
 *
 * <h2>核心组件：</h2>
 *
 * <ul>
 *   <li>{@link com.patra.common.model.plan.PlanMetadata} - 计划元数据抽象基类
 *   <li>{@link com.patra.common.model.plan.PubmedPlanMetadata} - PubMed 特定元数据（WebEnv + QueryKey）
 *   <li>{@link com.patra.common.model.plan.EpmcPlanMetadata} - EPMC 特定元数据（CursorMark）
 *   <li>{@link com.patra.common.model.plan.DoajPlanMetadata} - DOAJ 特定元数据（ScrollId + PageSize）
 * </ul>
 *
 * <h2>类型层次结构：</h2>
 *
 ...
 *       <ul>
 *         <li>✅ 编译时类型安全（无需运行时类型转换）
 *         <li>✅ IDE 友好（自动补全、重构安全）
 *         <li>✅ 业务约束验证（构造函数强制检查）
 *         <li>✅ 清晰的类型关系（继承体系表达数据源差异）
 *       </ul>
