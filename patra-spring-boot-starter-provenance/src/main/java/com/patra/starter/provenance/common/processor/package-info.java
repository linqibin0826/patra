 ...
 * 数据处理器核心包。
 *
 * <p>定义通用的数据处理器契约和上下文管理，为各数据源提供者（如 PubMed、EPMC）提供统一的数据处理基础设施。
 *
 * <h2>职责</h2>
 *
 * <ul>
 *   <li>定义通用数据处理器接口 {@link DataProcessor}
 *   <li>提供处理器上下文管理 {@link ProviderContext}
 *   <li>规范处理结果和验证结果模型
 *   <li>支持数据获取、转换和验证的完整生命周期
 * </ul>
 *
 * <h2>核心组件</h2>
 *
 * <ul>
 *   <li>{@link DataProcessor} - 数据处理器接口，定义数据获取和转换逻辑
 *   <li>{@link ProviderContext} - 处理器上下文，封装配置、客户端和扩展属性
 *   <li>{@link ProcessResult} - 处理结果，包含数据、游标和错误信息
 *   <li>{@link ValidationResult} - 验证结果，包含验证状态和错误详情
 * </ul>
 *
 * <h2>架构位置</h2>
 *
 ...
 *
 * <ul>
 *   <li><b>策略模式</b> - DataProcessor 作为可替换的数据处理策略
 *   <li><b>模板方法</b> - 定义数据处理的标准流程（process → validate → transform）
 *   <li><b>依赖注入</b> - 通过 ProviderContext 注入运行时依赖
 * </ul>
 *
 * <h2>核心流程</h2>
 *
 ...
 *
 * <ul>
 *   <li><b>配置优先</b> - 始终从 ProviderContext 获取配置，不硬编码
 *   <li><b>错误处理</b> - 使用 ProcessResult.failure() 封装错误，不抛异常
 *   <li><b>验证分离</b> - validate() 方法独立验证，不在 process() 中混合验证逻辑
 *   <li><b>转换统一</b> - transform() 方法负责所有数据转换，确保一致性
 *   <li><b>日志记录</b> - 记录关键步骤（搜索、获取、转换）的性能指标
 * </ul>
 *
 * @since 0.1.0
 * @author linqibin
