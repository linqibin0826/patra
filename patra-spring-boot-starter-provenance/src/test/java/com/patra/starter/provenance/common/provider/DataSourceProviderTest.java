 ...
 * <ol>
 *   <li>测试基本接口契约（getProvenanceCode、getSupportedDataTypes）</li>
 *   <li>测试类型支持检查（supports方法）</li>
 *   <li>测试泛型fetchData方法（多种数据类型）</li>
 *   <li>测试getProcessor可选方法</li>
 *   <li>测试ProviderResult泛型化</li>
 * </ol>
 *
...
class DataSourceProviderTest {
    ...
    class FetchDataTests {
        ...
        void should_fetch_literature_data() {
            ...
            // When: 获取LITERATURE数据
            ProviderResult<CanonicalLiterature> result = provider.fetchData(
                request,
                DataType.LITERATURE,
        ...
        void should_fetch_citation_data() {
            ...
            // When: 获取CITATION数据
            ProviderResult<DataType.Citation> result = provider.fetchData(
                request,
                DataType.CITATION,
                DataType.Citation.class
            );
        ...
        void should_reject_unsupported_data_type() {
            ...
            // When: 尝试获取不支持的数据类型
            ProviderResult<DataType.Drug> result = provider.fetchData(
                request,
                DataType.DRUG,
                DataType.Drug.class
            );
    ...
    class ProviderResultGenericTests {
        ...
        void should_create_generic_success_result() {
            ...
            List<CanonicalLiterature> literatures = List.of(
                createMockLiterature("PMID001")
            );

            // When: 创建成功结果
            ProviderResult<CanonicalLiterature> result = ProviderResult.success(
                literatures,
                DataType.LITERATURE,
                "cursor123"
            );
        ...
        void should_create_generic_failure_result() {
            // When: 创建失败结果
            ProviderResult<CanonicalLiterature> result = ProviderResult.failure(
                DataType.LITERATURE,
                "网络错误",
        ...
        void should_create_generic_partial_success_result() {
            ...
            // When: 创建部分成功结果
            ProviderResult<CanonicalLiterature> result = ProviderResult.partialSuccess(
                literatures,
                DataType.LITERATURE,
                "cursor456",
                "部分数据转换失败"
    ...
    private static class MockPubmedProvider implements DataSourceProvider {
        ...
        public <T> ProviderResult<T> fetchData(
            ...
            if (!supports(dataType)) {
                return (ProviderResult<T>) ProviderResult.failure(
                    dataType,
                    "不支持的数据类型: " + dataType,
            ...
            if (dataType == DataType.LITERATURE) {
                return (ProviderResult<T>) ProviderResult.success(
                    List.of(),
                    DataType.LITERATURE,
                    null
                );
