...
- **技术栈**：Java 25、Spring Boot 3.5.7、Spring Data JPA、MySQL 8.x、Nacos
...
├── patra-{service}-api      # 服务契约（DTO/Feign 接口/常量）
...
- **依赖方向（宏观）**：
  ...
  - `api`：只定义契约（DTO/Feign/常量），不承载实现与业务
  - `boot`：唯一启动入口与装配配置
...
- **依赖（Starter）**：数据库用 `patra-spring-boot-starter-jpa`（`starter-jpa`）；对象存储用 `patra-spring-boot-starter-object-storage`（`starter-object-storage`）；REST 调用用 `patra-spring-boot-starter-rest-client`（`starter-rest-client`）；Feign 调用用 `patra-spring-cloud-starter-feign`（`cloud-starter-feign`）
...
- **依赖（Starter）**：Web 用 `patra-spring-boot-starter-web`（`starter-web`）；Feign 用 `patra-spring-cloud-starter-feign`（`cloud-starter-feign`）

### API 模块（`*-api`）

- **职责**：定义服务契约（Feign 接口、DTO、常量），用于跨服务共享
- **禁止**：
  - 禁止包含 Controller 实现
  - 禁止包含业务逻辑
...
- 所有 JPA Entity 必须继承 `BaseJpaEntity`（雪花 ID、审计字段、乐观锁、软删除）
...
- **异常体系**：
  ...
  - `RemoteCallException`：Feign 调用失败的统一异常，实现 `HasErrorTraits`
- **分层规则**：
...
- **Feign**：禁止捕获 `FeignException`，必须走 `RemoteCallException`
