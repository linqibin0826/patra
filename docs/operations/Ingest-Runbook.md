...
How to Run Locally
- Start infra: `cd docker/compose && docker compose up -d`
- Launch service: `./mvnw -pl patra-ingest/patra-ingest-boot -am spring-boot:run`
...
- Correlate
  - `OutboxPublished v1` volume vs `papertrace.outbox.publish.total{status=success}`
  - Trace `ingest.relay.duration` spikes with broker/network incidents
