...
1. **可观测性验证**：
   - [ ] 在完整的 Docker Compose 环境中验证 Metrics → Prometheus → Grafana 流水线
   - [ ] 验证新的 Spring Batch 监控面板数据展示是否正确

2. **LexicalTag 集成**：
   - [ ] 验证新增的 MeSH 词法标记在实际数据解析中的使用情况
   - [ ] 补充 MeSH 术语类型的文档说明

3. **代码质量**：
   - [ ] 考虑为 starter-observability 添加 README 使用说明更新
   - [ ] 检查其他 starter 模块是否有类似的配置简化机会
