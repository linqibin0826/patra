...
**关联 spec:** `docs/superpowers/specs/2026-05-16-rename-config-prefix-to-linqibin-design.md`（HEAD `13ef313fa`）
...
grep -rnE "(^|[^a-zA-Z0-9_])patra\.(batch|async|command-bus|error|tracing|http\.interface|object-storage|object_storage|observability|openapi|redisson|rest-client|web\.problem|expr|provenance)\b" \
  ...
  . 2>/dev/null | grep -vE "(docs/superpowers|\.git/|build/|out/|node_modules/|target/)"
```
Expected: 零结果（注：排除 docs/superpowers 历史档案、构建产物）
...
详细映射表与 DoD 见:
- spec: docs/superpowers/specs/2026-05-16-rename-config-prefix-to-linqibin-design.md
- plan: docs/superpowers/plans/2026-05-16-rename-config-prefix.md
...
- (c) 在 docs/superpowers/ 历史档案中 —— **正常**，应保留
