...
public record RorId(String value) implements Serializable {
  ...
  public RorId {
    ...
    if (!value.startsWith(ROR_URL_PREFIX)) {
      // 验证纯 ID 格式
      Assert.isTrue(value.matches(ROR_ID_PATTERN), "ROR ID 格式无效,必须为 9 位字符且以 0 开头：%s", value);
      value = ROR_URL_PREFIX + value;
    } else {
      // 验证完整 URL 格式
      String id = value.substring(ROR_URL_PREFIX.length());
      Assert.isTrue(id.matches(ROR_ID_PATTERN), "ROR URL 格式无效,ID 部分必须为 9 位字符且以 0 开头：%s", value);
    }
  }
  ...
  public static RorId fromId(String id) {
    ...
    Assert.isTrue(id.matches(ROR_ID_PATTERN), "ROR ID 格式无效,必须为 9 位字符且以 0 开头：%s", id);
