...
/// OpenAlex 富化查询端口。
///
/// 批量查询 OpenAlex Sources API 获取期刊的引用指标、年度统计和开放获取信息，
/// 填充 `CitationMetrics`（引用指标快照）、`VenuePublicationStats`（年度时序统计）
/// 和 `OpenAccessInfo`（OA 状态、DOAJ 收录、APC 定价）。
///
/// **设计原则**：
///
...
/// **使用场景**：
///
/// - PubMed 数据导入时，批量富化新建/更新 Venue 的引用指标和开放获取信息
...
public interface OpenAlexEnrichmentQueryPort {
  ...
  /// 批量查询 ISSN-L 对应的 OpenAlex 富化数据（引用指标 + 年度统计 + 开放获取信息）。
  ///
