...
///
/// - 不可变性：Record 自动提供
///   - 值验证：验证 MD5 哈希格式(32位十六进制)
///   - 可追溯：保留原始输入用于调试
///   - **纯领域对象**：不依赖任何框架,MD5 计算在应用层完成
///
/// **使用示例**：
///
/// ```java
...
/// ```
///
/// **注意**：MD5 哈希计算应在应用层(AuthorDedupService)完成,领域层只负责存储和验证。
///
...
public record DedupKey(String value, String rawInput) implements Serializable {
  ...
  public DedupKey {
    ...
    Assert.isTrue(value.matches("^[a-f0-9]{32}$"), "去重键格式无效,必须为32位MD5哈希：%s", value);
  }
