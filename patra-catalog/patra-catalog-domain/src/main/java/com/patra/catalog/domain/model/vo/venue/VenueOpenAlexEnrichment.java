...
/// OpenAlex 富化数据值对象。
///
/// 封装从 OpenAlex Sources API 查询获取的期刊引用指标和年度统计数据，
/// 作为批量查询的聚合返回结果。
///
/// **包含的数据**：
///
...
///
/// **null 语义**：`citationMetrics` 为 null 表示 OpenAlex 中无对应数据，
...
public record VenueOpenAlexEnrichment(
    String openAlexId, CitationMetrics citationMetrics, List<VenuePublicationStats> yearlyStats) {
  ...
  public static VenueOpenAlexEnrichment of(
      String openAlexId, CitationMetrics citationMetrics, List<VenuePublicationStats> yearlyStats) {
    return new VenueOpenAlexEnrichment(openAlexId, citationMetrics, yearlyStats);
  }
