...
///
/// - 不可变性：Record 自动提供
///   - 格式验证：ISSN必须符合 "XXXX-XXXX" 格式
///   - 类型安全：通过枚举确保类型正确性
///   - 关联管理：通过 linking ISSN 关联纸质版和电子版
///
/// ISSN 格式说明：
///
/// - 国际标准连续出版物号（ISSN），格式为 "XXXX-XXXX"（8位数字，中间带连字符）
///   - 示例：Print ISSN: "1234-5678", Electronic ISSN: "1234-567X"
///   - Linking ISSN: 用于关联同一期刊的不同介质版本
///
/// 使用示例：
///
/// ```java
...
/// @author linqibin
/// @since 0.1.0
public record IssnInfo(String issn, IssnType issnType, String issnLinking) implements Serializable {
  ...
  public IssnInfo {
    ...
    // ISSN格式验证
    Assert.isTrue(issn.matches(ISSN_PATTERN), "ISSN格式无效，必须符合 'XXXX-XXXX' 格式：%s", issn);

    // Linking ISSN格式验证（如果提供）
    if (StrUtil.isNotBlank(issnLinking)) {
      Assert.isTrue(
          issnLinking.matches(ISSN_PATTERN),
          "Linking ISSN格式无效，必须符合 'XXXX-XXXX' 格式：%s",
          issnLinking);
    }
  ...
  /// 验证ISSN校验位（最后一位）。
  ///
  /// ISSN校验算法：前7位数字乘以权重（8,7,6,5,4,3,2），求和，模11，用11减去余数得到校验位（如果是10则用X表示）。
  ///
  /// @return true 如果校验位正确
