...
/// 三层设计原理：
///
/// - **raw**：外部系统采集的原始值（如 "Chinese", "English"）
///   - **code**：应用层标准化的语言代码（如 "zh-CN", "en-US"，遵循 BCP 47）
///   - **base**：基础语种代码（如 "zh", "en"，用于分组和统计）
///
/// 设计优势：
///
/// - 保留原始数据可追溯性（raw）
...
public record LanguageInfo(String raw, String code, String base) implements Serializable {
  ...
  public LanguageInfo {
    ...
    // 验证 base 是 code 的前缀
    Assert.isTrue(code.startsWith(base), "基础语种代码（base:%s）必须是语言代码（code:%s）的前缀", base, code);

    // 验证 base 长度（通常为 2-3 位）
    Assert.isTrue(base.length() >= 2 && base.length() <= 3, "基础语种代码（base）长度必须在 2-3 位之间：%s", base);
  }
