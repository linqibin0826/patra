 ...
 * MeSH 文件下载接口（Port）。
 *
 * <p>定义 MeSH XML 文件的下载和校验操作，由 Infrastructure 层实现。
 *
 * <p><b>设计原则</b>：
 *
 * <ul>
 *   <li>纯接口定义：不包含实现逻辑
 *   <li>面向领域概念：方法名称和参数符合领域语言
 *   <li>六边形架构：领域层定义接口，基础设施层实现
 *   <li>异常处理：实现层负责处理网络异常和 IO 异常
 * </ul>
 *
 * <p><b>实现说明</b>：
 *
 * <ul>
 *   <li>Infrastructure 层使用 Apache HttpClient 或 OkHttp 实现下载
 *   <li>支持断点续传和重试机制
 *   <li>下载到临时目录，校验通过后移动到目标目录
 *   <li>使用 MD5 哈希验证文件完整性
 * </ul>
 *
 * <p><b>使用示例</b>：
...
public interface MeshFileDownloadPort {
   ...
   *
   * <ul>
   *   <li>从 NLM 服务器下载 XML 文件（约 700MB）
   *   <li>支持断点续传（使用 HTTP Range 请求）
   *   <li>失败时自动重试（最多 3 次）
   *   <li>下载到临时目录：{@code /tmp/mesh-import/desc2025.xml}
   *   <li>记录下载进度日志
   * </ul>
   *
   ...
   *   <li>计算文件的 MD5 哈希值
   *   <li>与预期哈希值比较（忽略大小写）
   *   <li>使用 DigestUtils 或 Hutool 工具计算 MD5
   * </ul>
   *
