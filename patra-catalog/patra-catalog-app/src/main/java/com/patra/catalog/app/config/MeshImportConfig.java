 ...
 * MeSH 数据导入配置属性。
 *
 * <p>配置前缀: {@code patra.catalog.mesh.import}
 *
 * <p>提供 MeSH 数据导入任务的核心配置,包括:
 * <ul>
 *   <li>数据源 URL 配置</li>
 *   <li>批次大小配置 (表级别可定制)</li>
 *   <li>下载超时和重试配置</li>
...
public class MeshImportConfig {
     ...
     * NLM 官方 MeSH 数据源 URL。
     *
     * <p>默认值: https://nlmpubs.nlm.nih.gov/projects/mesh/MESH_FILES/xmlmesh/desc2025.xml
     *
     * <p>说明: 支持通过 Nacos Config 动态修改,无需重启应用
     */
     ...
     * </pre>
     *
     * <p>说明: 根据表的数据量和复杂度调整批次大小,优化导入性能
     */
    private Map<String, Integer> batchSizeMap = Map.of(
     ...
     * <p>默认值: 10 分钟 (600秒)
     *
     * <p>说明: MeSH XML 文件约 300MB,根据网络条件调整
     */
     ...
     * </pre>
     *
     * <p>说明: 导入完成后验证实际数量与预期的差异,超过 5% 生成警告
     */
    private Map<String, Integer> expectedCounts = Map.of(
        "descriptor", 35000,
        "qualifier", 80,
        "tree-number", 80000,
        "entry-term", 250000,
     ...
     * <p>查找顺序:
     * <ol>
     *   <li>优先使用 batchSizeMap 中表的特定配置</li>
     *   <li>找不到时使用 defaultBatchSize</li>
     * </ol>
     *
