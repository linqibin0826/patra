...
--   3. cat_mesh_scr_concept (SCR 概念表) - 依赖 cat_mesh_scr
--   4. cat_mesh_scr_term (SCR 术语表) - 依赖 cat_mesh_scr, cat_mesh_scr_concept
--   5. cat_mesh_scr_source (SCR 来源表) - 依赖 cat_mesh_scr
--   6. cat_mesh_scr_indexing_info (SCR 索引信息表) - 依赖 cat_mesh_scr, cat_mesh_descriptor, cat_mesh_qualifier
--   7. cat_mesh_scr_pharmacological_action (SCR 药理作用表) - 依赖 cat_mesh_scr, cat_mesh_descriptor
--
-- 导入顺序要求:
...
-- ============================================================
-- 表 3: cat_mesh_scr_concept (SCR 概念表)
-- ============================================================
-- 表说明: 存储 SCR 下的概念,结构与 Descriptor Concept 相同(DTD 共享定义)
-- 记录数预估: 初始 35万 / 年增长 2万 / 5年规模 45万 (每个SCR平均1.1个概念)
-- 主要查询场景:
--   1. 按 scr_ui 查询某 SCR 的所有概念(>300次/天,中频)
--   2. 按 concept_ui 精确查询(>200次/天,中频)
--   3. 按 registry_number 查询化学物质(<100次/天,低频)
-- ============================================================

CREATE TABLE IF NOT EXISTS `cat_mesh_scr_concept` (
    -- ========================================
    -- 业务字段
    -- ========================================
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键,雪花算法生成',
    `scr_ui` VARCHAR(10) NOT NULL COMMENT 'SCR UI(关联:cat_mesh_scr.ui,格式:C000001)',
    `concept_ui` VARCHAR(10) NOT NULL COMMENT '概念唯一标识符(格式:M0000001-M9999999)',
    `concept_name` VARCHAR(500) NOT NULL COMMENT '概念名称(化学物质名称可能很长)',
    ...
    -- 唯一索引
    UNIQUE INDEX `uk_concept_ui` (`concept_ui`) COMMENT '概念 UI 唯一索引,全局唯一',

    -- 普通索引
    INDEX `idx_scr_ui` (`scr_ui`) COMMENT 'SCR UI索引,支持查询某SCR的所有概念',
    INDEX `idx_preferred` (`scr_ui`, `is_preferred`) COMMENT 'SCR+首选概念复合索引,快速定位首选概念'

) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='SCR 概念表:存储 SCR 下的概念,结构与 Descriptor Concept 相同';


-- ============================================================
-- 表 4: cat_mesh_scr_term (SCR 术语表)
-- ============================================================
-- 表说明: 存储 SCR 概念下的术语(同义词、入口术语),结构与 Descriptor Term 相同
-- 记录数预估: 初始 60万 / 年增长 4万 / 5年规模 80万 (每个概念平均1.7个术语)
-- 主要查询场景:
--   1. 按 scr_ui 查询某 SCR 的所有术语(>200次/天,中频)
--   2. 按 concept_ui 查询某概念的术语(<100次/天,低频)
--   3. 全文检索术语(>300次/天,中频)
-- ============================================================

CREATE TABLE IF NOT EXISTS `cat_mesh_scr_term` (
    -- ========================================
    -- 业务字段
    -- ========================================
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键,雪花算法生成',
    `scr_ui` VARCHAR(10) NOT NULL COMMENT 'SCR UI(关联:cat_mesh_scr.ui,格式:C000001)',
    `concept_ui` VARCHAR(10) NOT NULL COMMENT '所属概念UI(关联:cat_mesh_scr_concept.concept_ui)',
    `term_ui` VARCHAR(12) NULL DEFAULT NULL COMMENT '术语UI(格式:T000001-T999999,可能为空)',
    `term` VARCHAR(500) NOT NULL COMMENT '术语文本(化学物质名称可能很长)',
    `lexical_tag` VARCHAR(10) NULL DEFAULT NULL COMMENT '词法标记(ABB/ABX/ACR/ACX/EPO/LAB/NAM/NON/TRD/Frelex/HIST)',
    `is_record_preferred` BOOLEAN NOT NULL DEFAULT 0 COMMENT '是否记录首选术语(0=否,1=是)',
    `is_concept_preferred` BOOLEAN NOT NULL DEFAULT 0 COMMENT '是否概念首选术语(0=否,1=是)',
    ...
    `date_created` DATE NULL DEFAULT NULL COMMENT '术语创建日期',
    ...
    -- 普通索引
    INDEX `idx_scr_ui` (`scr_ui`) COMMENT 'SCR UI索引,支持查询某SCR的所有术语',
    INDEX `idx_concept_ui` (`concept_ui`) COMMENT '概念UI索引,支持查询某概念的术语',
    INDEX `idx_term_ui` (`term_ui`) COMMENT '术语UI索引,支持精确查询'

) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='SCR 术语表:存储 SCR 概念下的术语(同义词、入口术语)';


-- ============================================================
-- 表 5: cat_mesh_scr_source (SCR 来源表)
-- ============================================================
...
-- ============================================================
-- 表 6: cat_mesh_scr_indexing_info (SCR 索引信息表)
-- ============================================================
...
-- ============================================================
-- 表 7: cat_mesh_scr_pharmacological_action (SCR 药理作用表)
-- ============================================================
...
-- cat_mesh_scr_term 表的全文索引
CREATE FULLTEXT INDEX `ft_term` ON `cat_mesh_scr_term` (`term`)
    WITH PARSER ngram
    COMMENT '术语全文索引,支持同义词模糊检索';
