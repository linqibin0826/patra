...
CREATE TABLE IF NOT EXISTS `cat_mesh_import_task` (
    ...
    `descriptor_source_url` VARCHAR(500) NOT NULL COMMENT 'NLM 主题词数据源 URL (desc{year}.xml)',
    `qualifier_source_url` VARCHAR(500) NOT NULL COMMENT 'NLM 限定词数据源 URL (qual{year}.xml)',
...
CREATE TABLE IF NOT EXISTS `cat_mesh_table_progress` (
    ...
    `total_count` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '总记录数（预期值，导入前从配置或XML元数据获取）',
    `actual_total_count` INT UNSIGNED NULL DEFAULT NULL COMMENT '实际总记录数（导入完成后设置，用于数据差异分析）',
