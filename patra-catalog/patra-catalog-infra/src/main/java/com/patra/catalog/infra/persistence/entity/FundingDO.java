...
/**
 * 资助信息数据库实体,映射到表 {@code cat_funding}。
 *
 * <p>表结构: 管理研究资金来源和项目信息,通过去重策略避免重复存储。
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code dedup_key} 去重键(MD5 哈希,基于 agency_name + grant_id),唯一约束 uk_dedup_key
 *   <li>{@code funder_id} Crossref Funder Registry ID,支持标准化查询
 *   <li>{@code ror_id} ROR(Research Organization Registry)标识符
 *   <li>{@code amount} 资助金额(DECIMAL 20,2),映射为 BigDecimal
 *   <li>{@code start_date} 和 {@code end_date} 开始/结束日期(DATE),映射为 Instant（UTC午夜）
 *   <li>{@code funding_type} 资助类型,CHECK 约束 6 个枚举值
