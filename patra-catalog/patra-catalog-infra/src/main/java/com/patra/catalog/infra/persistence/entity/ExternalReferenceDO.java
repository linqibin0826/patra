...
/**
 * 外部引用数据库实体,映射到表 {@code cat_external_reference}。
 *
 * <p>表结构: 管理外部数据库引用(基因库、临床试验、数据集等),与参考文献分离。
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code publication_id} 出版物 ID(外键:cat_publication.id)
 *   <li>{@code database_name} 数据库名称(如"GenBank","ClinicalTrials.gov")
 *   <li>{@code accession_number} 登录号/访问号
 *   <li>{@code access_date} 访问日期(最后验证日期)(DATE),映射为 Instant（UTC午夜）
 *   <li>{@code metadata} JSON 扩展数据字段
 * </ul>
 *
 * <p>唯一约束: uk_external_ref(publication_id, database_name, accession_number),防止重复引用。
 *
 *
 * @author linqibin
 * @since 0.4.0
