...
/**
 * 开放获取位置数据库实体,映射到表 {@code cat_oa_location}。
 *
 * <p>表结构: 详细记录文献的开放获取位置,支持多位置管理和最佳位置选择
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code publication_id} 出版物ID(外键:cat_publication.id)
 *   <li>{@code oa_status} OA状态(gold/green/hybrid/bronze/closed)
 *   <li>{@code url} 访问URL,唯一约束 uk_oa_url (publication_id + url)
 *   <li>{@code is_best} 是否最佳位置
 *   <li>{@code priority} 优先级(1=最高,数值越小优先级越高)
 *   <li>{@code available_date} 可用日期
 *   <li>{@code access_metrics} 访问指标(下载次数等)
 *   <li>{@code metadata} 位置元数据(灵活扩展)
 * </ul>
 *
 *
 * @author linqibin
