...
/**
 * 其他语言摘要数据库实体,映射到表 {@code cat_alternative_abstract}。
 *
 * <p>表结构: 管理文献摘要的多语言版本(官方翻译、专业翻译、机器翻译)
 *
 * <p>关键字段说明:
 *
 * <ul>
 *   <li>{@code publication_id} 出版物ID(外键:cat_publication.id)
 *   <li>{@code abstract_id} 主摘要ID(外键:cat_abstract.id,关联原摘要)
 *   <li>{@code language_code} 语言代码(ISO 639-1),唯一约束 uk_abstract_lang
 *   <li>{@code plain_text} 纯文本摘要(TEXT 类型,最大65535字符)
 *   <li>{@code structured_sections} 结构化摘要段落(JSON对象)
 *   <li>{@code is_official} 是否官方翻译
 *   <li>{@code metadata} 翻译元数据(灵活扩展)
 * </ul>
 *
 * @author linqibin
 * @since 0.5.0
