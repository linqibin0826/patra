 ...
 * MeSH 批次详情仓储实现。
 *
 * <p>实现 {@link MeshBatchDetailPort} 接口，提供批次详情查询能力。
 *
 * <p><b>实现策略</b>：
 *
 * <ul>
 *   <li>使用 MyBatis-Plus LambdaQueryWrapper 进行简单查询
 *   <li>只读仓储：不提供保存方法，批次详情由导入流程创建
 *   <li>查询优化：使用索引（idx_batch_detail_import_id、idx_batch_detail_status）
 *   <li>DO到值对象转换：查询后转换为Domain层的FailedBatch值对象
 * </ul>
 *
 * <p><b>技术实现</b>：
 *
 * <ul>
 *   <li>findFailedBatches()：WHERE import_id = ? AND status = 'FAILED'
 *   <li>countByStatus()：SELECT COUNT(*) WHERE import_id = ? AND status = ?
 * </ul>
 *
 * @author linqibin
 * @since 0.2.0
...
public class MeshBatchDetailRepositoryImpl implements MeshBatchDetailPort {
  ...
  public Long countByStatus(MeshImportId importId, MeshBatchStatus status) {
    ...
    log.debug(
        "统计结果：{} 个批次，任务ID：{}，状态：{}", count, importId.value(), status.getDisplayName());
