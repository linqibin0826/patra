 ...
 *   <li>{@code batch_size} - 批次大小（实际处理的记录数）
 *   <li>{@code status} - 批次状态（PENDING/PROCESSING/SUCCESS/FAILED）
 *   <li>{@code retry_count} - 重试次数（最多 3 次）
 *   <li>{@code error_message} - 错误信息（失败时记录）
 * </ul>
 *
 * <p><b>设计原则</b>：
 *
 ...
 *   <li>细粒度追踪：每个批次单独记录，便于定位问题
 *   <li>重试机制：记录重试次数，防止无限重试
 *   <li>性能监控：记录批次开始和结束时间，便于性能分析
 * </ul>
 *
 * <p><b>使用场景</b>：
 *
 * <ul>
 *   <li>失败重试：查询 status = 'FAILED' 且 retry_count < 3 的批次
 *   <li>错误分析：通过 error_message 分析失败原因
 *   <li>性能分析：计算批次处理时间（end_time - start_time）
 *   <li>进度恢复：查询最大 batch_num，从下一批次继续
 * </ul>
 *
 * @author linqibin
 * @since 0.2.0
