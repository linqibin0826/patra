...
/**
 * 文献-MeSH 关联表数据库实体，映射到表 {@code cat_publication_mesh}。
 *
 * <p>表结构：存储文献的 MeSH 标引，关联文献、主题词、限定词，支持主/副主题标记。
 *
 * <p>关键字段说明：
 *
 * <ul>
 ...
 *   <li>{@code qualifier_id} - 限定词 ID（外键：cat_mesh_qualifier.id，可选）
 *   <li>{@code is_major_topic} - 是否主要主题（0=副主题，1=主要主题，对应 MeSH 星号 *）
 *   <li>{@code order_num} - 顺序号（在同一文献内的排序）
 *   <li>{@code indexing_method} - 标引方法（如 Manual/Automatic）
 * </ul>
 *
 * <p>索引说明：
 *
 * <ul>
 *   <li>idx_pub_desc - 文献+主题词复合索引，支持查询文献的 MeSH（&lt;20ms）
 *   <li>idx_desc_pub - 主题词+文献复合索引，支持查询 MeSH 的文献（&lt;50ms）
 *   <li>idx_major_topic - 主题词+主/副主题复合索引，筛选主要主题文献
 *   <li>idx_qualifier - 限定词索引，支持按限定词筛选文献
 * </ul>
 *
 *
 * @author linqibin
