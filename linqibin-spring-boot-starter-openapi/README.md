...
| 配置类 | 功能 | 条件 |
|--------|------|------|
| `OpenApiAutoConfiguration` | 创建默认 `OpenAPI` bean，配置文档标题、版本、描述 | Servlet Web 应用，`patra.openapi.enabled=true`（默认启用） |
...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `patra.openapi.enabled` | boolean | true | 全局开关 |
| `patra.openapi.title` | String | null（自动推导） | API 文档标题 |
| `patra.openapi.version` | String | 0.1.0 | API 版本号 |
| `patra.openapi.description` | String | null | API 文档描述 |
...
└── config/
    └── OpenApiProperties          # 配置属性（patra.openapi.*）
```
