...
因此**改 compose / .env 不能只在本机改**，要让 Mac mini 上的仓库副本 `git pull` 后重启容器。日常迭代命令见 README "日常迭代"节，本质是 `ssh linqibin@linqibins-mac-mini 'cd ~/Projects/patra-api && git pull && docker compose -f patra-infra/docker/docker-compose.dev.yaml up -d --remove-orphans'`。
