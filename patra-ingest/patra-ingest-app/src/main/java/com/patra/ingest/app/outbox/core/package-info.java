/// Outbox 核心抽象和发布逻辑包。
/// 
/// 本包提供 Outbox 发布器的抽象基类和核心模型，定义通用发布流程和扩展点。
/// 
/// ## 职责
/// 
/// - 定义 Outbox 发布器的模板方法（Template Method Pattern）
///   - 提供批量发布的通用逻辑
///   - 管理 Outbox 消息的生命周期（PENDING → PUBLISHED）
///   - 定义发布上下文和发布结果模型
/// 
/// ## 核心组件
/// 
/// - `AbstractOutboxPublisher<E, P, H>` - Outbox 发布器抽象基类
///       
/// - **E**: 领域事件类型（如 `TaskQueuedEvent`）
///         - **P**: 负载类型（如 `TaskPayload`，继承 `OutboxPayload`）
///         - **H**: 消息头类型（如 `TaskHeaders`，继承 `OutboxHeaders`）
/// 
///   - `OutboxPublishContext` - 发布上下文
///       
/// - 包含批次信息、租约信息、超时配置
/// 
///   - `OutboxPublishResult` - 发布结果
///       
/// - 包含成功数、失败数、错误信息
/// 
/// ## 模板方法流程
...
/// ## 设计模式
/// 
/// - **模板方法模式**: `AbstractOutboxPublisher` 定义发布流程骨架
///   - **策略模式**: 子类定义具体的分区策略、幂等策略
///   - **泛型设计**: 支持不同事件、负载、消息头类型
/// 
/// @since 0.1.0
/// @author linqibin
