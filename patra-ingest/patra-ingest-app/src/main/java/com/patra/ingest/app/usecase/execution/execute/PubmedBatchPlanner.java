...
public class PubmedBatchPlanner implements BatchPlanner {
    ...
    public BatchPlan plan(ExecutionContext ctx) {
        ...
        if (total <= 0) {
            log.info("[INGEST][APP] pubmed planner: no results termHash={} pageSize={}", safeHash(term), pageSize);
            return BatchPlan.empty();
        }

        int pagesNeeded = (int) Math.ceil(total / (double) pageSize);
        if (pagesNeeded > maxPages) {
            log.warn("[INGEST][APP] pubmed planner fail-fast: pagesNeeded={} > maxPages={} termHash={} pageSize={} total={}",
                    pagesNeeded, maxPages, safeHash(term), pageSize, total);
        ...
        log.info("[INGEST][APP] pubmed planner: planned {} batches termHash={} pageSize={} total={}",
                pages, safeHash(term), pageSize, total);
    ...
    private int resolvePageSize(ObjectNode params, ProvenanceConfigSnapshot snapshot) {
        ...
        Integer fromCfg = snapshot != null && snapshot.pagination() != null
                ? snapshot.pagination().pageSizeValue() : null;

        int pageSize = fromParams != null ? fromParams : (fromCfg != null ? fromCfg : DEFAULT_PAGE_SIZE);
        if (pageSize <= 0) pageSize = DEFAULT_PAGE_SIZE;
        if (pageSize > PUBMED_RETMAX_LIMIT) {
            log.warn("[INGEST][APP] pubmed planner: retmax clamped to {} from {}", PUBMED_RETMAX_LIMIT, pageSize);
    ...
    private int resolveMaxPages(ProvenanceConfigSnapshot snapshot) {
        Integer max = snapshot != null && snapshot.pagination() != null
                ? snapshot.pagination().maxPagesPerExecution() : null;
    ...
    private static Integer intOrNull(ObjectNode node, String field) {
        ...
        if (v.isTextual()) {
            try { return Integer.parseInt(v.asText()); } catch (NumberFormatException ignored) { return null; }
        }
        return null;
    }
