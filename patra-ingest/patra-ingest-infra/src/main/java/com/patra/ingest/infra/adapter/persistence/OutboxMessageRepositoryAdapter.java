...
import com.patra.ingest.infra.adapter.persistence.converter.mapper.OutboxMessageJpaMapper;
...
/// ### 职责
///
/// - 初始消息写入（PENDING 状态），通过 (channel, dedupKey) 唯一性保证幂等性
/// - 按通道和可用时间窗口获取可发布消息（无锁，过滤条件：status=PENDING 且 availableAt <= now）
/// - 通过乐观锁 + 租约字段（leaseOwner/leaseExpireAt/version）竞争分布式"发布权"
/// - 根据发布结果推进状态：PUBLISHED / DEFERRED（重试回到 PENDING）/ FAILED（终态）
///
/// ### 状态机（简化版）
///
/// ```
...
/// ### 并发控制
///
/// - 版本号通过条件更新防止并发覆盖；acquireLease/mark* 方法使用 version 作为乐观锁
/// - 租约由两个元素组成：leaseOwner/leaseExpireAt；一旦获取，消息对其他消费者不可见直到过期
/// - 如果发布进程崩溃或超时，过期租约允许新消费者接管
///
/// ### 幂等性
///
/// - 上游在 Outbox 插入时通过 (channel, dedupKey) 确保无重复写入；本仓储提供查询支持
/// - 成功发布后，markPublished 仅在 version 匹配时生效；重复调用（version 已变）触发异常供上游幂等确认
///
/// ### 错误处理策略
///
/// - 受影响行数 != 1 表示版本冲突或记录缺失：抛出 {@link OutboxPersistenceException} 供上游决策（忽略/告警）
/// - 高频路径无 INFO 日志；仅关键状态转换使用 DEBUG
///
/// ### 日志策略
///
/// DEBUG 级别记录关键状态转换（获取成功、发布/延迟/失败），避免批处理循环中的噪音；无业务 WARN 输出。
///
/// ### 线程安全
///
/// 无共享可变状态（Dao/JpaMapper 无状态或线程安全）；实例可跨线程重用。
///
/// @author linqibin
/// @since 0.1.0
@Slf4j
...
public class OutboxMessageRepositoryAdapter
    implements OutboxMessageRepository, OutboxRelayRepository {

  /// OutboxMessage JPA Repository
  private final OutboxMessageDao outboxMessageDao;

  /// 领域实体与 JPA 实体转换器
  private final OutboxMessageJpaMapper outboxMessageJpaMapper;

  /// 批量保存（仅插入）PENDING 状态的发件箱消息。
  ///
  /// 不执行去重：调用方必须在插入前处理 (channel, dedupKey) 幂等性。
  ///
  /// 日志记录：DEBUG 级别记录批次大小以减少噪音；无单条消息日志。
  ///
  ...
  public void saveAll(List<OutboxMessage> messages) {
    ...
    List<OutboxMessageEntity> entities = new ArrayList<>(messages.size());
    for (OutboxMessage message : messages) {
      OutboxMessageEntity entity = outboxMessageJpaMapper.toEntity(message);
      entity.setId(SnowflakeIdGenerator.getId());
      entities.add(entity);
    }
    outboxMessageDao.saveAll(entities);
  }

  /// 插入或更新单个发件箱消息。
  ///
  /// 使用场景：补偿写入或更新非状态字段（罕见）。对于常规状态转换，使用专用 mark* 方法以确保版本语义。
  ///
  ...
  public Optional<OutboxMessage> findByChannelAndDedup(String channel, String dedupKey) {
    return outboxMessageDao
        .findByChannelAndDedupKey(channel, dedupKey)
        .map(outboxMessageJpaMapper::toAggregate);
  }
  ...
  /// 支持通道过滤或从所有通道获取：
  ///
  /// - 当 channel 非 null 时，仅从指定通道获取消息
  /// - 当 channel 为 null 时，从所有通道获取消息
  ///
  /// 过滤条件：state=PENDING AND available_at <= :availableTime AND (lease_owner IS NULL OR
  /// lease_expire_at < NOW)。
  ///
  /// 此处不强制排序保证；由 Dao 层定义（推荐：available_at, id ASC）。
  ///
  ...
  /// 通过乐观锁获取租约。
  ///
  /// 条件：id = ? AND version = :expectedVersion AND (lease_owner IS NULL OR lease_expire_at <
  /// NOW)。
  ///
  /// 成功：更新 leaseOwner/leaseExpireAt/version=version+1，返回 true；失败：返回 false（可能被其他人获取或版本冲突）。
  ///
  /// 日志记录：仅在成功时记录 DEBUG；失败是正常竞争，不记录。
  ///
  /// @param id 消息 ID
  ...
  public boolean acquireLease(
    ...
    int affectedRows =
        outboxMessageDao.acquireLease(id, expectedVersion, leaseOwner, leaseExpireAt);
  ...
  /// 标记消息为已成功发布。
  ///
  /// 要求当前版本 == expectedVersion；更新字段：state=PUBLISHED, published_at, version=version+1。
  ///
  /// @param id 消息 ID
  ...
  public void markPublished(Long id, Long expectedVersion) {
    int affectedRows = outboxMessageDao.markPublished(id, expectedVersion, Instant.now());
    if (affectedRows != 1) {
  ...
  public void markDeferred(
      ...
      String errorMessage) {
    int affectedRows =
        outboxMessageDao.markDeferred(
            id, expectedVersion, retryCount, nextRetryAt, errorCode, errorMessage);
    if (affectedRows != 1) {
      throw new OutboxPersistenceException(
  ...
  ///
  /// 典型场景：重试将现有消息状态重置为 PENDING，更新 payload/headers，重置重试计数。
  ///
  /// 注意：使用 JPA save 逐条更新；适用于小批量（<100 条消息）。
  ///
  ...
  ///
  /// 通过唯一约束（channel + dedupKey）实现幂等性：
  ///
  /// - 如果消息不存在，插入新记录
  /// - 如果消息存在（dedupKey 冲突），更新 payload/headers/status 并重置 retryCount
  ///
  /// 解决 publishRetry 并发竞争条件（两个实例同时重试同一消息）。
  ///
  ...
  public void upsertBatch(List<OutboxMessage> messages) {
    ...
    // JPA 没有原生 UPSERT，使用先查后插入/更新的方式
    for (OutboxMessage message : messages) {
      Optional<OutboxMessageEntity> existing =
          outboxMessageDao.findByChannelAndDedupKey(message.getChannel(), message.getDedupKey());

      OutboxMessageEntity entity = outboxMessageJpaMapper.toEntity(message);
      if (existing.isPresent()) {
        // 更新现有记录
        entity.setId(existing.get().getId());
        entity.setVersion(existing.get().getVersion());
      } else {
        // 插入新记录
        entity.setId(SnowflakeIdGenerator.getId());
      }
    ...
    if (log.isDebugEnabled()) {
      log.debug("Outbox upsert batch completed size={}", messages.size());
    }
  }
}
