...
import com.patra.ingest.infra.adapter.persistence.converter.mapper.OutboxRelayLogJpaMapper;
...
/// ### 职责
///
/// - 持久化中继执行日志（单条或批量）到数据库
/// - 按消息、批次、通道、状态或时间范围查询日志
/// - 支持故障排查、监控和分析用例
///
/// ### 设计原则
///
/// - **仅追加**：日志创建后永不更新（不可变审计轨迹）
/// - **批量优化**：批量插入使用 JPA saveAll 以提高性能
/// - **查询优化**：方法利用数据库索引实现高效检索
/// - **无业务逻辑**：纯数据访问层，委托给领域层处理规则
///
/// ### 性能考虑
///
/// - 批量插入：使用 {@link #saveBatch(List)} 处理 100-500 条日志
/// - 索引覆盖：所有查询方法使用适当索引（参见 Dao JavaDoc）
/// - 分页：查询方法接受 limit 参数以防止大结果集
///
/// ### 日志策略
///
/// - DEBUG：批量插入操作（大小和受影响行数）
/// - DEBUG：查询操作（方法名和结果计数）
/// - 高频操作无 INFO 日志（避免日志噪音）
///
/// ### 线程安全
///
...
public class OutboxRelayLogRepositoryAdapter implements OutboxRelayLogRepository {

  /// OutboxRelayLog JPA Repository
  private final OutboxRelayLogDao outboxRelayLogDao;

  /// 领域实体与 JPA 实体转换器
  private final OutboxRelayLogJpaMapper outboxRelayLogJpaMapper;

  /// 持久化单条中继日志到数据库。
  ///
  /// 使用场景：单次中继执行（罕见，通常使用批量插入）。
  ///
  /// 性能注意：对于多条日志，优先使用 {@link #saveBatch(List)} 以减少数据库往返。
  ///
  ...
  public void save(OutboxRelayLog relayLog) {
    ...
    OutboxRelayLogEntity entity = outboxRelayLogJpaMapper.toEntity(relayLog);
    entity.setId(SnowflakeIdGenerator.getId());
    outboxRelayLogDao.save(entity);
  }

  /// 在单条 SQL 语句中批量持久化多条中继日志。
  ///
  /// 使用场景：中继作业完成 100-500 条消息，一次性插入所有日志。
  ///
  /// 性能：使用 JPA saveAll 批量插入。
  ///
  /// 推荐批次大小：100-500 条日志（超过 500 可能触及 SQL 长度限制）。
  ///
  ...
  public void saveBatch(List<OutboxRelayLog> logs) {
    ...
    List<OutboxRelayLogEntity> entities = new ArrayList<>(logs.size());
    for (OutboxRelayLog relayLog : logs) {
      OutboxRelayLogEntity entity = outboxRelayLogJpaMapper.toEntity(relayLog);
      entity.setId(SnowflakeIdGenerator.getId());
      entities.add(entity);
    }
    outboxRelayLogDao.saveAll(entities);
  ...
  ///
  /// 使用的索引：`idx_message_id(message_id, started_at DESC)`。
  ///
  ...
  public List<OutboxRelayLog> findByOutboxMessageId(Long messageId) {
    ...
    List<OutboxRelayLogEntity> entities =
        outboxRelayLogDao.findByMessageIdOrderByStartedAtDesc(messageId);
    List<OutboxRelayLog> logs =
        entities.stream().map(outboxRelayLogJpaMapper::toAggregate).toList();
  ...
  ///
  /// 使用的索引：`idx_batch_id(relay_batch_id)`。
  ///
  ...
  public List<OutboxRelayLog> findByBatchId(String batchId) {
    ...
    List<OutboxRelayLogEntity> entities =
        outboxRelayLogDao.findByRelayBatchIdOrderByStartedAtAsc(batchId);
    List<OutboxRelayLog> logs =
        entities.stream().map(outboxRelayLogJpaMapper::toAggregate).toList();
  ...
  /// 使用场景：
  ///
  /// - 监控仪表板："最近 1 小时内 INGEST 通道的成功率"
  /// - 告警查询："最近 10 分钟内有多少 FAILED 中继?"
  ///
  /// 使用的索引：`idx_channel_time` 或 `idx_status`。
  ///
  ...
  public long countByChannelAndStatus(
    ...
    long count = outboxRelayLogDao.countByChannelAndStatus(channel, status, startTime, endTime);
  ...
  /// 查询最近失败的中继日志（用于告警）。
  ///
  /// 使用场景：值班分诊 - "显示最近 10 次失败"。
  ///
  /// 使用的索引：`idx_status(relay_status='FAILED', started_at DESC)`。
  ///
  /// @param channel 通道名称过滤器（null = 所有通道）
  ...
  public List<OutboxRelayLog> findRecentFailed(String channel, int limit) {
    ...
    List<OutboxRelayLogEntity> entities =
        outboxRelayLogDao.findRecentFailed(channel, PageRequest.of(0, limit));
    List<OutboxRelayLog> logs =
        entities.stream().map(outboxRelayLogJpaMapper::toAggregate).toList();
  ...
  /// 查询时间范围内某通道的中继日志。
  ///
  /// 使用场景：历史审查 - "显示今天 INGEST 通道的所有中继尝试"。
  ///
  /// 使用的索引：`idx_channel_time(channel, started_at)`。
  ///
  /// @param channel 通道名称过滤器（null = 所有通道）
  /// @param startTime 时间范围起始（包含）
  /// @param endTime 时间范围结束（不包含）
  ...
  public List<OutboxRelayLog> findByChannelAndTimeRange(
    ...
    List<OutboxRelayLogEntity> entities =
        outboxRelayLogDao.findByChannelAndTimeRange(
            channel, startTime, endTime, PageRequest.of(0, limit));
    List<OutboxRelayLog> logs =
        entities.stream().map(outboxRelayLogJpaMapper::toAggregate).toList();
