package dev.linqibin.patra.ingest.domain.model.vo.query;
...
/// 查询会话 - Ingest 领域模型
///
/// 封装从外部数据源获取的查询会话信息，用于批次生成规划。
///
/// **职责**：
///
/// - 提供总记录数，用于计算批次数量
///   - 提供数据源标识，用于策略路由
///   - 提供状态令牌（opaque），用于跨批次传递上下文
///
/// **设计原则**：
///
/// - 与外部数据源实现解耦
///   - 不包含框架或技术细节
///   - 状态令牌为 opaque 类型，Ingest 不解析其内容
///
/// **使用场景**：
///
/// - ProvenanceDataPort.prepareQuerySession() 返回此接口
...
public interface QuerySession {
  ...
  /// 检查是否包含状态令牌
  ///
  /// 某些数据源（如 PubMed）在计划阶段会返回状态令牌（如 WebEnv）， 执行阶段可以使用该令牌避免重复查询。
  ///
  ...
  /// 获取状态令牌
  ///
  /// 状态令牌是 opaque 的，Ingest 不解析其内容，只负责在批次间传递。 具体的令牌格式由数据源定义（如 PubMed 的 webEnv + queryKey）。
  ///
