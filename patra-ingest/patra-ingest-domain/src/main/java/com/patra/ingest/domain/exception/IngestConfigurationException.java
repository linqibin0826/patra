 ...
 *
 * <p>Trigger: raised when provenance or operation metadata cannot be loaded from Registry/config
 * center, or the payload violates schema constraints. Compared with {@link
 * PlanValidationException}, this focuses on platform configuration defects rather than runtime
 * parameters.
 *
 * <p>Handling guidance:
 *
 * <ul>
 *   <li>Temporary fetch failures (network/timeouts): retry a limited number of times at the
 *       application layer.
 *   <li>Missing configuration: log at ERROR level, raise alerts, and request the data to be
 *       populated.
 *   <li>Malformed configuration: stop execution and describe the offending field path and expected
 *       format.
 * </ul>
 *
 * <p>Observability: include {@code provenanceCode} / {@code endpointName} in logs to aid
 * aggregation.
 *
 * @author linqibin
 * @since 0.1.0
...
public class IngestConfigurationException extends IngestException implements HasErrorTraits {
   ...
   *
   * <p>Use when the configuration defect is detected immediately.
   *
   ...
   *
   * <p>Use to wrap remote call failures, JSON parsing problems, or mapping conversions.
   *
   * @param provenanceCode provenance code
   * @param operationCode operation code
