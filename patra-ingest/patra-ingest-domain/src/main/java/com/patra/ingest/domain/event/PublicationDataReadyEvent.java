 ...
 *
 * <p>该事件通过 Outbox 持久化并中继到下游服务。负载仅包含元数据;实际的出版物文档存储在外部(例如,对象存储)并通过 {@link #storageKeys()}
 * 引用。
 *
 * <p>触发条件:任务成功完成出版物数据采集并上传到存储系统后触发。
 *
 * <p>用途:
 *
 * <ul>
 *   <li>下游消费:通知下游服务出版物数据已就绪,可进行后续处理
 *   <li>审计跟踪:记录数据采集的完成情况和存储位置
 *   <li>监控指标:统计出版物采集成功率和批次处理情况
 * </ul>
 */
@Builder
