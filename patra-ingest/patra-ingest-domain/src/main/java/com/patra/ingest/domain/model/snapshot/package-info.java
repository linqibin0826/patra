 ...
 * Ingest 领域模型 - 快照包。
 *
 * <p>本包包含领域对象的快照(Snapshot)实现,用于捕获特定时间点的配置或状态,确保数据一致性和可回溯性。
 * 快照模式在分布式系统中广泛应用于配置版本管理、事件溯源和断点续传。
 *
 * <h2>核心快照</h2>
 *
 * <ul>
 ...
 *       <ul>
 *         <li>捕获 patra-registry 服务返回的 Provenance 配置在特定时间点的完整状态</li>
 *         <li>保证任务执行过程中使用一致的配置,即使 registry 配置变更</li>
 *         <li>包含配置内容的 JSON 表示和 SHA-256 哈希值</li>
 *         <li>支持配置变更检测和版本管理</li>
 *       </ul>
 *   </li>
 * </ul>
 ...
 * <h2>快照模式动机</h2>
 *
 * <p><b>问题场景</b>: 在长时间运行的数据采集任务中,外部配置(如 Provenance 的 API 端点、认证密钥)
 * 可能在任务执行过程中被修改,导致以下问题:
 *
 * <ul>
 *   <li>任务执行不一致:同一计划的不同任务使用不同配置</li>
 *   <li>断点续传失败:恢复执行时配置已变更,无法继续</li>
 *   <li>审计追溯困难:无法确定历史任务使用了哪个版本的配置</li>
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>不可变性</b>: 快照创建后不可修改,保证历史数据完整性</li>
 *   <li><b>自包含性</b>: 快照包含恢复对象所需的全部信息</li>
 *   <li><b>版本标识</b>: 通过哈希值或版本号标识快照唯一性</li>
 *   <li><b>序列化友好</b>: 快照应易于序列化和反序列化(通常为 JSON)</li>
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>聚合快照</b>: 定期保存聚合根的完整状态,避免重放全部事件</li>
 *   <li><b>配置快照</b>: 记录外部依赖的历史版本,支持重现历史执行环境</li>
 *   <li><b>断点快照</b>: 捕获任务执行的中间状态,支持断点续传</li>
 * </ul>
 *
 * <h2>性能考虑</h2>
 *
 * <ul>
 *   <li><b>序列化开销</b>: JSON 序列化可能较慢,考虑使用 Protobuf 或 Avro</li>
 *   <li><b>存储空间</b>: 快照包含完整配置,需合理设置保留策略</li>
 *   <li><b>哈希计算</b>: SHA-256 计算成本可接受,避免使用 MD5(不安全)</li>
 * </ul>
 *
 * <h2>最佳实践</h2>
 *
 * <ul>
 *   <li>快照应包含时间戳,标识捕获时间</li>
 *   <li>使用强哈希算法(SHA-256)而非弱哈希(MD5)</li>
 *   <li>快照不可变,避免提供 setter 方法</li>
 *   <li>反序列化失败应有优雅降级机制</li>
 *   <li>定期清理过期快照,避免存储膨胀</li>
 * </ul>
 *
