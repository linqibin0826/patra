...
///
/// **职责**: 持久化待发布的 Outbox 消息,实现:
///
/// - 幂等性保证 - 通过 channel + dedupKey 唯一约束防止重复消息
///   - 批量操作 - 支持批量保存、更新、upsert 提升性能
///   - 生命周期管理 - 与 {@link OutboxRelayRepository} 协同管理消息状态转换
///
...
public interface OutboxMessageRepository {
  ...
  ///
  /// **业务含义**: 支持补偿流程(如 `publishRetry`)中的批量幂等检查。
  ///
  /// @param channel 频道标识符
  /// @param dedupKeys 幂等键列表(注意:列表不宜过大,避免 `IN` 子句性能问题)
  ...
  ///
  /// **业务含义**: 常见场景:将已有消息重置为 `PENDING`、刷新负载/头部、重置重试计数。
  ///
  ...
  ///
  /// **业务含义**: 通过唯一约束 `channel + dedupKey` 实现幂等性:
  ///
  /// - 不存在时插入新记录
  ///   - 已存在时更新负载/头部/状态并重置重试计数
  ///
  /// **并发安全**: 设计用于避免多节点并发重试同一消息时的竞态条件。
  ///
