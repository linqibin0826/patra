 ...
 * <p>Persists pending messages, enforces idempotency, and enables bulk operations. Typically coordinates with
 * {@link OutboxRelayStore} to manage the message lifecycle.</p>
 *
 * @author linqibin
 * @since 0.1.0
 */
public interface OutboxMessageRepository {
     ...
     * <p>Supports batch idempotency checks during compensation workflows (for example, {@code publishRetry}).</p>
     *
     * @param channel   channel identifier
     * @param dedupKeys idempotent keys (keep the list reasonably small to avoid large {@code IN} clauses)
     ...
     * <p>Common use: reset existing messages to {@code PENDING}, refresh payload/headers, and reset retry counts.</p>
     *
     ...
     * <p>Enforces idempotency via the unique constraint on {@code channel + dedupKey}:</p>
     * <ul>
     *   <li>Insert a new record when no match exists.</li>
     *   <li>Update payload/headers/status and reset retry count when a deduplication key already exists.</li>
     * </ul>
     * <p>Designed to avoid race conditions when multiple nodes retry the same message concurrently.</p>
     *
