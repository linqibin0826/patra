...
/**
 * Window specification value object.
 * Sealed hierarchy ensures compile-time exhaustiveness.
 *
 * <p>This represents the boundary specification for ingestion windows,
 * supporting different strategies for partitioning data collection:</p>
 * <ul>
 *   <li>TIME: Time-based windows (from/to timestamps)</li>
 *   <li>ID_RANGE: ID-based windows (from/to IDs)</li>
 *   <li>CURSOR_LANDMARK: Cursor/pagination-based windows</li>
 *   <li>VOLUME_BUDGET: Volume-limited windows</li>
 *   <li>SINGLE: Single window (no partitioning)</li>
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
...
public sealed interface WindowSpec permits 
     ...
     *
     * <p>The map structure varies by strategy type (Format B - nested JSON):</p>
     * <ul>
     *   <li>TIME: {"strategy":"TIME","window":{"from":"2024-01-01T00:00:00Z","to":"2024-12-31T23:59:59Z","boundary":{"from":"CLOSED","to":"OPEN"},"timezone":"UTC"}}</li>
     *   <li>ID_RANGE: {"strategy":"ID_RANGE","window":{"from":1000000,"to":2000000}}</li>
     *   <li>CURSOR_LANDMARK: {"strategy":"CURSOR_LANDMARK","window":{"from":"token1","to":"token2"}}</li>
     *   <li>VOLUME_BUDGET: {"strategy":"VOLUME_BUDGET","limit":100000,"unit":"RECORDS"}</li>
     *   <li>SINGLE: {"strategy":"SINGLE"}</li>
     * </ul>
     *
    ...
    record IdRange(Long from, Long to) implements WindowSpec {
        ...
        public Map<String, Object> toMap() {
            ...
            return Map.of(
                "strategy", SliceStrategy.ID_RANGE.getCode(),
                "window", windowMap
            );
        }
    }
    ...
    record CursorLandmark(String from, String to) implements WindowSpec {
        ...
        public Map<String, Object> toMap() {
            ...
            return Map.of(
                "strategy", SliceStrategy.CURSOR_LANDMARK.getCode(),
                "window", windowMap
            );
        }
    }
     ...
     *
     * <p>Expected map format (Format B - nested JSON):</p>
     * <ul>
     *   <li>TIME: {"strategy":"TIME","window":{"from":"2024-01-01T00:00:00Z","to":"2024-12-31T23:59:59Z",...}}</li>
     *   <li>ID_RANGE: {"strategy":"ID_RANGE","window":{"from":1000000,"to":2000000}}</li>
     *   <li>CURSOR_LANDMARK: {"strategy":"CURSOR_LANDMARK","window":{"from":"token1","to":"token2"}}</li>
     *   <li>VOLUME_BUDGET: {"strategy":"VOLUME_BUDGET","limit":100000,"unit":"RECORDS"}</li>
     *   <li>SINGLE: {"strategy":"SINGLE"}</li>
     ...
     * @throws IllegalArgumentException if map is null, empty, has invalid structure, or contains unknown strategy
     */
    static WindowSpec fromMap(Map<String, Object> map) {
        ...
        if (!(strategyObj instanceof String)) {
            throw new IllegalArgumentException("Window spec 'strategy' must be a string, got: " + strategyObj.getClass().getSimpleName());
        }

        String strategyCode = (String) strategyObj;
        SliceStrategy strategy = SliceStrategy.fromCode(strategyCode)
            .orElseThrow(() -> new IllegalArgumentException("Unknown slice strategy code: '" + strategyCode + "'"));
        ...
        return switch (strategy) {
            case TIME -> {
                ...
                if (!(fromObj instanceof String) || !(toObj instanceof String)) {
                    throw new IllegalArgumentException("TIME window 'from' and 'to' must be ISO-8601 timestamp strings");
                }

                yield new Time(
                    Instant.parse((String) fromObj),
                    Instant.parse((String) toObj)
                );
            }
            case ID_RANGE -> {
                ...
                yield new IdRange(
                    ((Number) fromObj).longValue(),
                    ((Number) toObj).longValue()
                );
            }
            case CURSOR_LANDMARK -> {
                ...
                if (fromObj == null || toObj == null) {
                    throw new IllegalArgumentException("CURSOR_LANDMARK window requires both 'from' and 'to' fields");
                }
                if (!(fromObj instanceof String) || !(toObj instanceof String)) {
                    throw new IllegalArgumentException("CURSOR_LANDMARK window 'from' and 'to' must be string values");
                }

                yield new CursorLandmark(
                    (String) fromObj,
                    (String) toObj
                );
            }
            case VOLUME_BUDGET -> {
                ...
                if (limitObj == null || unitObj == null) {
                    throw new IllegalArgumentException("VOLUME_BUDGET strategy requires both 'limit' and 'unit' fields");
                }
    ...
    private static Map<String, Object> extractRequiredWindowMap(Map<String, Object> map, String strategyName) {
        ...
        if (!(windowObj instanceof Map)) {
            throw new IllegalArgumentException(strategyName + " 'window' must be a JSON object, got: " + windowObj.getClass().getSimpleName());
        }
