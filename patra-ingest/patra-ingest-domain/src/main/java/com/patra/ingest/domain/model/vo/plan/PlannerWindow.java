 ...
 * 规划窗口值对象,表示任务规划的时间窗口。
 *
 * <p>使用UTC半开区间 {@code [from, to)}, {@code null}值表示无界/全量范围。
 *
 * <p>业务约束:
 *
 * <ul>
 *   <li>如果from和to都非空,则from必须早于to
 * </ul>
 *
 * <p>典型用法:
 *
 * <ul>
 *   <li>{@link #full()} 返回用于全量刷新的无界窗口
 *   <li>{@code new PlannerWindow(from, to)} 提供具体的时间切片
 *   <li>{@link #isFull()} 检查窗口是否无界
 *   <li>{@link #isEmpty()} 检测无效(非前向)范围
 * </ul>
 *
...
public record PlannerWindow(Instant from, Instant to) {
  ...
  /**
   * 创建无界/全量窗口({@code from = null}, {@code to = null})。
   *
   * @return 无界规划窗口
   */
