...
public final class TestingRules {
    ...
    public static final ArchRule test_classes_should_follow_naming_convention =
        classes()
            ...
            .and().areNotEnums()
            .and(new DescribedPredicate<JavaClass>("不是 @Nested 内部类") {
                @Override
                public boolean test(JavaClass javaClass) {
                    return !javaClass.isAnnotatedWith("org.junit.jupiter.api.Nested");
                }
            })
            .and(new DescribedPredicate<JavaClass>("不是测试工具类") {
                @Override
                public boolean test(JavaClass javaClass) {
            ...
            .should(new ArchCondition<JavaClass>("命名以 Test/IT/E2E 结尾") {
                @Override
                public void check(JavaClass javaClass, ConditionEvents events) {
                    String simpleName = javaClass.getSimpleName();
                    boolean isValid = simpleName.endsWith("Test")
                        || simpleName.endsWith("IT")
                        || simpleName.endsWith("E2E");
                    if (!isValid) {
                        String message = String.format(
                            "测试类 %s 命名不符合规范，应以 Test/IT/E2E 结尾",
    ...
    public static final ArchRule test_methods_should_have_test_annotation =
        methods()
            ...
            .or().areDeclaredInClassesThat().haveSimpleNameEndingWith("E2E")
            .and().arePublic()
            .and().areNotStatic()
            .and().doNotHaveName("setUp")
            .and().doNotHaveName("tearDown")
            .and(new DescribedPredicate<JavaMethod>("不是 @BeforeEach/@AfterEach/@BeforeAll/@AfterAll 方法") {
                @Override
            ...
            .should(new ArchCondition<JavaMethod>("有 @Test 或 @ParameterizedTest 注解") {
                @Override
                public void check(JavaMethod method, ConditionEvents events) {
                    boolean hasTestAnnotation = method.isAnnotatedWith(Test.class)
    ...
    public static final ArchRule repository_integration_tests_should_reside_in_infra =
        classes()
            ...
            .and(new DescribedPredicate<JavaClass>("类名包含 Repository") {
                @Override
                public boolean test(JavaClass javaClass) {
    ...
    public static final ArchRule tests_should_not_depend_on_other_tests =
        noClasses()
            ...
            .or().haveSimpleNameEndingWith("E2E")
            .should().dependOnClassesThat(new DescribedPredicate<JavaClass>("是其他测试类（非工具类）") {
                @Override
                public boolean test(JavaClass javaClass) {
                    String simpleName = javaClass.getSimpleName();
                    boolean isTestClass = simpleName.endsWith("Test")
                        || simpleName.endsWith("IT")
                        || simpleName.endsWith("E2E");
                    boolean isUtilityClass = simpleName.endsWith("Builder")
                        || simpleName.endsWith("Support")
                        || simpleName.endsWith("Helper")
                        || simpleName.endsWith("Collector")
                        || simpleName.endsWith("Initializer");
    ...
    public static final ArchRule e2e_tests_should_reside_in_boot_module =
        classes()
            .that().haveSimpleNameEndingWith("E2E")
            .should().resideInAPackage("..boot..")
