...
import com.patra.ingest.architecture.rules.DomainLayerRules;
import com.patra.ingest.architecture.rules.EncapsulationRules;
import com.patra.ingest.architecture.rules.LayerDependencyRules;
import com.patra.ingest.architecture.rules.NamingConventionRules;
import com.patra.ingest.architecture.rules.TestingRules;
import com.patra.ingest.architecture.rules.TransactionBoundaryRules;
 ...
 *
 * <p><b>冻结模式：</b>
 * 首次运行会记录现有违规到 {@code src/test/resources/archunit/} 目录，
 * 后续运行会禁止新增违规，并要求逐步减少现有违规。
 *
 * @author linqibin
 * @since 2025-01-10
 */
...
class IngestArchitectureTest {
    ...
    @BeforeAll
    static void setup() {
        // 导入 patra-ingest 所有模块的类（domain/app/infra/adapter/boot）
        // 自定义 ImportOption：包含 patra-ingest 模块的 JAR，排除第三方库
        classes = new ClassFileImporter()
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_TESTS)
            .withImportOption(location -> {
                // 包含 patra-ingest 模块的 JAR
                if (location.contains("/patra-ingest-")) {
                    return true;
                }
                // 排除第三方库 JAR
