# patra-object-storage-infra — 基础设施层

## 概述

**patra-object-storage-infra** 是 patra-object-storage 服务的基础设施层,负责提供领域层所需的技术实现,主要包括数据持久化、数据库访问、对象映射和数据库迁移。
...
**XML 映射示例**:
```xml
<mapper namespace="com.patra.objectstorage.infra.persistence.mapper.FileMetadataMapper">
    <select id="findByStorageKey" resultType="com.patra.objectstorage.infra.persistence.entity.FileMetadataDO">
...
- **patra-object-storage-domain**: 领域层(聚合根、值对象、端口接口)
...
- **patra-object-storage-boot**: 启动模块依赖本模块提供仓储实现
...
mybatis-plus:
  ...
  type-handlers-package: com.patra.objectstorage.infra.persistence.typehandler
```
...
- **领域层**: 参见 `patra-object-storage-domain/README.md` 了解聚合根和端口接口
- **应用层**: 参见 `patra-object-storage-app/README.md` 了解如何调用仓储
