 ...
 * 文件元数据聚合根。
 *
 * <p>表示存储在对象存储系统中的每个文件的元数据记录。作为聚合根,它封装了文件的完整生命周期信息, 包括存储位置、大小、校验和、业务上下文、状态追踪和审计信息。
 *
 * <p>聚合根通过工厂方法创建新实例({@link #create})或从持久化快照恢复({@link #restore}), 确保所有必需的业务规则在对象创建时得到验证。
 */
@Getter
@ToString
public class FileMetadata {
   ...
   *
   * <p>工厂方法用于创建新的文件元数据记录,自动设置初始状态为ACTIVE, 初始化上传时间、版本号和审计时间戳。
   *
   ...
   * 从现有的持久化快照恢复聚合根。
   *
   * <p>工厂方法用于从数据库加载已存在的元数据记录,重建完整的聚合根状态。 此方法用于仓储实现将数据实体转换为领域对象。
   *
   * @param id 主键
   ...
   * 在持久化后分配生成的标识符。
   *
   * <p>仓储实现在保存新记录后调用此方法,将数据库生成的主键ID回填到聚合根。
   *
   * @param id 数据库主键
   ...
   * 将文件标记为已删除,同时保留审计追踪。
   *
   * <p>软删除操作,将状态更新为DELETED,设置删除标志和删除时间,并更新审计信息。
   *
   ...
   * 检查元数据是否已超过保留期限。
   *
   * @return 当文件应被视为已过期时返回 {@code true}
   */
   ...
   * 返回存储的IP地址的防御性副本。
   *
   * @return 二进制IP,如果未提供则返回 {@code null}
   */
