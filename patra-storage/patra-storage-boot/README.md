...
**patra-storage-boot** 是 patra-storage 服务的 Spring Boot 启动模块,负责应用的引导和配置。本模块聚合所有其他模块(adapter、app、infra),提供可执行的 JAR 包,是整个服务的入口点。
...
│   │   └── PatraStorageApplication.java    # Spring Boot 启动类
│   └── resources/
...
- **组件扫描**: 自动扫描 `com.patra.storage` 包下的所有 Spring Bean
...
**依赖传递**:
- `patra-storage-adapter` → `patra-storage-app` → `patra-storage-domain`
- `patra-storage-infra` → `patra-storage-domain`
...
<build>
    <plugins>
        <plugin>
            ...
            <configuration>
                <mainClass>com.patra.storage.PatraStorageApplication</mainClass>
            </configuration>
            <executions>
                <execution>
                    <goals>
...
**产物**:
- `target/patra-storage-boot-0.1.0-SNAPSHOT.jar`: 可执行 JAR(包含所有依赖)
...
**方式二:可执行 JAR**
```bash
java -jar target/patra-storage-boot-0.1.0-SNAPSHOT.jar
```

**方式三:指定 Profile**
```bash
java -jar target/patra-storage-boot-0.1.0-SNAPSHOT.jar --spring.profiles.active=prod
```
...
WORKDIR /app
COPY target/patra-storage-boot-0.1.0-SNAPSHOT.jar app.jar
EXPOSE 8080
...
<configuration>
    <springProfile name="dev">
        <logger name="com.patra.storage" level="DEBUG"/>
        <logger name="org.springframework.web" level="DEBUG"/>
    </springProfile>

    <springProfile name="prod">
        <logger name="com.patra.storage" level="INFO"/>
        <logger name="org.springframework.web" level="WARN"/>
    </springProfile>

    <appender name="FILE" class="ch.qos.logback.core.rolling.RollingFileAppender">
        <file>logs/patra-storage.log</file>
        <rollingPolicy class="ch.qos.logback.core.rolling.TimeBasedRollingPolicy">
            <fileNamePattern>logs/patra-storage.%d{yyyy-MM-dd}.log</fileNamePattern>
...
**本地日志文件**:
```bash
tail -f logs/patra-storage.log
```

**Docker 容器日志**:
```bash
docker logs -f patra-storage
```
...
- **顶层 README**: 参见 `patra-storage/README.md` 了解整体架构
- **适配器层**: 参见 `patra-storage-adapter/README.md` 了解 REST 端点
- **基础设施层**: 参见 `patra-storage-infra/README.md` 了解数据库配置
