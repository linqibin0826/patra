 ...
 *
 * <ul>
 *   <li>✅ 枚举值存在性测试（MINIO, S3, OSS, COS）
 *   <li>✅ values() 方法测试（数量、包含关系、顺序）
 *   <li>✅ valueOf() 方法测试（有效字符串、无效字符串、null）
 *   <li>✅ name() 方法测试（枚举名称验证）
 *   <li>✅ ordinal() 方法测试（枚举序号验证）
 *   <li>✅ fromName() 自定义方法测试（大小写不敏感、异常场景）
 *   <li>✅ equals() 和 == 比较测试
 *   <li>✅ 枚举单例不变性测试
 *   <li>✅ 真实场景集成测试（存储提供商选择、配置解析）
 * </ul>
 *
 * @author linqibin
 * @since 0.2.0
