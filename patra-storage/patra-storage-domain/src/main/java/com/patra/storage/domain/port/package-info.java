 ...
 * Storage 领域层端口包。
 *
 * <p>本包定义了 patra-storage 服务的领域层端口(Port),作为六边形架构的核心抽象。
 * 端口接口定义了领域层与外部基础设施层的交互契约,实现了依赖倒置原则,确保领域层独立于技术实现细节。
 *
 * <h2>核心端口</h2>
 *
 * <ul>
 *   <li>{@link com.patra.storage.domain.port.FileMetadataRepository} - 文件元数据仓储端口,定义聚合根的持久化操作
 * </ul>
 *
 * <h2>端口类型</h2>
 *
 ...
 * </ul>
 *
 * {@link com.patra.storage.domain.port.FileMetadataRepository} 是被驱动端口,由基础设施层的适配器实现。
 *
 * <h2>设计原则</h2>
 *
 * <ul>
