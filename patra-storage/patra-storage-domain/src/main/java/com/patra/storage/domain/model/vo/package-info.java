 ...
 * Storage 领域模型值对象包。
 *
 * <p>本包包含 patra-storage 服务的值对象(Value Object),作为六边形架构领域层的重要组成部分。
 * 值对象封装了领域概念,提供不可变性、自我验证和业务语义,是构建健壮领域模型的基石。
 *
 * <h2>核心值对象</h2>
 *
 * <ul>
 *   <li>{@link com.patra.storage.domain.model.vo.StorageKey} - 不可变的存储定位符,包含 bucket 和 objectKey
 *   <li>{@link com.patra.storage.domain.model.vo.FileSize} - 文件大小封装,单位字节
 *   <li>{@link com.patra.storage.domain.model.vo.FileChecksum} - 校验和封装,包含 md5Hash 和 sha256Hash
 *   <li>{@link com.patra.storage.domain.model.vo.BusinessContext} - 业务上下文封装,包含服务名、业务类型、业务标识、关联数据
 * </ul>
 *
 * <h2>值对象特性</h2>
 *
