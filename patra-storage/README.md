...
```
patra-storage/
├── patra-storage-api/        # 外部契约层 - Feign 客户端接口 + DTO
├── patra-storage-domain/     # 领域层 - 聚合根、值对象、仓储端口(纯 Java)
├── patra-storage-app/        # 应用层 - 用例编排器、事务管理
├── patra-storage-infra/      # 基础设施层 - MyBatis 仓储实现 + Flyway 迁移
├── patra-storage-adapter/    # 适配器层 - REST 控制器、端点实现
└── patra-storage-boot/       # 启动模块 - Spring Boot 应用入口
```
...
# 查看服务注册(如果使用 Nacos)
curl http://localhost:8848/nacos/v1/ns/instance/list?serviceName=patra-storage
```
...
- **领域模型**: 参见 `patra-storage-domain/README.md`
- **数据库设计**: 参见 `patra-storage-infra/src/main/resources/db/migration/V0.1.0__init_storage_schema.sql`
