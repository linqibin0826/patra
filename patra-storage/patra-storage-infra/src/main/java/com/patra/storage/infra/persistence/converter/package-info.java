 ...
 *
 * <ul>
 *   <li><strong>领域 → 数据</strong>: 将聚合根转换为数据对象(DO),用于数据库持久化
 *   <li><strong>数据 → 领域</strong>: 将数据对象转换为聚合根,用于业务逻辑处理
 ...
 *   <li><strong>复杂类型映射</strong>: 处理 JSON 字段、二进制字段、时间戳等复杂类型
 * </ul>
 *
 * <h2>设计原则</h2>
 *
 * <ul>
 *   <li><strong>使用 MapStruct</strong>: 通过注解处理器在编译期生成转换代码,性能高且类型安全
 *   <li><strong>显式映射</strong>: 避免隐式类型转换,所有映射规则明确定义
 *   <li><strong>空值处理</strong>: 对可选字段执行空值检查,防止 NullPointerException
 *   <li><strong>防御性拷贝</strong>: 对可变字段(如 Map、数组)执行防御性拷贝
 *   <li><strong>单向依赖</strong>: 转换器依赖领域层和数据层,但两者不依赖转换器
 * </ul>
 *
 * <h2>FileMetadataConverter 转换器</h2>
 *
 * 文件元数据转换器定义了以下映射方法:
 *
 * <ul>
 *   <li>{@code toAggregate(FileMetadataDO)} - 将数据对象转换为聚合根
 *       <ul>
 *         <li>使用 {@code FileMetadata.restore()} 工厂方法重建聚合根
 *         <li>转换值对象(StorageKey、FileSize、FileChecksum、BusinessContext)
 *         <li>转换枚举类型(FileStatus、StorageProvider)
 *         <li>处理 JSON 字段(correlationData、recordRemarks)
 *       </ul>
 *   <li>{@code toDO(FileMetadata)} - 将聚合根转换为数据对象
 *       <ul>
 *         <li>提取聚合根字段填充数据对象
 *         <li>分解值对象为基本类型字段
 *         <li>序列化 Map 为 JSON 字符串
 *       </ul>
 * </ul>
 *
 * <h2>MapStruct 注解示例</h2>
 *
 * <pre>{@code
 ...
 *
 * <ul>
 *   <li><strong>编译期生成</strong>: 避免运行时反射,性能接近手写代码
 *   <li><strong>类型安全</strong>: 编译期检查映射规则,避免运行时类型错误
 *   <li><strong>代码可读</strong>: 生成的代码可调试,便于问题排查
 *   <li><strong>零配置</strong>: 无需 XML 配置,注解驱动即可
 * </ul>
 *
 * <h2>相关文档</h2>
 *
 ...
 *   <li>数据对象: {@link com.patra.storage.infra.persistence.entity.FileMetadataDO}
 *   <li>仓储实现: {@link com.patra.storage.infra.persistence.repository.FileMetadataRepositoryImpl}
 *   <li>MapStruct 官方文档: <a href="https://mapstruct.org/">mapstruct.org</a>
 * </ul>
 *
 * @author linqibin
 * @since 0.1.0
