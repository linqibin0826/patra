...
export function FilterTrigger({ count }: { count: number }) {
  ...
  return (
    ...
    >
      ...
      筛选
      {count > 0 && (
        <span className="absolute -top-1.5 -right-1.5 flex size-4 items-center justify-center rounded-full bg-action-primary text-3xs font-semibold text-fg-on-clay">
          {count}
        </span>
      )}
    </button>
