...
**产物保存位置：** `<git-root>/docs/patra/release-specs/<version>.md`（默认）
...
- 这条规则适用于 skill-creator / eval 等评测场景的 `outputs/` 目录——不要污染默认的 `<git-root>/docs/patra/release-specs/`
...
1. **探索项目上下文** — 按以下顺序检查（视可用性灵活降级，**不阻塞**）：
   ...
   - **`<git-root>/docs/patra/release-specs/`** — 读上一版 release-spec，了解上一版做了什么、留了什么"不做"承诺
...
1. **写到 `<git-root>/docs/patra/release-specs/<version>.md`**（Markdown 格式）
...
本 skill 是 `<git-root>/docs/patra/specs/2026-05-19-solo-dev-sop-design.md` 中 §2.2 Project 层执行 skill。Skill 行为细节（字段定义、产物路径、终止状态）必须与 SOP 文档保持一致。SOP 文档修订时同步检查本 skill；本 skill 修订时同步检查 SOP §2.2 / §5.1 / §8.1 / §9.2 / §9.4。
