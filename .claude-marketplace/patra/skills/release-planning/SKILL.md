...
- ✅ 优秀示例：
  - "[BE] 实现 GET /api/v1/health（聚合 registry/ingest/catalog actuator）"（area:be）
  - "[Design] portal Health 卡片 hi-fi 稿（3 卡片 + 状态色 + 时延）"（area:design）
  - "[FE] portal 首页 Health 卡片组件 + 30s 轮询"（area:fe）
- ❌ 反面示例：
  - "用户能看到服务健康状态"（用户故事——Linear Method 反对）
  - "Health 功能"（粒度太粗，BE+Design+FE 都塞）
...
3. **对第 2 字段的每条工程任务，提议在 Linear 创建 1 个 Issue**：
   - **Title**：以 `[area]` 前缀开头的工程任务描述（如 `[BE] 实现 GET /api/v1/health`）
   - **Description**：Magical.pm Everyday 三字段模板：
     ```markdown
     ## Overview
   ...
   - **Status**：`Backlog`
   - **Labels**：
     - 必填 area：`area:be` / `area:fe` / `area:design`（视交付物类型）
...
本 skill 是 `<git-root>/docs/patra/specs/2026-05-19-solo-dev-sop-design.html` 中 §4 Project 层的执行 skill。Skill 行为细节（字段定义、产物路径、Issue 派生模板、终止状态）必须与 SOP 文档保持一致。SOP 修订时同步检查本 skill；本 skill 修订时同步检查 SOP 以下章节：

- **§3 双层架构** — Project / Issue / Sub-Issue 三层定义
- **§4 Project 层** — Project 状态机、5 字段模板、Entry Gate
- **§5.1 Issue 状态机** — Linear 默认 5 态（Backlog / Todo / In Progress / In Review / Done）
- **§5.3 Issue 标签分流** — area:be / area:fe / area:design / path:* / review:*
- **§6.1 Tech Design 活动** — Project 启动期一次性产物
- **§9.1 path:fast-track / §9.2 Project Milestone 表达阶段顺序**
- **§10 决策 4（Issue 状态机使用 Linear 默认 5 态）+ 决策 8（Sub-Issue 不进默认派生流程）** — 派生模型的决策依据
