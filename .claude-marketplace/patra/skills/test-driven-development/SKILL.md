...
| 维度 | 规范 |
|------|------|
| 测试位置 | `src/test/java/{对应包路径}/` ——跟生产代码 `src/main/java/` 镜像 |
| 测试类命名 | `{被测类}Test`（例：`ProvenanceCode` → `ProvenanceCodeTest`） |
...
| 运行命令 | `./gradlew :module-name:test`（整模块） 或 `./gradlew :module-name:test --tests "*ClassName*"`（精确） |
