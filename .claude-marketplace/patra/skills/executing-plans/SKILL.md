...
详细状态机与切换约定见 `skills/brainstorming/html-output-guide.md` 的"AI 代理状态切换约定"章节。简表：
...
1. 读取计划文件（路径形如 `docs/patra/plans/YYYY-MM-DD-<feature>.html`）
...
**审查示例：**
```
计划文件：docs/patra/plans/2026-05-19-add-user-validator.html
任务清单：5 个任务
...
1. **标记为进行中** — 编辑 plan HTML：`<article id="task-N" data-status="pending">` → `data-status="in-progress"`
2. **理解目标** — 重读任务描述，明确完成标准
3. **执行实现** — 严格按计划的 5 步 TDD 循环执行（写测试 → 跑测试失败 → 写实现 → 跑测试通过 → commit）。每开始一个步骤把对应 `<li class="step" data-status="pending">` 改为 `"in-progress"`，做完改 `"done"`。
4. **运行验证** — 按要求运行测试或检查
5. **提交变更** — 每完成一个任务提交一次，commit message 引用任务编号
6. **标记为已完成** — 编辑 plan HTML：article 的 `data-status` 改为 `"done"`，确认 5 个 `<li class="step">` 都是 `"done"`
...
→ 编辑 plan HTML：<article id="task-3"> 的 data-status="in-progress" 改为 data-status="blocked"
  并在 article 内插入 <aside class="blocker">任务 3 需要 Redis 连接，
  计划中未包含配置步骤。建议：在任务 3 前插入 "配置 Redis 连接" 步骤。</aside>
→ 停止执行
→ 向伙伴报告
```
...
**计划：** docs/patra/plans/2026-05-19-add-user-validator.html
...
### 偏离计划的地方
- 任务 3：Flyway 脚本编号从 V12 改为 V13（经伙伴同意，避开本地已存在的 V12）
