...
- **同步执行**：JobLauncher 使用 `SyncTaskExecutor`，适配 XXL-Job 调度
...
public class MyJobService {
    ...
    public Long runJob(String param) {
        return jobLauncherHelper.launch(myJob, Map.of("param", param));
    }
}
```
...
```java
// 默认：每次执行创建新实例
jobLauncherHelper.launch(job, Map.of("date", "2024-01-01"));

// 幂等：相同参数只执行一次
jobLauncherHelper.launch(job, Map.of("date", "2024-01-01"), false);

// 查询执行状态
jobLauncherHelper.findJobExecution(executionId)
    .ifPresent(execution -> {
...
### 同步执行（SyncTaskExecutor）

**问题**：Spring Batch 默认使用 `SimpleAsyncTaskExecutor` 异步执行 Job。

**影响**：
- XXL-Job 已提供异步调度能力（线程池执行）
- 二次异步导致 XXL-Job 无法正确追踪执行状态
- 分布式锁在 Job 启动后立即释放（而非执行完成后）

**解决方案**：使用 `SyncTaskExecutor` 同步执行，确保：
- XXL-Job 能正确追踪 Job 执行状态
- 分布式锁在 Job 执行期间有效
- 异常能正确传播到调度层
